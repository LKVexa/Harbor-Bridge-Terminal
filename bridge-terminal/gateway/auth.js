...
function staticFile(file, log) {
  ...
  return {
    ...
    stillValid(principal) {
      reload();
      // Valid only if SOME current entry for this principal is live and still grants every capability the session holds
      // (so removing `fabric` from the file ends fabric-capable sessions too).
      return entries.some((x) => x.sub === principal.sub && x.tenant === principal.tenant && !x.revoked && (x.notAfter === null || Date.now() < x.notAfter) && [...principal.capabilities].every((c) => x.capabilities.has(c)));
    }
  };
}
