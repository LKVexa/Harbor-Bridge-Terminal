...
class Studio:
    ...
    def fabric_tick(self, ref: str, adapter: str = "deterministic",
        ...
        if prev["pages"] >= fabric_tif.MAX_PAGES:
            raise fabric_tif.FabricError("fabric history is full; checkpoint before executing another tick")
        if prev["tick"] == (1 << 64) - 1:
            raise fabric_tif.FabricError("fabric tick counter exhausted; no VM execution attempted")
        ...
        with open(run_state, "rb") as fh:
            new_blob = fh.read(fabric_tif.G.MAX_BLOB_BYTES + 1)
        history = fabric_tif.load_frames(tif)
        tick = prev["tick"] + 1
        try:
            info = fabric_tif.write_tif(new_blob, tif, tick=tick, history=history,
                                         expected_sha256=prev["file_sha256"])
        finally:
