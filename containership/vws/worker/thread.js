...
parentPort.on('message', (m) => { if (typeof m.t === 'string' && m.t.startsWith('ship.')) require('../protocol/bridge').validateRecord(m, 'gateway_to_worker'); session.handle(m, m.t === 'input' ? Buffer.from(m.buf) : null); });
