...
function commandFilter({ fabric, ship = false }) {
  return (d) => !DENY_REMOTE.has(d.name) && (fabric || !FABRIC.has(d.name)) && (ship || d.name !== 'ship');
}
function advertised({ fabric, ship = false, shipControl = false }) {
  return { virtualCommands: true, browserPane: false, fabric: !!fabric, photon: false, ship: !!ship, shipControl: !!shipControl };
}
