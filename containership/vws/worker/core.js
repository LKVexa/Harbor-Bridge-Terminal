...
function readConfig(env) {
  ...
  return {
    ship: env.VWS_W_SHIP === '1',
    shipControl: env.VWS_W_SHIP_CONTROL === '1',
...
function createWorkerSession({ env, emit, onFinished, diag }) {
  ...
  const shipClient = cfg.ship ? require('../ship/worker-client').createShipClient(emit, { control: cfg.shipControl }) : null;
  ...
  function buildKernel() {
    ...
    if (shipClient) host.shipBridge = shipClient;
    ...
    const k = new SpiralKernel({ hostBridge: host, limits: { captureBytes: cfg.captureBytes, historyEntries: cfg.historyEntries }, vfsLimits: { maxBytes: cfg.vfsBytes, maxNodes: cfg.vfsNodes }, commandFilter: commandFilter({ fabric: cfg.fabric, ship: cfg.ship }) });
  ...
  function finish(code, reason) {
    ...
    if (shipClient) shipClient.close();
  ...
  function handle(rec, bytes) {
    switch (rec.t) {
      case 'ship.chunk': case 'ship.result': if (shipClient) shipClient.handle(rec); return;
      case 'open': {
  ...
  return { handle, finish, sinkDrained: () => { sinkFull = false; gate.sinkDrained(); }, gate, cfg, ready: () => emit({ t: 'ready', pid: process.pid, version: VERSION, capabilities: advertised({ fabric: cfg.fabric, ship: cfg.ship, shipControl: cfg.shipControl }) }),
