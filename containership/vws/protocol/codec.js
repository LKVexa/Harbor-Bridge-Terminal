...
(function (root, factory) {
  ...
  const SUPPORTED = new Set(['$schema', '$id', 'title', 'description', 'oneOf', 'type', 'properties', 'required',
    'additionalProperties', 'const', 'enum', 'minimum', 'maximum', 'minLength', 'maxLength', 'pattern', 'contentEncoding', 'items', 'minItems', 'maxItems']);
  ...
  function assertSupported(schema) {
    ...
    for (const k of Object.keys(schema)) {
      ...
      else if (k === 'items') assertSupported(schema[k]);
    }
  }
  ...
  function check(value, schema, path) {
    ...
    if (Array.isArray(value)) {
      if (schema.minItems !== undefined && value.length < schema.minItems) return `${path}: minItems`;
      if (schema.maxItems !== undefined && value.length > schema.maxItems) return `${path}: maxItems`;
      if (schema.items) for (let i = 0; i < value.length; i++) { const e = check(value[i], schema.items, `${path}[${i}]`); if (e) return e; }
    }
