...
// The inherited stress workload exceeded seq's own one-million-item guard,
// so it never exercised capture/heap enforcement and waited for an unrelated error.
// Stay below that guard but above the configured 1 MiB pipeline-capture budget.
// This proves bounded capture and tenant responsiveness, NOT a V8 OOM or native-child RAM limit.
for (const backend of ['thread', 'process']) {
  test(`[${backend}] allocation guard rejects oversized pipeline capture and another tenant remains responsive`, async () => {
    const h = await startGateway({ workerBackend: backend, workerHeapBytes: 16 * 1048576, workerRuntimeOverheadBytes: 12 * 1048576 });
    try {
      const a = await new Client(h.url, h.tokens.alice).open();
      const b = await new Client(h.url, h.tokens.bob).open();
      a.input('seq 1 1000000 | wc -l\r');
      await a.until(() => /pipeline capture limit exceeded/.test(a.text()), 20000, 'configured capture budget');
      assert.match(await b.run('echo other-ok', 'other-ok\r\n'), /other-ok/);
      const live = await (await fetch(h.http + '/live')).json();
      assert.strictEqual(live.status, 'live');
      assert.strictEqual(live.ram.heapCap, h.gw.ram.heapCap, 'heap capability is reported separately from capture-budget enforcement');
      a.close(); b.close();
    } finally { await h.stop(); }
  });
}
