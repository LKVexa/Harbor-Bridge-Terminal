...
const CAPS = new Set(['terminal', 'fabric', 'ship.read', 'ship.control']);
...
function staticFile(file, log) {
  ...
  return {
    ...
    authenticate(token) {
      ...
      return { sub: hit.sub, tenant: hit.tenant, credentialId: hit.hash.toString('hex'), capabilities: new Set(hit.capabilities), notAfter: hit.notAfter };
    },
    /** Still valid? Used to end live sessions after revocation/expiry. */
    stillValid(principal) {
      reload();
      // Valid only if this exact credential is live and still grants every capability the session holds
      // (so removing `fabric` from the file ends fabric-capable sessions too).
      return entries.some((x) => x.sub === principal.sub && x.tenant === principal.tenant && x.hash.toString('hex') === principal.credentialId && !x.revoked && (x.notAfter === null || Date.now() < x.notAfter) && [...principal.capabilities].every((c) => x.capabilities.has(c)));
    }
  };
}
