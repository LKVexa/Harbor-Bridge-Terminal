...
class Connection {
  constructor({ ws, principal, origin, cfg, codec, log, registry, ram, account, remote }) {
    ...
    this._control({ v: 2, type: 'hello', payload: { protocol: 'hermit.vws.v2', serverEpoch: ram.serverEpoch, profile: 'LOCAL_VOLATILE',
      ...
      capabilities: { resume: false, browserPane: false, virtualCommands: true, fabric: !!(cfg.fabric && principal.capabilities.has('fabric')), ...(cfg.shipRoot ? { ship: principal.capabilities.has('ship.read'), shipControl: !!(cfg.shipControl && principal.capabilities.has('ship.read') && principal.capabilities.has('ship.control')) } : {}) } } });
  ...
  _open(msg) {
    ...
    if (cfg.python && fabric) env.VWS_W_PYTHON = cfg.python;
    if (cfg.shipRoot && this.principal.capabilities.has('ship.read')) Object.assign(env, { VWS_W_SHIP: '1', VWS_W_SHIP_CONTROL: cfg.shipControl && this.principal.capabilities.has('ship.control') ? '1' : '0' });
    const g = this.gen;
  ...
  _fromWorker(r, bytes, info) {
    switch (r.t) {
      case 'ship.request': return this._shipRequest(r);
      case 'ship.cancel': if (this.registry.shipBroker) this.registry.shipBroker.cancel(this.id, r.id); return;
  ...
  _shipRequest(r) {
    const broker = this.registry.shipBroker;
    const reply = (record, callback) => this.link ? this.link.send(record, null, callback) : (callback && callback(new Error('worker closed')));
    let lease, job;
    try {
      require('../protocol/bridge').validateRecord(r, 'worker_to_gateway');
      if (this.closing || this.state !== 'ACTIVE' || !broker || !this.principal.capabilities.has('ship.read') || !this.registry.identity.stillValid(this.principal)) throw new Error('ship capability is unavailable or revoked');
      if (this.shipPending) throw new Error('one ship command per session is permitted');
      lease = this.account.reserveCapacity('ship_bridge', this.cfg.shipOutputBytes * 16, 'ship-broker');
      job = broker.run(this.id, r.id, r.argv, { control: this.principal.capabilities.has('ship.control') });
      this.shipPending = r.id;
    } catch (e) {
      if (lease) lease.release();
      if (typeof r.id === 'string' && /^[a-f0-9]{24}$/.test(r.id)) reply({ t: 'ship.result', id: r.id, code: 3, outcome: 'REFUSED', reason: String(e.message).slice(0, 256) });
      return;
    }
    const g = this.gen;
    job.then(result => {
      let released = false; const release = () => { if (released) return; released = true; if (this.shipPending === r.id) this.shipPending = null; lease.release(); };
      if (g !== this.gen || !this.link || this.closing) { release(); return; }
      try {
        for (const stream of ['stdout', 'stderr']) {
          const b = result[stream];
          for (let i = 0; i < b.length; i += 4096) {
            const ok = reply({ t: 'ship.chunk', id: r.id, stream, data: b.subarray(i, i + 4096).toString('base64') });
            if (!ok) { release(); this._workerFailed('ship bridge output failed'); return; }
          }
        }
        reply({ t: 'ship.result', id: r.id, code: result.code, outcome: result.outcome, reason: result.reason.slice(0, 256) }, release);
      } catch (e) { release(); this._workerFailed('ship bridge delivery failed'); }
    }).catch(() => { this.shipPending = null; lease.release(); this._workerFailed('ship broker failed'); });
  }
  ...
  _workerFailed(why) { if (this.registry.shipBroker) this.registry.shipBroker.cancelOwner(this.id); this.workerExit = { code: 70, reason: 'worker_failure' }; if (this.link) this.link.kill(); this.log.warn('worker.failed', { conn: this.id, why }); this._finalExit(); this._closeSocket(1011, 'worker failure'); }
  _workerExited(e) {
    if (this.registry.shipBroker) this.registry.shipBroker.cancelOwner(this.id);
