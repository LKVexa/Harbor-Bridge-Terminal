...
function createGateway(cfg, { logSink } = {}) {
  ...
  if (cfg.shipRoot) connReservation.parts.ship_bridge = cfg.shipOutputBytes * 16;
  ...
  const registry = {
    identity, shipBroker: cfg.shipRoot ? new (require('../ship/broker').ShipBroker)(cfg, log) : null, draining: false, ready: false, sessionCap: memoryPlan.admittedSessionCap,
  ...
  const server = http.createServer((req, res) => {
    ...
    try {
      ...
      if (p === '/live') {
        ...
        return json(res, 200, { status: 'live', uptimeS: Math.floor((Date.now() - startedAt) / 1000), sessions: registry.reserved.size, connections: registry.conns.size, draining: registry.draining, ship: registry.shipBroker ? registry.shipBroker.telemetry() : null,
  ...
  function shutdown(reason = 'shutdown') {
    ...
    const shipClose = registry.shipBroker ? registry.shipBroker.close() : Promise.resolve();                       // /health -> 503, tickets + upgrades refused from now on
    ...
    shutdownPromise = new Promise((resolve) => {
      ...
      const finish = (forced) => {
        ...
        shipClose.then(() => resolve({ forced, drainMs: Date.now() - t0 }));
      };
