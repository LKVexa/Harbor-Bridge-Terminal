...
function load(env = process.env) {
  ...
  const cfg = {
    ...
    shipRoot: env.VWS_SHIP_ROOT ? path.resolve(env.VWS_SHIP_ROOT) : null,
    shipControl: boolVar(env, 'VWS_SHIP_CONTROL', false),
    shipOutputBytes: intVar(env, 'VWS_SHIP_OUTPUT_BYTES', 131072, 8192, 1048576),
    shipReadMs: intVar(env, 'VWS_SHIP_READ_MS', 60000, 1000, 300000),
    shipControlMs: intVar(env, 'VWS_SHIP_CONTROL_MS', 180000, 1000, 900000),
  ...
  if (cfg.shipControl && !cfg.shipRoot) throw new ConfigError('VWS_SHIP_CONTROL requires VWS_SHIP_ROOT');
  if (cfg.shipRoot) {
    if (!['127.0.0.1', '::1'].includes(cfg.host)) throw new ConfigError('containership bridge requires a numeric loopback bind');
    if (cfg.auth !== 'static-file') throw new ConfigError('containership bridge requires authenticated static-file principals');
    if (cfg.fabric) throw new ConfigError('legacy DF control cannot be combined with the ship bridge');
    if (!cfg.python || !path.isAbsolute(cfg.python)) throw new ConfigError('ship bridge requires an absolute PYTHON executable');
    try { if (!fs.statSync(cfg.python).isFile() || !fs.statSync(path.join(cfg.shipRoot, 'uc.py')).isFile()) throw new Error('not a file'); }
    catch { throw new ConfigError('ship root or Python executable is missing'); }
  }
...
function publicConfig(cfg) {
  return { protocol: 'hermit.vws.v2', profile: cfg.profile, wsPath: '/ws/terminal', ticketPath: '/api/ws-ticket', auth: cfg.auth === 'none-loopback-dev' ? 'none' : 'bearer-ticket',
    capabilities: { resume: false, browserPane: false, virtualCommands: true, fabric: !!cfg.fabric, ship: !!cfg.shipRoot, shipControl: !!cfg.shipControl },
