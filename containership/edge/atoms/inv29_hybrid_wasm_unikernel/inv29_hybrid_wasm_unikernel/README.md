...
**Master prompts:** `MASTER.md` is not included (owner action INV29-MC007).
**Production status:** see `conformance/PK_GATE_RESULTS.json` (machine-derived) and `MISSING_COMPONENTS_STATUS.json`.
...
| Module | Role |
|---|---|
| `model.py` | layer + import-closure invariants (`verify`, `compose`) |
| `admission.py` | identity, digests, attestations, policy, replay, signed records |
| `interfaces.py` | typed capability/interface link check |
| `records.py` + `schemas/` | strict parser, JSON Schemas, redaction |
| `deps.py` | dependency boundary, timeouts, retries, breaker, test doubles |
| `lifecycle.py` | store, state machine, reconcile, backup/restore, control file |
| `telemetry.py`, `service.py` | metrics, logs, traces, decisions, HTTP endpoints |
| `evidence.py` | ledger, waivers, machine-derived gate |
| `component.py`, `contract.py` | pk_core 100-item assessment (needs pk_core) |

Standalone (no pk_core):

```
python tools/run_tests.py          # every suite, normal and -O -> conformance/test_results.json
python tools/bench.py              # perf baseline / regression gate
python tools/fuzz.py 600           # long fuzz run
python tools/build_release.py      # lock, SBOM, checksums, provenance, scans, traceability, gate
python tools/inv29ctl.py --help    # disable / enable / backup / restore / rollback / verify-ledger
```

## Running it with pk_core

```
python inv29_hybrid_wasm_unikernel/tests/test_model.py       # standalone core security/correctness tests
python inv29_hybrid_wasm_unikernel/tests/test_component.py   # pk_core conformance; set PK_CORE_PATH if needed
