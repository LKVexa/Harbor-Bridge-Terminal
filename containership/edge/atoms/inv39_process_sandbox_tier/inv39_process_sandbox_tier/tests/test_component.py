"""Conformance test for INV-39 (inv39_process_sandbox_tier) -- stdlib unittest, no network.

Run from the folder that contains this package (and ``pk_core``), e.g.::

    python inv39_process_sandbox_tier/tests/test_component.py
...
def _gate(cls):
    """No silent skips (MC-094): on a certification target a missing pk_core FAILS."""
    if pk_core is not None:
        return cls
    if os.environ.get("INV39_CERT_TARGET") == "1":
        def test_pk_core_present(self):
            self.fail("pk_core is required for 100-check conformance on a certification target")
        return type(cls.__name__, (unittest.TestCase,), {"test_pk_core_present": test_pk_core_present})
    return unittest.skip("pk_core not importable; set PK_CORE_PATH (MC-094 stays BLOCKED)")(cls)


@_gate
class ConformanceTest(unittest.TestCase):
    def test_version(self):
        self.assertEqual(_load().__version__, "5.1.0")
        self.assertEqual((PKG_DIR / "VERSION").read_text().strip(), "5.1.0")
    ...
    def test_all_100_requirements_answered(self):
        ...
        # In a partial estate install, a sibling component may be absent; all other
        # non-passing findings are unexpected unless explicitly listed above.
