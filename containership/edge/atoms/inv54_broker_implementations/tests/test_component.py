"""Conformance test for INV-54 (inv54_broker_implementations) -- stdlib unittest, no network.

Run from the folder that contains this package (and ``pk_core``), e.g.::

    python inv54_broker_implementations/tests/test_component.py
...
class VersionTest(unittest.TestCase):
    def test_version(self):
        # single source of truth: VERSION; package, pyproject and CHANGELOG must agree (C100 / roll-up)
        v = (PKG_DIR / "VERSION").read_text().strip()
        self.assertEqual(_load().__version__, v)
