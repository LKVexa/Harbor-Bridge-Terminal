...
# Addendum — v4.3.0 completion pass (2026-09-23)

The missing-components checklist (`docs/COMPLETION_CHECKLIST_v4.2.0.md`, M01–M32, 1,345 items)
was executed against this package. Every item now carries a status and evidence in
`CHECKLIST_STATUS.json` / `CHECKLIST_STATUS.md`, assigned by two independent assessment passes
(pass 1 over all 1,345 items before fixes; pass 2 re-checked the 295 items the fixes touched).
`tools/checklist_status.py --check` verifies that every cited test id still exists.

| Status | Items |
|---|---|
| LOCALLY_VERIFIED — code + executed test in this build | 184 |
| DOCUMENTED | 35 |
| PARTIAL | 460 |
| OPEN — engineering work, no outside input needed | 402 |
| BLOCKED — needs an owner, pk_core, MASTER.md, CI runners, other hosts/OS, PKI/signing, real neighbours | 264 |

No component's Definition-of-Done gate is fully met. **Production NOT authorised.**

Defects found by this pass and fixed with regression tests are listed in CHANGELOG 4.3.0.
Measured, not claimed: envelope overhead p99 23 µs (meets the 50 µs SLO); including argument
encoding the p99 is ~190 µs (does not); loopback call p99 ~0.9 ms; 60 s soak with random connection
kills: ~129 000 calls, 0 unexpected outcomes, audit chain intact, peak RSS ~110 MB (the 100 000-entry
idempotency cache). A 150 s run (`evidence/soak_150s.json`): ~320 000 calls, 0 unexpected, peak RSS ~121 MB —
roughly levelling off (+10 % for 2.5× the calls), consistent with the cache cap, though a multi-hour soak has not been run.
The release-gate runner (`tools/run_gate_tests.py`) **fails** in this build by design: the three
`pk_core` conformance tests and the pk_core gate test skip (`evidence/release_gate_run.txt`).
