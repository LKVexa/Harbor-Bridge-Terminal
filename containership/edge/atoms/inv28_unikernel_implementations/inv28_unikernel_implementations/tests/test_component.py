"""Conformance test for INV-28 (inv28_unikernel_implementations) -- stdlib unittest, no network.

Run from the folder that contains this package (and ``pk_core``), e.g.::

    python inv28_unikernel_implementations/tests/test_component.py

``pk_core`` resolves from PK_CORE_PATH, an installed copy, or the vendored ``_vendor/pk_core`` (MC-002).
"""
import importlib
import os
import pathlib
...
class ConformanceTest(unittest.TestCase):
    ...
    def test_checks_survive_optimised_mode(self):
        ...
        out = subprocess.run([sys.executable, "-B", "-O", "-W", "ignore", "-c", code], capture_output=True, text=True, cwd=str(ROOT))
