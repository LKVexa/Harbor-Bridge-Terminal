"""pk_core integration/conformance tests for SCH-01.

The dependency-independent engine suite is in ``test_engine.py`` and always runs.
These tests run when the external ``pk_core`` framework is available.
"""
...
class ConformanceTest(unittest.TestCase):
    def test_all_100_requirements_answered(self):
        comp_type = _load().COMPONENT
        self.assertIsNotNone(comp_type)
        comp = comp_type()
