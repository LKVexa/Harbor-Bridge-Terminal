...
def build() -> Contract:
    ...
    return Contract(
        ...
        source_of_truth="Fresh measured thermal and power signals plus the active validated thermal policy; missing, stale, or unusable sensor evidence is treated as constrained, not as cool.",
        assumptions=[
            "Sensors may be absent on cheap edge hardware",
            "Thermal response lags load by seconds to minutes",
            "Telemetry has an explicit freshness window and bounded future-clock skew",
        ...
        interfaces={
            "thermal": "PK_THERMAL_STATE/1 - measured temperature, power draw/budget, battery state and observation time",
            "ceiling": "PK_POWER_CEILING/1 - derived capacity ceiling, exclusion verdict, telemetry status and reasons",
            "policy": "PK_THERMAL_POLICY/1 - validated thermal/power thresholds, freshness limits and hysteresis per environment"
        },
        threats=[
