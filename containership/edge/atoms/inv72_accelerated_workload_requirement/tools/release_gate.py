"""INV-72 release gate (C090, C100, C070, C099; global definition of done).

    python -m inv72_accelerated_workload_requirement.tools.release_gate [--pk-gate FILE] [--approval FILE]
...
Runs (or reads) every mandatory lane and writes machine-readable release evidence.  Verdict GO only when:
  ...
  * RTM validation, dependency check and manifest verification pass
  ...
  * the vendored pk_core conformance gate returns GO (tools/pk_gate.py; necessary, never sufficient)
...
def evaluate(lanes: dict, rtm: dict, waivers: dict, governance: dict, perf: dict | None, pk_gate: dict | None,
    ...
    if not pk_gate or pk_gate.get("verdict") != "GO":
        blockers.append("no PASSing pk_core PK_GATE_RESULTS supplied (tools/pk_gate.py)")
    approver = str((approval or {}).get("approver", ""))
    sec_owner = next((r["holder"] for r in owners["roles"] if r["alias"] == "inv72-security-owner"), None)
...
def main(argv=None) -> int:
    ...
    if not a.pk_gate:
        _run(["-B", "-m", f"{pkg}.tools.pk_gate"])
        a.pk_gate = str(ROOT / "evidence" / "PK_GATE_RESULTS.json")
    ...
    evidence = {
        "schema": "PK_RELEASE_EVIDENCE/1", "element": "INV-72", "version": (ROOT / "VERSION").read_text().strip(),
