"""Governance checks (C009, C098, C099, C094 EOL, C016 packaging consistency).
    ...
    python -m inv72_accelerated_workload_requirement.tools.governance_check [--today YYYY-MM-DD] [--json]
...
def check_owners() -> list[str]:
    ...
    so, ra = holders.get("inv72-security-owner"), holders.get("inv72-release-approver")
    ...
    for path in ("/matcher.py", "/state.py", "/trust.py", "/discovery.py", "/config.py", "/service.py", "/audit.py", "/schemas/", "/config/profiles/", "/ops/WAIVERS.json", "/ops/OWNERS.json", "/tools/release_gate.py"):
        if path not in co:
