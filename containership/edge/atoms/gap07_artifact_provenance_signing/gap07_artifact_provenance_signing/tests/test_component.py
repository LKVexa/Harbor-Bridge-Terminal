"""Conformance tests for GAP-07 v6.0.0.

Core security tests run without ``pk_core`` in ``test_core.py``. These adapter
checks run when the parent estate's ``pk_core`` package is available.
"""
...
class PackageMetadataTest(unittest.TestCase):
    def test_version_without_pk_core(self):
        self.assertEqual(_load().__version__, "6.0.0")
        self.assertEqual((PKG_DIR / "VERSION").read_text(encoding="utf-8").strip(), "6.0.0")
