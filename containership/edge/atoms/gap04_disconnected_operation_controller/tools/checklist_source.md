...
- [ ] **GAP04-C01-001** — Document the threat model, trust anchors, cryptographic assumptions, protected assets, attacker capabilities, and explicit non-goals.

- [ ] **GAP04-C01-002** — Define a versioned schema/interface with strict parsing, size limits, canonical encoding requirements, and backward/forward compatibility behavior.

- [ ] **GAP04-C01-003** — Add secure defaults and fail-closed behavior for unknown versions, invalid cryptographic material, unavailable trust state, and ambiguous inputs.

- [ ] **GAP04-C01-004** — Instrument stable reason codes, security audit events, health state, and bounded-cardinality metrics for verification failures.

- [ ] **GAP04-C01-005** — Create positive, negative, boundary, corruption, downgrade, replay, and version-skew test vectors and retain them as release evidence.

- [ ] **GAP04-C01-006** — Define a canonical, versioned lease envelope containing issuer identity, subject/site/node binding, lease ID, issuance time, not-before time, expiry time, policy version/hash, capability set, authority generation/epoch, nonce, key ID, signature algorithm, and signature bytes.

- [ ] **GAP04-C01-007** — Define a deterministic canonical serialization for signing and verification; prohibit ambiguous field ordering, duplicate keys, non-canonical numeric encodings, and alternate Unicode representations.

- [ ] **GAP04-C01-008** — Bind every lease to a specific trust domain and deployment scope so a lease issued for one site, tenant, cluster, node class, or environment cannot be replayed in another.

- [ ] **GAP04-C01-009** — Implement signature verification against an explicit trust store; reject unknown issuers, unknown key IDs, revoked keys, unsupported algorithms, malformed signatures, and unverifiable certificate chains.

- [ ] **GAP04-C01-010** — Pin an allow-list of cryptographic algorithms and key sizes; explicitly reject algorithm substitution, `none`, downgrade, legacy, and weak-curve/key configurations.

- [ ] **GAP04-C01-011** — Validate issuance/not-before/expiry ordering, maximum lease lifetime, permitted clock skew, policy digest format, capability syntax, generation monotonicity, and nonce length before accepting a lease.

- [ ] **GAP04-C01-012** — Make lease verification fail closed before any offline action authorization; no local decision may use a lease whose authenticity, freshness, scope, policy binding, or authority epoch is uncertain.

- [ ] **GAP04-C01-013** — Add deterministic negative tests for forged signatures, wrong-site bindings, wrong policy hashes, expired leases, future-issued leases, stale epochs, modified capabilities, truncated payloads, and unknown issuers.

- [ ] **GAP04-C01-014** — Add key-rotation tests covering overlapping signer validity, new-key rollout, old-key revocation, forced rollover, and recovery from stale verifier trust bundles.

- [ ] **GAP04-C01-015** — Record the verified lease fingerprint, signer/key ID, authority generation, policy hash, and verification result in the audit record without logging private key material or sensitive capability payloads.
...
- [ ] **GAP04-C01-017** — Define a signed-envelope schema fixture and compatibility suite for every supported lease-envelope version.
...
- [ ] **GAP04-C01-020** — Production gate: demonstrate that no unsigned, unverifiable, stale, cross-site, downgraded, or policy-mismatched lease can authorize an offline action.
...
- [ ] **GAP04-C01-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C01-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- **Evidence bundle / CI run:** ____________________
- **Waiver(s):** ____________________
- **Status:** Not started / In progress / Verified / Blocked / N/A
- **GO impact:** Blocking / Required evidence / Governance
...
- [ ] **GAP04-C02-001** — Document clock trust assumptions, threat model, supported hardware/virtualization environments, and fail-safe behavior when trust degrades.

- [ ] **GAP04-C02-002** — Define explicit API contracts for trusted monotonic time, wall time, uncertainty/quality, checkpoint persistence, and rollback detection.

- [ ] **GAP04-C02-003** — Ensure all safety timers use one reviewed time abstraction rather than direct calls to wall-clock APIs.
...
- [ ] **GAP04-C02-006** — Define a trusted-time abstraction that separates wall-clock presentation from monotonic safety time used for lease expiry, policy freshness, and timeout decisions.
...
- [ ] **GAP04-C02-008** — Detect wall-clock rollback beyond an explicitly configured tolerance and enter a fail-safe state that forbids extension of any lease or policy freshness window.
...
- [ ] **GAP04-C02-010** — Ensure reboot cannot reset elapsed-disconnect duration, lease age, policy age, revocation age, or reconciliation deadlines.
...
- [ ] **GAP04-C02-015** — Define behavior when trusted time is unavailable: deny new autonomy grants, preserve conservative expiry, surface degraded health, and prevent manual override from silently extending authority.
...
- [ ] **GAP04-C02-017** — Expose monotonic-age and trusted-time-quality indicators through telemetry without leaking security-sensitive platform data.

- [ ] **GAP04-C02-018** — Verify integer widths and overflow behavior for long uptimes, far-future timestamps, epoch conversions, and serialization across supported runtimes.
...
- [ ] **GAP04-C02-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C02-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C03-001** — Document durability guarantees, crash model, filesystem/database assumptions, integrity model, retention, and recovery invariants.

- [ ] **GAP04-C03-002** — Version all on-disk formats and define forward migration, backward-read, and unsupported-version behavior.

- [ ] **GAP04-C03-003** — Make writes bounded, checksummed/integrity-protected as appropriate, and safe under abrupt termination.
...
- [ ] **GAP04-C03-006** — Replace the in-memory journal with a durable write-ahead or append-only log whose commit point is defined in terms of actual persistence semantics.
...
- [ ] **GAP04-C03-010** — Separate `accepted locally`, `persisted`, `applied to supervisor`, `queued for reconciliation`, and `reconciled` states so crash recovery can determine exactly what must be replayed.

- [ ] **GAP04-C03-011** — Guarantee stable ordering using a monotonic per-controller sequence and partition epoch; define behavior for sequence gaps and duplicates.

- [ ] **GAP04-C03-012** — Implement bounded retention with segment rotation, compaction, archival, and watermarks that never delete unreconciled or legally required audit evidence.

- [ ] **GAP04-C03-013** — Reserve emergency journal capacity so storage pressure cannot prevent recording the decision that autonomy is being frozen.

- [ ] **GAP04-C03-014** — Add configurable durability modes only if safety policy permits; the production profile must not acknowledge a decision before required persistence is achieved.

- [ ] **GAP04-C03-015** — Protect journal contents with integrity checks and, where sensitive, encryption; manage keys independently from data files.
...
- [ ] **GAP04-C03-020** — Production gate: demonstrate lossless recovery of every acknowledged offline decision after abrupt power loss with deterministic handling of corruption.
...
- [ ] **GAP04-C03-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C03-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C04-001** — Define audit threat model, evidence retention period, authority/ownership, privacy classification, and verifier trust model.

- [ ] **GAP04-C04-002** — Version the event/export format and provide independent verification tooling.

- [ ] **GAP04-C04-003** — Make audit persistence non-optional for actions that require accountability and fail safely if required audit evidence cannot be recorded.
...
- [ ] **GAP04-C04-005** — Retain verification reports and representative tamper tests in release evidence.
...
- [ ] **GAP04-C04-007** — Choose and document the integrity construction: hash chain, Merkle accumulator, signed checkpoints, or an equivalent design with explicit threat assumptions.
...
- [ ] **GAP04-C04-012** — Define crash-consistent ordering between business decisions and their audit events so an accepted decision cannot exist without durable accountability evidence.

- [ ] **GAP04-C04-013** — Implement verifier tooling that can validate an exported chain offline and produce machine-readable success/failure plus first-corrupt-event information.
...
- [ ] **GAP04-C04-016** — Ensure logs redact secrets and sensitive payloads while retaining enough normalized identifiers to support forensic reconstruction.
...
- [ ] **GAP04-C04-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C04-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C05-001** — Document the threat model, trust anchors, cryptographic assumptions, protected assets, attacker capabilities, and explicit non-goals.

- [ ] **GAP04-C05-002** — Define a versioned schema/interface with strict parsing, size limits, canonical encoding requirements, and backward/forward compatibility behavior.

- [ ] **GAP04-C05-003** — Add secure defaults and fail-closed behavior for unknown versions, invalid cryptographic material, unavailable trust state, and ambiguous inputs.

- [ ] **GAP04-C05-004** — Instrument stable reason codes, security audit events, health state, and bounded-cardinality metrics for verification failures.
...
- [ ] **GAP04-C05-007** — Canonicalize policy bytes before hashing/signing and define exactly which metadata is covered by the signature.
...
- [ ] **GAP04-C05-009** — Pin the lease to the exact policy digest or immutable policy identifier used for every offline decision.

- [ ] **GAP04-C05-010** — Maintain a monotonic policy rollback floor so an attacker cannot reactivate an older but correctly signed vulnerable policy.

- [ ] **GAP04-C05-011** — Persist activation metadata and the last-known-good policy transactionally with controller generation and lease state.
...
- [ ] **GAP04-C05-013** — Fail closed if the active policy bundle becomes unreadable, unverifiable, internally inconsistent, or newer than the local evaluator supports.
...
- [ ] **GAP04-C05-017** — Define secure cache eviction so the last-known-good policy is retained until an explicitly safer fallback is available.
...
- [ ] **GAP04-C05-020** — Production gate: only an authenticated, integrity-verified, non-rolled-back, lease-matched policy artifact may authorize offline behavior.
...
- [ ] **GAP04-C05-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C05-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C06-001** — Document authority topology, trust roots, propagation assumptions, offline risk window, and disaster-recovery constraints.
...
- [ ] **GAP04-C06-006** — Define a monotonically increasing authority epoch or revocation watermark scoped to the trust domain and bound into every lease/cached grant.
...
- [ ] **GAP04-C06-009** — Ensure reconnect processing applies newer revocation state before renewing or issuing replacement offline authority.
...
- [ ] **GAP04-C06-012** — Include revocation epoch in journal and audit records for every decision so later reconciliation can prove which authority state was used.

- [ ] **GAP04-C06-013** — Invalidate cached grants deterministically when their epoch falls below the accepted watermark.
...
- [ ] **GAP04-C06-015** — Define how offline nodes bound risk when revocation information may have advanced while disconnected.
...
- [ ] **GAP04-C06-018** — Document epoch exhaustion/width, serialization, wraparound prohibition, and migration strategy.

- [ ] **GAP04-C06-019** — Define disaster-recovery rules so restoring the authority service cannot move the generation backward.

- [ ] **GAP04-C06-020** — Production gate: a previously revoked or superseded grant must remain invalid after reconnect, restart, failover, and snapshot restoration.
...
- [ ] **GAP04-C06-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C06-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C07-001** — Document durability guarantees, crash model, filesystem/database assumptions, integrity model, retention, and recovery invariants.

- [ ] **GAP04-C07-002** — Version all on-disk formats and define forward migration, backward-read, and unsupported-version behavior.

- [ ] **GAP04-C07-003** — Make writes bounded, checksummed/integrity-protected as appropriate, and safe under abrupt termination.
...
- [ ] **GAP04-C07-006** — Assign every locally accepted decision a globally unique, durable decision ID generated before side effects occur.

- [ ] **GAP04-C07-007** — Assign every reconciliation batch/transaction a stable idempotency key that survives retry, process crash, timeout, and network reconnect.

- [ ] **GAP04-C07-008** — Define identifier entropy, format, namespace, collision assumptions, and whether IDs encode site/controller generation or remain opaque.

- [ ] **GAP04-C07-009** — Persist IDs transactionally with decision payload, journal sequence, effect status, and reconciliation acknowledgement.
...
- [ ] **GAP04-C07-012** — Never generate a new idempotency key merely because a request timed out; retries must reuse the original key.

- [ ] **GAP04-C07-013** — Bind IDs to canonical request hashes so an attacker or bug cannot reuse an old key with different content.

- [ ] **GAP04-C07-014** — Add tests for retry storms, lost acknowledgements, crash after remote commit/before local commit, crash before remote commit, duplicate journal replay, and concurrent retry from duplicate controllers.
...
- [ ] **GAP04-C07-016** — Expose duplicate/replay counters and dedupe-cache pressure metrics.

- [ ] **GAP04-C07-017** — Audit detected replays with source peer, original decision ID, canonical request hash, and disposition.
...
- [ ] **GAP04-C07-019** — Ensure identifiers contain no sensitive information if exposed across trust boundaries.

- [ ] **GAP04-C07-020** — Production gate: repeated delivery of any previously committed reconciliation or supervisor command must not create a second side effect.
...
- [ ] **GAP04-C07-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C07-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C08-001** — Document convergence model, authoritative source, conflict taxonomy, reversible/irreversible operations, and operator escalation rules.

- [ ] **GAP04-C08-002** — Version reconciliation request/result schemas and bind them to stable decision/idempotency identifiers.

- [ ] **GAP04-C08-003** — Persist progress so retries and crashes are safe and deterministic.
...
- [ ] **GAP04-C08-012** — Reconcile authenticated revocation and policy updates before granting any new lease after reconnect.

- [ ] **GAP04-C08-013** — Batch decisions with bounded size, stable ordering, checkpoints, resumable progress, and partial-failure isolation.
...
- [ ] **GAP04-C08-015** — Persist reconciliation progress transactionally so a crash cannot forget a remote commit or double-apply a compensation.
...
- [ ] **GAP04-C08-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C08-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C09-002** — Validate peer identity, trust domain, schema/protocol version, and message scope before consuming data or issuing commands.

- [ ] **GAP04-C09-003** — Bound adapter queues/concurrency and isolate dependency failure with circuit breaking/backpressure.
...
- [ ] **GAP04-C09-007** — Map GAP-04 decision IDs, partition epochs, controller generations, policy hashes, and authority epochs into the replication protocol.
...
- [ ] **GAP04-C09-012** — Ensure replication callbacks cannot mutate GAP-04 state without passing validation, authentication, and transactional persistence boundaries.

- [ ] **GAP04-C09-013** — Implement bounded retry, backoff, circuit breaking, queue limits, and cancellation behavior for the adapter.
...
- [ ] **GAP04-C09-016** — Propagate trace context and stable correlation IDs across the adapter.
...
- [ ] **GAP04-C09-020** — Production gate: GAP-04 must make no claim of authoritative convergence without verified GAP-05 state and explicit compatibility evidence.
...
- [ ] **GAP04-C09-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C09-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C10-002** — Validate peer identity, trust domain, schema/protocol version, and message scope before consuming data or issuing commands.

- [ ] **GAP04-C10-003** — Bound adapter queues/concurrency and isolate dependency failure with circuit breaking/backpressure.
...
- [ ] **GAP04-C10-008** — Require authenticated challenge/response or equivalent for authority/service reachability so spoofed packets cannot falsely exit partition mode.

- [ ] **GAP04-C10-009** — Implement hysteresis, debounce, minimum dwell times, and transition thresholds to prevent rapid mode oscillation under link flapping.

- [ ] **GAP04-C10-010** — Distinguish network reachability from successful authority synchronization; connected mode is not safe until required revocation/policy/reconciliation steps complete.
...
- [ ] **GAP04-C10-018** — Define adapter version negotiation and explicit unsupported-state behavior.

- [ ] **GAP04-C10-019** — Document platform-specific network event sources and privilege requirements.

- [ ] **GAP04-C10-020** — Production gate: an attacker or flaky network must not be able to cause premature reconnect/renewal or conceal a true partition.
...
- [ ] **GAP04-C10-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C10-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C11-002** — Validate peer identity, trust domain, schema/protocol version, and message scope before consuming data or issuing commands.

- [ ] **GAP04-C11-003** — Bound adapter queues/concurrency and isolate dependency failure with circuit breaking/backpressure.
...
- [ ] **GAP04-C11-008** — Ensure policy evaluation is deterministic for the same canonical input and policy version where the policy language promises determinism.
...
- [ ] **GAP04-C11-012** — Define policy staleness bounds and prevent stale policy from being refreshed merely by process restart.
...
- [ ] **GAP04-C11-016** — Record policy decision reason codes and matched rule identifiers in audit records without leaking policy secrets.
...
- [ ] **GAP04-C11-020** — Production gate: every offline authorization must be reproducibly attributable to a verified policy version and validated decision context.
...
- [ ] **GAP04-C11-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C11-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C12-002** — Validate peer identity, trust domain, schema/protocol version, and message scope before consuming data or issuing commands.

- [ ] **GAP04-C12-003** — Bound adapter queues/concurrency and isolate dependency failure with circuit breaking/backpressure.
...
- [ ] **GAP04-C12-008** — Enforce least privilege and deny unknown/ambiguous capabilities; wildcard privileges must be explicitly bounded and reviewed.

- [ ] **GAP04-C12-009** — Verify issuer trust and revocation before accepting or caching a capability grant.

- [ ] **GAP04-C12-010** — Bind cached capability state to device/site identity, policy digest, and authority generation.

- [ ] **GAP04-C12-011** — Define behavior when security-plane state is unavailable, stale, partially synchronized, or internally inconsistent.
...
- [ ] **GAP04-C12-015** — Propagate stable security decision reason codes into GAP-04 machine-readable errors and audit events.
...
- [ ] **GAP04-C12-018** — Define secure cache persistence and rollback protection if capability state survives restart.

- [ ] **GAP04-C12-019** — Document emergency revocation semantics and expected propagation SLO.

- [ ] **GAP04-C12-020** — Production gate: no local action can exceed the authenticated capability envelope in force for that subject at the accepted authority epoch.
...
- [ ] **GAP04-C12-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C12-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C13-002** — Validate peer identity, trust domain, schema/protocol version, and message scope before consuming data or issuing commands.

- [ ] **GAP04-C13-003** — Bound adapter queues/concurrency and isolate dependency failure with circuit breaking/backpressure.
...
- [ ] **GAP04-C13-007** — Map every GAP-04 policy action to an explicit supervisor operation or explicit unsupported result; no implicit fall-through.
...
- [ ] **GAP04-C13-009** — Persist the intended supervisor side effect before issuing it and persist the returned result/operation handle after completion.
...
- [ ] **GAP04-C13-014** — Map supervisor failures to machine-readable GAP-04 error codes and reconciliation states.

- [ ] **GAP04-C13-015** — Propagate trace context and audit correlation IDs across the boundary.
...
- [ ] **GAP04-C13-018** — Document exactly which local actions are reversible and define compensation for each reversible operation.

- [ ] **GAP04-C13-019** — Verify supervisor responses are integrity-protected and belong to the expected request/operation ID.

- [ ] **GAP04-C13-020** — Production gate: each accepted GAP-04 decision must have a durable, idempotent, attributable supervisor effect or a durable, explicit failure state.
...
- [ ] **GAP04-C13-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C13-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C14-001** — Maintain a boundary/identity/data-flow threat model and explicit trust-domain map.

- [ ] **GAP04-C14-002** — Use deny-by-default authorization and least-privilege service/operator identities.

- [ ] **GAP04-C14-003** — Version security-sensitive protocols and reject downgrade/unknown modes.
...
- [ ] **GAP04-C14-006** — Inventory every inbound/outbound boundary: policy engine, security plane, supervisor, replication, WAN probes, reconciliation service, telemetry sink, operator API, storage/key services, and local IPC.

- [ ] **GAP04-C14-007** — Assign an identity mechanism to each boundary (mTLS SPIFFE/SVID, device certificate, workload token, OS peer credentials, TPM attestation, or documented equivalent).
...
- [ ] **GAP04-C14-009** — Authorize every operation using explicit least-privilege roles/capabilities; authenticated identity alone must never imply permission.
...
- [ ] **GAP04-C14-013** — Pin protocol versions/cipher suites and disable insecure renegotiation/downgrade paths.

- [ ] **GAP04-C14-014** — Define credential bootstrap/rotation with no long-lived secret embedded in source, image, configuration, or logs.
...
- [ ] **GAP04-C14-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C14-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C15-001** — Document protected data classes, key hierarchy, hardware trust assumptions, and recovery/compromise model.

- [ ] **GAP04-C15-002** — Version encrypted record formats including algorithm and key identifiers without permitting unsafe algorithm agility.

- [ ] **GAP04-C15-003** — Fail closed on integrity/authentication failure and avoid automatic destructive recovery.
...
- [ ] **GAP04-C15-006** — Inventory all persistent sensitive data: leases, policies, journal, audit chain, identity material, cached grants, configuration, reconciliation state, backups, and temporary files.

- [ ] **GAP04-C15-007** — Classify each data set and define encryption-at-rest requirements, key scope, retention, and permitted plaintext exposure.

- [ ] **GAP04-C15-008** — Use authenticated encryption with approved algorithms/modes; reject unauthenticated encryption and home-grown cryptography.
...
- [ ] **GAP04-C15-016** — Define key-version metadata and migration so older encrypted records remain decryptable only for the approved retention window.
...
- [ ] **GAP04-C15-019** — Verify backup/restore tooling preserves encryption guarantees and does not export plaintext by default.
...
- [ ] **GAP04-C15-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C15-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C16-001** — Define distributed-system safety invariants, authority source, network partition assumptions, and recovery semantics.

- [ ] **GAP04-C16-002** — Version fencing/ownership messages and bind them to authenticated identity and persistent generations.

- [ ] **GAP04-C16-003** — Make stale-owner rejection enforceable at every side-effect boundary that can honor fencing.
...
- [ ] **GAP04-C16-009** — Reject commands and writes from a controller generation older than the highest accepted generation.
...
- [ ] **GAP04-C16-013** — Persist ownership/generation state crash-consistently and bind it to site/node identity.
...
- [ ] **GAP04-C16-019** — Ensure operator override cannot bypass fencing silently.
...
- [ ] **GAP04-C16-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C16-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C17-001** — Document durability guarantees, crash model, filesystem/database assumptions, integrity model, retention, and recovery invariants.

- [ ] **GAP04-C17-002** — Version all on-disk formats and define forward migration, backward-read, and unsupported-version behavior.

- [ ] **GAP04-C17-003** — Make writes bounded, checksummed/integrity-protected as appropriate, and safe under abrupt termination.
...
- [ ] **GAP04-C17-006** — Persist partition epoch and controller generation in durable integrity-protected state rather than initializing them solely in memory.

- [ ] **GAP04-C17-007** — Increment partition epoch atomically at the defined transition into a new disconnected episode and never reuse an earlier epoch for the same controller identity.

- [ ] **GAP04-C17-008** — Bind every offline decision, journal segment, audit event, reconciliation record, and lease evaluation to partition epoch and controller generation.

- [ ] **GAP04-C17-009** — Persist generation changes before enabling any side effects under the new controller instance.
...
- [ ] **GAP04-C17-011** — Define integer width, monotonicity, wraparound prohibition, serialization, and migration semantics.

- [ ] **GAP04-C17-012** — Detect missing/corrupt epoch state at startup and enter recovery/quarantine rather than silently resetting to zero.

- [ ] **GAP04-C17-013** — Define behavior when durable state is restored from backup older than authority service state.

- [ ] **GAP04-C17-014** — Add tests for crash between epoch increment and journal append, reboot during partition, snapshot rollback, duplicated storage, generation change, and migration from pre-persistence releases.
...
- [ ] **GAP04-C17-017** — Audit every increment and generation change with cause.
...
- [ ] **GAP04-C17-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C17-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C18-001** — Document the atomicity/isolation/durability invariants and the exact external side effects covered by each transaction.

- [ ] **GAP04-C18-002** — Version durable records and ensure recovery code is compatible with all supported formats.

- [ ] **GAP04-C18-003** — Instrument commit/recovery failures with stable error codes and safety readiness impact.

- [ ] **GAP04-C18-004** — Prohibit best-effort continuation after an uncertain commit for safety-critical state.

- [ ] **GAP04-C18-005** — Qualify using exhaustive crash-point and I/O-fault injection around transaction boundaries.

- [ ] **GAP04-C18-006** — Define the exact safety transaction boundary spanning lease state, active policy digest/version, authority epoch, partition epoch, controller generation, journal append, and side-effect intent.

- [ ] **GAP04-C18-007** — Choose a transaction mechanism appropriate to the persistence substrate: embedded transactional database, WAL with commit record, copy-on-write manifest, or equivalent.

- [ ] **GAP04-C18-008** — Ensure no externally visible side effect occurs from a state combination that was never durably committed.

- [ ] **GAP04-C18-009** — Use explicit transaction IDs and commit sequence numbers to correlate journal/audit/reconciliation state.

- [ ] **GAP04-C18-010** — On startup, recover to the last complete committed transaction and deterministically roll forward or roll back incomplete work.

- [ ] **GAP04-C18-011** — Prevent torn activation where a new policy is marked active but the corresponding lease/policy binding or rollback floor is not persisted.

- [ ] **GAP04-C18-012** — Prevent decision acknowledgement before both authorization context and decision intent meet required durability.

- [ ] **GAP04-C18-013** — Ensure compaction/checkpoint operations are themselves atomic and recoverable.

- [ ] **GAP04-C18-014** — Add injected-crash tests before/after every persistence write, fsync, rename, commit record, and side-effect dispatch.

- [ ] **GAP04-C18-015** — Add ENOSPC, permission-loss, filesystem-read-only, corruption, I/O timeout, and key-unavailable tests.

- [ ] **GAP04-C18-016** — Define transaction isolation/concurrency semantics for simultaneous renewal, policy refresh, reconciliation, and operator override.

- [ ] **GAP04-C18-017** — Expose current durable transaction sequence, recovery-required flag, and last commit failure.

- [ ] **GAP04-C18-018** — Audit recovery decisions without double-recording business actions.
...
- [ ] **GAP04-C18-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C18-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C19-001** — Publish the contract as a versioned schema/specification and treat it as a compatibility surface.

- [ ] **GAP04-C19-002** — Validate every boundary input and bound message size, nesting, string length, and collection count.

- [ ] **GAP04-C19-003** — Use stable identifiers/correlation IDs and deterministic retry/idempotency semantics.

- [ ] **GAP04-C19-004** — Instrument error frequency/latency without leaking sensitive details.

- [ ] **GAP04-C19-005** — Maintain golden contract fixtures and client/server compatibility tests.
...
- [ ] **GAP04-C19-007** — Allocate documented error namespaces for authorization, lease, policy, time, connectivity, storage, reconciliation, dependency, configuration, schema, overload, and internal faults.

- [ ] **GAP04-C19-008** — Keep error codes stable across patch releases; changes to semantic meaning require versioning.

- [ ] **GAP04-C19-009** — Separate safe machine-readable details from operator diagnostics so secrets, keys, tokens, policy contents, and tenant data do not leak.
...
- [ ] **GAP04-C19-011** — Define HTTP/gRPC/IPC status mappings if those transports are used, including idempotent retry guidance.
...
- [ ] **GAP04-C19-014** — Add schema tests for every error variant and golden fixtures across supported versions.

- [ ] **GAP04-C19-015** — Add negative tests for malformed requests, unsupported versions, dependency timeouts, auth failures, ENOSPC, stale epochs, and reconciliation conflicts.

- [ ] **GAP04-C19-016** — Document operator-facing remediation for each production error code.

- [ ] **GAP04-C19-017** — Emit metrics by stable error code, not raw exception text.

- [ ] **GAP04-C19-018** — Ensure logs preserve internal diagnostic detail behind access controls while external responses remain sanitized.

- [ ] **GAP04-C19-019** — Define localization policy for human messages without changing machine codes.
...
- [ ] **GAP04-C19-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C19-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C20-001** — Define safety state transitions, resource budgets, overload/failure thresholds, and recovery prerequisites.

- [ ] **GAP04-C20-002** — Make overload and resource-failure handling deterministic and bounded.

- [ ] **GAP04-C20-003** — Prioritize safety-critical state/audit work over optional work.

- [ ] **GAP04-C20-004** — Expose saturation/freeze state through health, metrics, alerts, and machine-readable errors.
...
- [ ] **GAP04-C20-007** — Reserve protected emergency capacity that ordinary logs/cache/compaction cannot consume.
...
- [ ] **GAP04-C20-009** — Before accepting any offline action, verify that required durable journal and audit writes can complete; fail closed if safety evidence cannot be persisted.

- [ ] **GAP04-C20-010** — Define eviction priority: discard reconstructible caches/logs before authoritative journal/audit records; never evict unreconciled decisions silently.

- [ ] **GAP04-C20-011** — Implement bounded segment rotation and backpressure so producers cannot grow storage without limit.

- [ ] **GAP04-C20-012** — On critical exhaustion, freeze unsafe mutations, emit an auditable state transition, preserve enough capacity for recovery evidence, and surface operator alerts.
...
- [ ] **GAP04-C20-015** — Verify recovery after space is freed: integrity scan, compaction/checkpoint, reconcile backlog, then explicitly exit freeze only when health criteria pass.
...
- [ ] **GAP04-C20-018** — Document capacity planning formula based on maximum decision rate and maximum disconnected duration.

- [ ] **GAP04-C20-019** — Define operator tooling for safe export/prune of eligible data with authentication and audit.

- [ ] **GAP04-C20-020** — Production gate: complete storage exhaustion must degrade into a deterministic safe freeze rather than silent data loss, corrupt state, or unaudited action.
...
- [ ] **GAP04-C20-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C20-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C21-001** — Version the configuration schema and track provenance/digest for the effective configuration.

- [ ] **GAP04-C21-002** — Validate types, ranges, units, cross-field constraints, and security-sensitive combinations before activation.

- [ ] **GAP04-C21-003** — Separate secrets from non-secret configuration and apply least-privilege access to config stores.

- [ ] **GAP04-C21-004** — Expose active version/digest and validation/activation state in health/audit records.

- [ ] **GAP04-C21-005** — Maintain golden valid/invalid fixtures and migration tests.

- [ ] **GAP04-C21-006** — Define a versioned schema for site/environment configuration covering lease duration, tier thresholds, allowed actions, policy staleness, quotas, storage limits, retry budgets, timeouts, endpoints, and feature gates.

- [ ] **GAP04-C21-007** — Separate immutable build-time defaults from environment/site overrides and operator emergency overrides.

- [ ] **GAP04-C21-008** — Define type, range, enum, unit, cross-field, and semantic validation for every configuration value.

- [ ] **GAP04-C21-009** — Prohibit unknown keys in production unless an explicit forward-compatibility extension mechanism is defined.

- [ ] **GAP04-C21-010** — Support deterministic layered precedence and expose the fully resolved effective configuration.

- [ ] **GAP04-C21-011** — Bind configuration to site/tenant/trust-domain identity to prevent accidental cross-environment reuse.

- [ ] **GAP04-C21-012** — Support secrets by reference to secret stores rather than inline plaintext values.

- [ ] **GAP04-C21-013** — Attach config ID/version/digest, author/source, creation time, and provenance to every effective configuration.

- [ ] **GAP04-C21-014** — Add configuration schema migration and compatibility tests across supported releases.

- [ ] **GAP04-C21-015** — Add negative tests for missing required values, out-of-range thresholds, invalid units, contradictory settings, and unsafe combinations.

- [ ] **GAP04-C21-016** — Expose effective config digest/version in health and audit records without leaking secrets.

- [ ] **GAP04-C21-017** — Provide a config linter/dry-run command usable in CI and before activation.

- [ ] **GAP04-C21-018** — Document defaults, rationale, safe ranges, and whether each setting is dynamic or restart-required.

- [ ] **GAP04-C21-019** — Pin production configuration examples as test fixtures.

- [ ] **GAP04-C21-020** — Production gate: no unvalidated or ambiguous configuration may influence autonomy decisions.
...
- [ ] **GAP04-C21-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C21-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C22-001** — Version the configuration schema and track provenance/digest for the effective configuration.

- [ ] **GAP04-C22-002** — Validate types, ranges, units, cross-field constraints, and security-sensitive combinations before activation.

- [ ] **GAP04-C22-003** — Separate secrets from non-secret configuration and apply least-privilege access to config stores.

- [ ] **GAP04-C22-004** — Expose active version/digest and validation/activation state in health/audit records.
...
- [ ] **GAP04-C22-007** — Use an atomic pointer/transaction to switch from old to new configuration; never partially apply fields.

- [ ] **GAP04-C22-008** — Persist previous known-good configuration and its digest for deterministic rollback.
...
- [ ] **GAP04-C22-011** — Require two-person approval for high-risk autonomy/security/storage thresholds where governance requires it.
...
- [ ] **GAP04-C22-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C22-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C23-001** — Version the configuration schema and track provenance/digest for the effective configuration.

- [ ] **GAP04-C23-002** — Validate types, ranges, units, cross-field constraints, and security-sensitive combinations before activation.

- [ ] **GAP04-C23-003** — Separate secrets from non-secret configuration and apply least-privilege access to config stores.

- [ ] **GAP04-C23-004** — Expose active version/digest and validation/activation state in health/audit records.

- [ ] **GAP04-C23-005** — Maintain golden valid/invalid fixtures and migration tests.

- [ ] **GAP04-C23-006** — Replace static `0/30/120` thresholds with a validated versioned tier policy.

- [ ] **GAP04-C23-007** — Define units explicitly and reject implicit seconds/minutes conversions.

- [ ] **GAP04-C23-008** — Support per-site/per-workload-class overrides only within centrally approved safety bounds.

- [ ] **GAP04-C23-009** — Require thresholds to be strictly ordered and non-negative with an explicit maximum disconnected horizon.

- [ ] **GAP04-C23-010** — Define transition hysteresis and whether tier changes are based on trusted elapsed disconnect time, lease remaining, policy age, or a composed rule.

- [ ] **GAP04-C23-011** — Bind the tier schedule digest/version to decisions and audit records.

- [ ] **GAP04-C23-012** — Define action allow/deny matrix for every tier and verify no undefined action inherits permissive behavior.

- [ ] **GAP04-C23-013** — Add boundary tests at threshold−ε, threshold, threshold+ε, clock jump, reboot, and policy refresh.

- [ ] **GAP04-C23-014** — Add configuration tests for equal/reversed thresholds, extreme values, integer overflow, and unsupported tier names.
...
- [ ] **GAP04-C23-019** — Ensure tier policy changes cannot extend an already-expired authority period.
...
- [ ] **GAP04-C23-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C23-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C24-002** — Make operator actions durable, time-bounded where appropriate, and fully auditable.

- [ ] **GAP04-C24-003** — Protect management interfaces against replay, stale commands, cross-site scope errors, and unauthorized local access.

- [ ] **GAP04-C24-004** — Expose active emergency/override state prominently in health and dashboards.

- [ ] **GAP04-C24-005** — Exercise operator workflows through tabletop/game-day and negative authorization testing.

- [ ] **GAP04-C24-006** — Define explicit override types: freeze, disable, tier restriction, emergency deny, limited temporary allow, and diagnostic mode; default to deny-only overrides where possible.

- [ ] **GAP04-C24-007** — Require strong operator authentication, authorization, and scoped roles separate from ordinary application identities.
...
- [ ] **GAP04-C24-009** — Use two-person approval for overrides that expand authority or bypass a normal safety control where policy requires it.
...
- [ ] **GAP04-C24-011** — Apply overrides through the same atomic durable state machinery as policy/config transitions.

- [ ] **GAP04-C24-012** — Ensure override expiry is evaluated using trusted time and cannot be extended by reboot or clock rollback.

- [ ] **GAP04-C24-013** — Make conflicting overrides deterministic with documented precedence; fail closed on ambiguity.

- [ ] **GAP04-C24-014** — Provide explicit revoke/cancel with confirmation of resulting effective policy.
...
- [ ] **GAP04-C24-017** — Expose active override state prominently in health and dashboards.

- [ ] **GAP04-C24-018** — Prevent override secrets/tokens from appearing in logs or command history.

- [ ] **GAP04-C24-019** — Require post-incident review and evidence retention for production overrides.

- [ ] **GAP04-C24-020** — Production gate: no override can silently persist, exceed its scope/time, or erase the evidence that it existed.
...
- [ ] **GAP04-C24-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C24-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C25-001** — Define a versioned telemetry schema/taxonomy and map signals to SLOs and failure modes.

- [ ] **GAP04-C25-002** — Control cardinality, privacy, redaction, retention, and access.

- [ ] **GAP04-C25-003** — Ensure telemetry pipelines are bounded and cannot block safety-critical control flow.

- [ ] **GAP04-C25-004** — Correlate metrics/logs/traces/audit using stable identifiers without conflating diagnostic telemetry with authoritative audit.

- [ ] **GAP04-C25-005** — Test telemetry correctness, missing-data behavior, and alertability under fault injection.

- [ ] **GAP04-C25-006** — Define separate liveness, readiness, and safety-readiness semantics; a process can be alive while not safe to authorize offline actions.
...
- [ ] **GAP04-C25-008** — Report lease validity/remaining time, policy freshness, authority epoch, partition epoch, controller generation, current tier, journal pressure, reconciliation backlog, and active override/freeze state.
...
- [ ] **GAP04-C25-010** — Return stable machine-readable health codes and component states rather than free-form text only.

- [ ] **GAP04-C25-011** — Protect detailed health endpoints with authentication/authorization if they reveal topology or security posture.

- [ ] **GAP04-C25-012** — Provide a minimal local liveness endpoint that does not depend on external services to avoid restart loops.

- [ ] **GAP04-C25-013** — Define readiness thresholds and reason precedence deterministically.
...
- [ ] **GAP04-C25-016** — Expose age of health data and avoid reporting stale cached success as current.

- [ ] **GAP04-C25-017** — Integrate health state with orchestration without causing destructive restart loops during expected partitions.

- [ ] **GAP04-C25-018** — Audit transitions into/out of safety-not-ready states where operationally significant.

- [ ] **GAP04-C25-019** — Document which health states should page, freeze autonomy, or merely warn.

- [ ] **GAP04-C25-020** — Production gate: an external operator/orchestrator can determine whether GAP-04 is safe to serve, not merely whether its process is running.
...
- [ ] **GAP04-C25-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C25-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C26-001** — Define a versioned telemetry schema/taxonomy and map signals to SLOs and failure modes.

- [ ] **GAP04-C26-002** — Control cardinality, privacy, redaction, retention, and access.

- [ ] **GAP04-C26-003** — Ensure telemetry pipelines are bounded and cannot block safety-critical control flow.

- [ ] **GAP04-C26-004** — Correlate metrics/logs/traces/audit using stable identifiers without conflating diagnostic telemetry with authoritative audit.
...
- [ ] **GAP04-C26-008** — Define gauges for lease remaining, policy age, journal bytes/utilization, reconciliation backlog, current tier, authority epoch, active partition duration, queue depth, and storage free space.

- [ ] **GAP04-C26-009** — Use bounded label cardinality; never label metrics with raw decision IDs, workload IDs at fleet scale, user input, secrets, or unbounded error strings.

- [ ] **GAP04-C26-010** — Standardize units and histogram buckets based on measured SLOs rather than arbitrary defaults.

- [ ] **GAP04-C26-011** — Expose component/version/build/config metadata via dedicated info metrics.

- [ ] **GAP04-C26-012** — Protect metrics endpoints or transport according to deployment sensitivity.

- [ ] **GAP04-C26-013** — Implement scrape/export failure isolation so telemetry outage cannot block control decisions.
...
- [ ] **GAP04-C26-016** — Instrument rejected/failed operations with stable reason-code labels.
...
- [ ] **GAP04-C26-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C26-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C27-001** — Define a versioned telemetry schema/taxonomy and map signals to SLOs and failure modes.

- [ ] **GAP04-C27-002** — Control cardinality, privacy, redaction, retention, and access.

- [ ] **GAP04-C27-003** — Ensure telemetry pipelines are bounded and cannot block safety-critical control flow.

- [ ] **GAP04-C27-004** — Correlate metrics/logs/traces/audit using stable identifiers without conflating diagnostic telemetry with authoritative audit.

- [ ] **GAP04-C27-005** — Test telemetry correctness, missing-data behavior, and alertability under fault injection.

- [ ] **GAP04-C27-006** — Adopt a versioned structured log schema with timestamp, severity, event name, component, site/node/workload scope, decision ID, correlation/trace IDs, partition epoch, controller generation, and reason code.

- [ ] **GAP04-C27-007** — Normalize identifiers and field names across GAP-04 and adjacent adapters.

- [ ] **GAP04-C27-008** — Define data classification and field-level redaction; prohibit credentials, private keys, tokens, raw secrets, and unnecessary tenant payloads.

- [ ] **GAP04-C27-009** — Separate audit records from diagnostic logs; diagnostic retention/rotation must not weaken audit guarantees.
...
- [ ] **GAP04-C27-012** — Support sampling for high-volume debug/info events while never sampling required security/audit events.

- [ ] **GAP04-C27-013** — Define retention, rotation, compression, export, and deletion policies.

- [ ] **GAP04-C27-014** — Include build/config/policy version at process start and relevant transition events.

- [ ] **GAP04-C27-015** — Add tests scanning representative logs for secret patterns and forbidden fields.

- [ ] **GAP04-C27-016** — Add schema/golden tests for critical lifecycle/security events.

- [ ] **GAP04-C27-017** — Ensure timestamps include trusted-time quality or clearly distinguish untrusted wall-clock values.

- [ ] **GAP04-C27-018** — Support correlation across decision, supervisor, replication, and reconciliation flows.
...
- [ ] **GAP04-C27-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C27-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C28-001** — Define a versioned telemetry schema/taxonomy and map signals to SLOs and failure modes.

- [ ] **GAP04-C28-002** — Control cardinality, privacy, redaction, retention, and access.

- [ ] **GAP04-C28-003** — Ensure telemetry pipelines are bounded and cannot block safety-critical control flow.

- [ ] **GAP04-C28-004** — Correlate metrics/logs/traces/audit using stable identifiers without conflating diagnostic telemetry with authoritative audit.

- [ ] **GAP04-C28-005** — Test telemetry correctness, missing-data behavior, and alertability under fault injection.

- [ ] **GAP04-C28-006** — Adopt a standard trace context format (for example W3C Trace Context/OpenTelemetry) across supported transports.
...
- [ ] **GAP04-C28-009** — Do not encode secrets, raw policy data, or high-cardinality sensitive attributes in spans.

- [ ] **GAP04-C28-010** — Define sampling policy that preserves error/security traces while controlling fleet cost.

- [ ] **GAP04-C28-011** — Correlate trace/span IDs with structured logs and audit references without making traces authoritative audit evidence.
...
- [ ] **GAP04-C28-014** — Bound span creation and exporter queues so telemetry backpressure cannot affect control-plane safety.

- [ ] **GAP04-C28-015** — Add tests for propagation through each adapter and for missing/malformed/untrusted trace headers.
...
- [ ] **GAP04-C28-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C28-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C29-001** — Define a versioned telemetry schema/taxonomy and map signals to SLOs and failure modes.

- [ ] **GAP04-C29-002** — Control cardinality, privacy, redaction, retention, and access.

- [ ] **GAP04-C29-003** — Ensure telemetry pipelines are bounded and cannot block safety-critical control flow.

- [ ] **GAP04-C29-004** — Correlate metrics/logs/traces/audit using stable identifiers without conflating diagnostic telemetry with authoritative audit.

- [ ] **GAP04-C29-005** — Test telemetry correctness, missing-data behavior, and alertability under fault injection.

- [ ] **GAP04-C29-006** — Define separate alert conditions for ordinary short partition, prolonged partition, stale policy, lease nearing expiry, lease expired, journal pressure, audit failure, storage critical, reconciliation conflict, duplicate controller, key failure, and software fault.

- [ ] **GAP04-C29-007** — Base alerts on durable service symptoms/SLOs rather than single noisy samples where possible.
...
- [ ] **GAP04-C29-010** — Create dashboards for connectivity/tier state, lease/policy freshness, decision outcomes, persistence health, reconciliation, dependencies, capacity, and security events.
...
- [ ] **GAP04-C29-013** — Distinguish security indicators from reliability failures and route to appropriate responders.
...
- [ ] **GAP04-C29-016** — Detect missing telemetry itself so silent exporter failure is visible.
...
- [ ] **GAP04-C29-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C29-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C30-001** — Define safety state transitions, resource budgets, overload/failure thresholds, and recovery prerequisites.

- [ ] **GAP04-C30-002** — Make overload and resource-failure handling deterministic and bounded.

- [ ] **GAP04-C30-003** — Prioritize safety-critical state/audit work over optional work.

- [ ] **GAP04-C30-004** — Expose saturation/freeze state through health, metrics, alerts, and machine-readable errors.
...
- [ ] **GAP04-C30-009** — Use explicit overload errors with retryability and backoff hints instead of unbounded latency.

- [ ] **GAP04-C30-010** — Implement circuit breakers for failing adjacent services with half-open recovery and bounded probe rate.

- [ ] **GAP04-C30-011** — Define retry budgets and exponential backoff with jitter; prohibit infinite tight retry loops.

- [ ] **GAP04-C30-012** — Drop or coalesce only operations proven safe to discard/merge; never silently drop accepted decisions, revocations, or required audit work.
...
- [ ] **GAP04-C30-019** — Verify backpressure behavior during storage-pressure freeze.
...
- [ ] **GAP04-C30-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C30-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C31-001** — Link every scenario to explicit requirements/invariants and stable test IDs.

- [ ] **GAP04-C31-002** — Use deterministic seeds/fixtures and archive enough evidence to reproduce failures.

- [ ] **GAP04-C31-003** — Run tests under production-equivalent optimization, crypto, persistence, and concurrency settings.

- [ ] **GAP04-C31-004** — Include negative/fail-closed assertions, not only happy-path behavior.

- [ ] **GAP04-C31-005** — Integrate the suite into CI/release qualification with clearly defined blocking criteria.

- [ ] **GAP04-C31-006** — Build an automated harness that can terminate the controller at deterministic persistence/lifecycle fault points and restart it repeatedly.
...
- [ ] **GAP04-C31-008** — Inject crashes before/after lease commit, policy activation, epoch increment, journal append, audit append, supervisor dispatch, reconciliation remote commit, and acknowledgement persistence.

- [ ] **GAP04-C31-009** — Verify startup integrity scans and recovery do not silently discard or duplicate durable decisions.

- [ ] **GAP04-C31-010** — Verify pending side effects are queried/replayed idempotently after restart.

- [ ] **GAP04-C31-011** — Verify lease age, policy age, partition duration, revocation epoch, and controller generation remain safe across restart.

- [ ] **GAP04-C31-012** — Verify corrupt/torn tail recovery and quarantine of unrecoverable mid-log corruption.
...
- [ ] **GAP04-C31-016** — Record durable-state hashes/sequence numbers before crash and after recovery for deterministic assertions.

- [ ] **GAP04-C31-017** — Fail the suite on unreconciled orphan state, duplicate effect, lost acknowledged action, or rollback of authority generation.

- [ ] **GAP04-C31-018** — Integrate the fault harness into release CI rather than keeping it manual.

- [ ] **GAP04-C31-019** — Archive representative recovery evidence with the release.
...
- [ ] **GAP04-C31-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C31-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C32-001** — Link every scenario to explicit requirements/invariants and stable test IDs.

- [ ] **GAP04-C32-002** — Use deterministic seeds/fixtures and archive enough evidence to reproduce failures.

- [ ] **GAP04-C32-003** — Run tests under production-equivalent optimization, crypto, persistence, and concurrency settings.

- [ ] **GAP04-C32-004** — Include negative/fail-closed assertions, not only happy-path behavior.
...
- [ ] **GAP04-C32-007** — Exercise transitions at every tier threshold and immediately before/after lease expiry.

- [ ] **GAP04-C32-008** — Simulate authority/policy/revocation changes while the node is disconnected.

- [ ] **GAP04-C32-009** — Generate conflicting remote state during partition and verify reconciliation classification on reconnect.

- [ ] **GAP04-C32-010** — Simulate rapid flap patterns and verify hysteresis prevents unsafe oscillation.

- [ ] **GAP04-C32-011** — Simulate reconnect with degraded bandwidth and partial service reachability.

- [ ] **GAP04-C32-012** — Verify revocation/policy synchronization and reconciliation ordering precede renewal when required.

- [ ] **GAP04-C32-013** — Test stale DNS and endpoint migration during long partitions.

- [ ] **GAP04-C32-014** — Test simultaneous reconnect of many nodes/sites to expose thundering-herd behavior.

- [ ] **GAP04-C32-015** — Verify authenticated reachability cannot be spoofed by injected unauthenticated packets.

- [ ] **GAP04-C32-016** — Measure time to safe connected state, backlog drain, and error rate.

- [ ] **GAP04-C32-017** — Capture deterministic seeds/scenarios so regressions are reproducible.

- [ ] **GAP04-C32-018** — Run representative long-duration partitions measured in hours/days in scheduled qualification.

- [ ] **GAP04-C32-019** — Assert audit/journal continuity across every scenario.
...
- [ ] **GAP04-C32-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C32-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C33-001** — Link every scenario to explicit requirements/invariants and stable test IDs.

- [ ] **GAP04-C33-002** — Use deterministic seeds/fixtures and archive enough evidence to reproduce failures.

- [ ] **GAP04-C33-003** — Run tests under production-equivalent optimization, crypto, persistence, and concurrency settings.

- [ ] **GAP04-C33-004** — Include negative/fail-closed assertions, not only happy-path behavior.

- [ ] **GAP04-C33-005** — Integrate the suite into CI/release qualification with clearly defined blocking criteria.

- [ ] **GAP04-C33-006** — Identify shared mutable state and publish an explicit locking/transaction ownership model.
...
- [ ] **GAP04-C33-008** — Race multiple callers submitting duplicate and distinct decisions for the same resource.

- [ ] **GAP04-C33-009** — Race controller ownership takeover against stale in-flight requests from the prior generation.
...
- [ ] **GAP04-C33-018** — Run concurrency tests under optimized/runtime production settings.
...
- [ ] **GAP04-C33-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C33-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C34-001** — Link every scenario to explicit requirements/invariants and stable test IDs.

- [ ] **GAP04-C34-002** — Use deterministic seeds/fixtures and archive enough evidence to reproduce failures.

- [ ] **GAP04-C34-003** — Run tests under production-equivalent optimization, crypto, persistence, and concurrency settings.

- [ ] **GAP04-C34-004** — Include negative/fail-closed assertions, not only happy-path behavior.

- [ ] **GAP04-C34-005** — Integrate the suite into CI/release qualification with clearly defined blocking criteria.

- [ ] **GAP04-C34-006** — Define state-machine properties for lease lifecycle, partition transitions, tier monotonicity, policy freshness, epoch monotonicity, and reconciliation idempotency.

- [ ] **GAP04-C34-007** — Generate randomized valid/invalid event sequences including reboot, disconnect, reconnect, renewal, revocation, policy update, override, and storage fault.

- [ ] **GAP04-C34-008** — Fuzz all accepted schema payloads with malformed types, duplicate fields, oversized values, invalid Unicode, numeric extremes, and nested-depth abuse.
...
- [ ] **GAP04-C34-010** — Fuzz signed envelope parsing before signature verification to detect parser inconsistencies and resource exhaustion.

- [ ] **GAP04-C34-011** — Property-test canonical serialization round trips and signature/hash stability.
...
- [ ] **GAP04-C34-014** — Assert fail-closed behavior for malformed security-critical inputs.
...
- [ ] **GAP04-C34-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C34-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C35-001** — Link every scenario to explicit requirements/invariants and stable test IDs.

- [ ] **GAP04-C35-002** — Use deterministic seeds/fixtures and archive enough evidence to reproduce failures.

- [ ] **GAP04-C35-003** — Run tests under production-equivalent optimization, crypto, persistence, and concurrency settings.

- [ ] **GAP04-C35-004** — Include negative/fail-closed assertions, not only happy-path behavior.

- [ ] **GAP04-C35-005** — Integrate the suite into CI/release qualification with clearly defined blocking criteria.

- [ ] **GAP04-C35-006** — Create a threat model covering malicious network peers, compromised adjacent services, hostile local processes, stolen disk, clock manipulation, rollback, operator misuse, and supply-chain tampering.

- [ ] **GAP04-C35-007** — Test forged/modified leases, policies, capabilities, audit events, and reconciliation messages.

- [ ] **GAP04-C35-008** — Test replay of old valid messages after revocation, restart, ownership change, and snapshot restore.

- [ ] **GAP04-C35-009** — Test spoofed reachability and downgrade attacks against authentication, crypto algorithms, protocol versions, and policy schemas.
...
- [ ] **GAP04-C35-015** — Test break-glass/override abuse including expired approval and scope expansion.

- [ ] **GAP04-C35-016** — Perform dependency vulnerability scanning and targeted penetration testing for exposed management surfaces.

- [ ] **GAP04-C35-017** — Verify security failures produce sanitized external errors and sufficiently detailed protected audit evidence.

- [ ] **GAP04-C35-018** — Map tests to threat-model controls and accepted residual risks.
...
- [ ] **GAP04-C35-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C35-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C36-001** — Link every scenario to explicit requirements/invariants and stable test IDs.

- [ ] **GAP04-C36-002** — Use deterministic seeds/fixtures and archive enough evidence to reproduce failures.

- [ ] **GAP04-C36-003** — Run tests under production-equivalent optimization, crypto, persistence, and concurrency settings.

- [ ] **GAP04-C36-004** — Include negative/fail-closed assertions, not only happy-path behavior.
...
- [ ] **GAP04-C36-007** — Version schemas explicitly and document backward/forward compatibility policy.

- [ ] **GAP04-C36-008** — Create golden fixtures for minimum, typical, maximum, optional-field, deprecated-field, and invalid payloads.

- [ ] **GAP04-C36-009** — Validate emitted messages against schemas in tests, not only accepted messages.

- [ ] **GAP04-C36-010** — Test unknown fields, missing fields, enum extension, numeric limits, Unicode, ordering-insensitive fields, and duplicate-key parser behavior.

- [ ] **GAP04-C36-011** — Run supported N/N-1 or documented version-skew test matrices between GAP-04 and adjacent components.

- [ ] **GAP04-C36-012** — Define negotiation/rejection behavior for unsupported major/minor versions.

- [ ] **GAP04-C36-013** — Pin canonical serialization fixtures for signed/hashed structures.

- [ ] **GAP04-C36-014** — Generate or validate client/server models from schemas where practical to reduce drift.

- [ ] **GAP04-C36-015** — Fail CI when code and published schemas diverge.

- [ ] **GAP04-C36-016** — Include machine-readable error schema fixtures.

- [ ] **GAP04-C36-017** — Test migrations of persisted record schemas across supported releases.

- [ ] **GAP04-C36-018** — Archive fixtures as part of release artifacts.
...
- [ ] **GAP04-C36-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C36-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C37-001** — Link every scenario to explicit requirements/invariants and stable test IDs.

- [ ] **GAP04-C37-002** — Use deterministic seeds/fixtures and archive enough evidence to reproduce failures.

- [ ] **GAP04-C37-003** — Run tests under production-equivalent optimization, crypto, persistence, and concurrency settings.

- [ ] **GAP04-C37-004** — Include negative/fail-closed assertions, not only happy-path behavior.
...
- [ ] **GAP04-C37-008** — Exercise normal connected mode, partition entry, offline decisions, tier transitions, reconnect, reconciliation, renewal, and policy/security updates end to end.

- [ ] **GAP04-C37-009** — Verify identity and trust propagation across all boundaries.

- [ ] **GAP04-C37-010** — Verify policy digest, authority epoch, partition epoch, controller generation, decision ID, and trace context remain consistent across systems.

- [ ] **GAP04-C37-011** — Inject one dependency failure at a time and validate fail-closed or degraded behavior.
...
- [ ] **GAP04-C37-015** — Verify metrics/logs/traces correlate one scenario across all services.

- [ ] **GAP04-C37-016** — Test supervisor side effects against replicated/reconciled authoritative state.

- [ ] **GAP04-C37-017** — Include real persistence and key-store implementations in qualification runs.
...
- [ ] **GAP04-C37-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C37-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C38-001** — Define representative workload/hardware profiles and record exact test environment metadata.

- [ ] **GAP04-C38-002** — Measure latency, throughput, resource use, queueing, and saturation with reproducible methodology.

- [ ] **GAP04-C38-003** — Establish release-regression thresholds tied to SLO/capacity requirements.

- [ ] **GAP04-C38-004** — Test both steady-state and degraded/reconnect/crypto/persistence-heavy conditions.

- [ ] **GAP04-C38-005** — Archive raw results and trend them across releases.

- [ ] **GAP04-C38-006** — Define service-level indicators for local decision latency, durable commit latency, reconnect convergence, reconciliation latency, and dependency call latency.
...
- [ ] **GAP04-C38-010** — Measure cold/warm startup, recovery scan time, policy load, journal replay, and readiness time.
...
- [ ] **GAP04-C38-013** — Define workload distributions and dataset sizes so results are reproducible.
...
- [ ] **GAP04-C38-017** — Capture compiler/runtime/build flags and hardware metadata with benchmark results.
...
- [ ] **GAP04-C38-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C38-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C39-001** — Define representative workload/hardware profiles and record exact test environment metadata.

- [ ] **GAP04-C39-002** — Measure latency, throughput, resource use, queueing, and saturation with reproducible methodology.

- [ ] **GAP04-C39-003** — Establish release-regression thresholds tied to SLO/capacity requirements.

- [ ] **GAP04-C39-004** — Test both steady-state and degraded/reconnect/crypto/persistence-heavy conditions.
...
- [ ] **GAP04-C39-009** — Simulate fleet-wide reconnect storms with synchronized and jittered node recovery.
...
- [ ] **GAP04-C39-014** — Verify audit/journal sequence continuity after the entire run.

- [ ] **GAP04-C39-015** — Measure reconciliation convergence and backend load under fleet storms.
...
- [ ] **GAP04-C39-018** — Preserve seed/config/result artifacts for reproducibility.
...
- [ ] **GAP04-C39-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C39-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C40-001** — Define representative workload/hardware profiles and record exact test environment metadata.

- [ ] **GAP04-C40-002** — Measure latency, throughput, resource use, queueing, and saturation with reproducible methodology.

- [ ] **GAP04-C40-003** — Establish release-regression thresholds tied to SLO/capacity requirements.

- [ ] **GAP04-C40-004** — Test both steady-state and degraded/reconnect/crypto/persistence-heavy conditions.

- [ ] **GAP04-C40-005** — Archive raw results and trend them across releases.

- [ ] **GAP04-C40-006** — Model maximum decisions per second and decisions per partition for each supported hardware tier.

- [ ] **GAP04-C40-007** — Model journal bytes per decision including framing, index, integrity metadata, encryption overhead, and compaction amplification.

- [ ] **GAP04-C40-008** — Model audit bytes per event and retained/export backlog.

- [ ] **GAP04-C40-009** — Model reconciliation batch sizes, maximum backlog, remote request volume, and expected drain time.
...
- [ ] **GAP04-C40-014** — Include safety margin and degraded-dependency multipliers rather than sizing to nominal averages.

- [ ] **GAP04-C40-015** — Derive storage reservation from maximum disconnected duration and peak decision rate.

- [ ] **GAP04-C40-016** — Define saturation signals and thresholds that trigger backpressure/freeze before failure.

- [ ] **GAP04-C40-017** — Validate the model empirically using load/soak test results.

- [ ] **GAP04-C40-018** — Document assumptions and recalculate when schema/crypto/retention changes alter record sizes.

- [ ] **GAP04-C40-019** — Provide an operator sizing worksheet or machine-readable calculator.
...
- [ ] **GAP04-C40-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C40-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C41-001** — Document authoritative state, RPO/RTO, trust/identity constraints, and rollback hazards.

- [ ] **GAP04-C41-002** — Integrity-protect and version backup/migration artifacts.
...
- [ ] **GAP04-C41-006** — Classify which state is authoritative and must be backed up versus reconstructible cache.
...
- [ ] **GAP04-C41-019** — Store restore tooling/version alongside release artifacts.
...
- [ ] **GAP04-C41-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C41-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C42-002** — Make operator actions durable, time-bounded where appropriate, and fully auditable.

- [ ] **GAP04-C42-003** — Protect management interfaces against replay, stale commands, cross-site scope errors, and unauthorized local access.

- [ ] **GAP04-C42-004** — Expose active emergency/override state prominently in health and dashboards.

- [ ] **GAP04-C42-005** — Exercise operator workflows through tabletop/game-day and negative authorization testing.

- [ ] **GAP04-C42-006** — Define distinct states for normal, degraded, frozen, quarantined, and administratively disabled operation.

- [ ] **GAP04-C42-007** — Specify which read-only/diagnostic actions remain available in each state and which mutations are categorically denied.

- [ ] **GAP04-C42-008** — Provide authenticated local and remote triggers with scoped authorization and stable reason codes.

- [ ] **GAP04-C42-009** — Make quarantine/disable durable across restart and impossible to clear merely by reconnect or service restart.

- [ ] **GAP04-C42-010** — Require explicit authenticated recovery action plus health prerequisites before returning to normal service.
...
- [ ] **GAP04-C42-012** — Ensure entering quarantine can still record the transition using reserved audit capacity.
...
- [ ] **GAP04-C42-016** — Audit trigger, initiator, reason, scope, resulting actions, and clear event.

- [ ] **GAP04-C42-017** — Expose quarantine status prominently through health, metrics, and dashboards.

- [ ] **GAP04-C42-018** — Document emergency procedures and recovery evidence requirements.

- [ ] **GAP04-C42-019** — Exercise the mechanism in incident-response game days.

- [ ] **GAP04-C42-020** — Production gate: critical integrity/security failures can force a durable safe state with a controlled, attributable recovery path.
...
- [ ] **GAP04-C42-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C42-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C43-001** — Define staged promotion criteria, rollback constraints, and supported mixed-version windows.

- [ ] **GAP04-C43-002** — Verify exact artifact identity/signature/provenance before activation.

- [ ] **GAP04-C43-003** — Coordinate durable schema/protocol migration with rollback and compatibility strategy.
...
- [ ] **GAP04-C43-006** — Define release rings/stages and eligibility criteria for dev, lab, canary, limited production, and broad production.
...
- [ ] **GAP04-C43-008** — Support mixed-version operation only for versions explicitly listed in the compatibility matrix.

- [ ] **GAP04-C43-009** — Coordinate schema/protocol changes with expand/contract or equivalent safe migration strategy.

- [ ] **GAP04-C43-010** — Prevent rollout of a build that cannot read existing durable state or safely downgrade where rollback is required.

- [ ] **GAP04-C43-011** — Define automatic halt conditions and manual approval points.
...
- [ ] **GAP04-C43-013** — Tag every controller instance with release digest/channel and expose it in telemetry.
...
- [ ] **GAP04-C43-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C43-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C44-001** — Make supported environments/dependencies explicit and machine-verifiable where possible.

- [ ] **GAP04-C44-002** — Reject unsupported/incompatible environments before safety-critical operation.

- [ ] **GAP04-C44-003** — Retain exact build/dependency/version evidence with every release.

- [ ] **GAP04-C44-004** — Automate compatibility and packaging checks in CI.

- [ ] **GAP04-C44-005** — Define deprecation/EOL handling for dependencies and runtime platforms.
...
- [ ] **GAP04-C44-007** — Distinguish build-time, runtime, protocol, and persisted-schema compatibility.
...
- [ ] **GAP04-C44-011** — Reject startup or readiness when a required dependency advertises an unsupported incompatible major/protocol version.
...
- [ ] **GAP04-C44-018** — Version the matrix itself and ship it with release documentation.
...
- [ ] **GAP04-C44-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C44-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C45-001** — Make supported environments/dependencies explicit and machine-verifiable where possible.

- [ ] **GAP04-C45-002** — Reject unsupported/incompatible environments before safety-critical operation.

- [ ] **GAP04-C45-003** — Retain exact build/dependency/version evidence with every release.

- [ ] **GAP04-C45-004** — Automate compatibility and packaging checks in CI.

- [ ] **GAP04-C45-005** — Define deprecation/EOL handling for dependencies and runtime platforms.
...
- [ ] **GAP04-C45-008** — Separate direct from transitive dependencies and document why each privileged/security-critical dependency is required.

- [ ] **GAP04-C45-009** — Support deterministic/reproducible builds to the extent practical and record build environment/toolchain versions.
...
- [ ] **GAP04-C45-014** — Verify package file permissions/ACLs, service identity, config directories, state directories, and key-store permissions.

- [ ] **GAP04-C45-015** — Do not bundle development/test secrets, private signing material, debug endpoints, or stale artifacts.

- [ ] **GAP04-C45-016** — Generate cryptographic checksums and signed manifests for package contents.

- [ ] **GAP04-C45-017** — Define upgrade behavior for existing durable state and preserve rollback compatibility rules.

- [ ] **GAP04-C45-018** — Scan locked dependencies for known vulnerabilities and license policy.

- [ ] **GAP04-C45-019** — Archive lockfiles/manifests with release evidence.
...
- [ ] **GAP04-C45-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C45-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C46-001** — Document software supply-chain threat model, trusted builders/signers, and artifact verification policy.

- [ ] **GAP04-C46-002** — Generate machine-readable composition/provenance evidence for every release.
...
- [ ] **GAP04-C46-006** — Generate an SBOM in an industry-standard machine-readable format covering direct/transitive libraries, runtime, bundled binaries, and relevant system components.
...
- [ ] **GAP04-C46-010** — Publish/ship checksums and signatures separately enough that a single artifact replacement cannot replace both evidence and payload unnoticed.
...
- [ ] **GAP04-C46-016** — Test tampered artifact, tampered SBOM, altered provenance, revoked signer, and expired credential paths.
...
- [ ] **GAP04-C46-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C46-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C47-002** — Use stable IDs and version-controlled records so decisions/status changes are traceable.

- [ ] **GAP04-C47-003** — Define objective completion/approval criteria and prohibit unsupported self-attestation.

- [ ] **GAP04-C47-004** — Link governance records to implementation, tests, releases, incidents, and exceptions.
...
- [ ] **GAP04-C47-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C48-002** — Use stable IDs and version-controlled records so decisions/status changes are traceable.

- [ ] **GAP04-C48-003** — Define objective completion/approval criteria and prohibit unsupported self-attestation.

- [ ] **GAP04-C48-004** — Link governance records to implementation, tests, releases, incidents, and exceptions.

- [ ] **GAP04-C48-005** — Review the control after major incident, architecture change, or release-process change.

- [ ] **GAP04-C48-006** — Create ADRs for offline authority model, lease semantics, trusted time, persistence substrate, audit integrity, reconciliation strategy, fencing, identity/authentication, encryption, and rollout compatibility.

- [ ] **GAP04-C48-007** — State context/problem, decision, alternatives considered, decision drivers, security implications, operational implications, and consequences.

- [ ] **GAP04-C48-008** — Record rejected alternatives and why they were not selected.

- [ ] **GAP04-C48-009** — Link each ADR to requirements, code modules, schemas, tests, and affected adjacent components.

- [ ] **GAP04-C48-010** — Assign ADR status: proposed, accepted, superseded, deprecated, or rejected.
...
- [ ] **GAP04-C48-014** — Include explicit threat assumptions and failure model for distributed/offline decisions.

- [ ] **GAP04-C48-015** — Include compatibility/migration implications for decisions affecting persisted data or wire protocols.
...
- [ ] **GAP04-C48-018** — Maintain an ADR index by control/component.
...
- [ ] **GAP04-C48-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C49-002** — Use stable IDs and version-controlled records so decisions/status changes are traceable.

- [ ] **GAP04-C49-003** — Define objective completion/approval criteria and prohibit unsupported self-attestation.

- [ ] **GAP04-C49-004** — Link governance records to implementation, tests, releases, incidents, and exceptions.

- [ ] **GAP04-C49-005** — Review the control after major incident, architecture change, or release-process change.

- [ ] **GAP04-C49-006** — Create a unique immutable requirement/control ID for each GAP-04 requirement, including the original 100 checks and all post-audit missing-component controls.

- [ ] **GAP04-C49-007** — Map each requirement to implementation module/function, configuration/policy field, schema, test case, operational monitor, evidence artifact, owner, and status.

- [ ] **GAP04-C49-008** — Distinguish implemented, partially implemented, verified, waived, not applicable, and blocked states.

- [ ] **GAP04-C49-009** — Require objective evidence references rather than narrative claims of completion.
...
- [ ] **GAP04-C49-013** — Map P0/P1/P2 priority and production gate impact.

- [ ] **GAP04-C49-014** — Include adjacent-system dependencies and the exact versions used for verification.

- [ ] **GAP04-C49-015** — Track known gaps and waiver IDs directly in the matrix.

- [ ] **GAP04-C49-016** — Require reviewer/approver and verification date for controls marked complete.

- [ ] **GAP04-C49-017** — Generate release summary from the matrix rather than manually maintained status prose.

- [ ] **GAP04-C49-018** — Add CI checks preventing deletion/renumbering of control IDs without migration.
...
- [ ] **GAP04-C49-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C50-002** — Use stable IDs and version-controlled records so decisions/status changes are traceable.

- [ ] **GAP04-C50-003** — Define objective completion/approval criteria and prohibit unsupported self-attestation.

- [ ] **GAP04-C50-004** — Link governance records to implementation, tests, releases, incidents, and exceptions.

- [ ] **GAP04-C50-005** — Review the control after major incident, architecture change, or release-process change.

- [ ] **GAP04-C50-006** — Define SLIs for safety-readiness availability, local decision latency, durable decision success, reconciliation convergence, policy/revocation freshness, and dependency availability where applicable.

- [ ] **GAP04-C50-007** — Set objective targets and measurement windows for each SLI.

- [ ] **GAP04-C50-008** — Define which errors count against the budget and which expected partition states do not.

- [ ] **GAP04-C50-009** — Define separate objectives for connected and disconnected operating modes where behavior differs.

- [ ] **GAP04-C50-010** — Specify maximum tolerated time in stale-policy, unreconciled, safety-not-ready, or degraded states.

- [ ] **GAP04-C50-011** — Define data sources and exact metric queries used to calculate each SLI.

- [ ] **GAP04-C50-012** — Set alerting burn-rate thresholds and escalation policies.

- [ ] **GAP04-C50-013** — Define release/change restrictions when error budget is exhausted.

- [ ] **GAP04-C50-014** — Document exclusions carefully and prohibit ad-hoc post-incident exclusion of bad periods.

- [ ] **GAP04-C50-015** — Review whether SLOs align with maximum lease and disconnect risk windows.
...
- [ ] **GAP04-C50-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C50-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C51-002** — Use stable IDs and version-controlled records so decisions/status changes are traceable.

- [ ] **GAP04-C51-003** — Define objective completion/approval criteria and prohibit unsupported self-attestation.

- [ ] **GAP04-C51-004** — Link governance records to implementation, tests, releases, incidents, and exceptions.

- [ ] **GAP04-C51-005** — Review the control after major incident, architecture change, or release-process change.

- [ ] **GAP04-C51-006** — Define severity levels with concrete GAP-04 examples: split-brain, audit integrity failure, lease verification failure, stale revocation, journal corruption, prolonged partition, data loss, security compromise.
...
- [ ] **GAP04-C51-008** — Provide first-response checks for health, version/config/policy, lease, epochs, storage, dependencies, and recent changes.

- [ ] **GAP04-C51-009** — Document safe containment actions: freeze, quarantine, revoke lease, disable rollout, isolate node/site, rotate keys, and preserve evidence.

- [ ] **GAP04-C51-010** — Explicitly warn against unsafe actions such as deleting journals, resetting epochs, rolling clocks, or restoring stale backups without authority review.

- [ ] **GAP04-C51-011** — Define evidence collection preserving audit chain, logs, traces, state hashes, memory/core dumps where allowed, and artifact versions.

- [ ] **GAP04-C51-012** — Provide decision trees for reconnect/reconciliation failures and data-integrity uncertainty.

- [ ] **GAP04-C51-013** — Define recovery validation before unfreezing autonomy.

- [ ] **GAP04-C51-014** — Define post-incident reconciliation and customer/data impact assessment.

- [ ] **GAP04-C51-015** — Require incident timeline and root-cause analysis for specified severities.

- [ ] **GAP04-C51-016** — Link runbook steps to tested operator commands/tooling.
...
- [ ] **GAP04-C51-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C51-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C52-002** — Use stable IDs and version-controlled records so decisions/status changes are traceable.

- [ ] **GAP04-C52-003** — Define objective completion/approval criteria and prohibit unsupported self-attestation.

- [ ] **GAP04-C52-004** — Link governance records to implementation, tests, releases, incidents, and exceptions.
...
- [ ] **GAP04-C52-007** — Define severity methodology and remediation SLAs for critical/high/medium/low findings.

- [ ] **GAP04-C52-008** — Define emergency patch and release-signing process for critical vulnerabilities.

- [ ] **GAP04-C52-009** — Map SBOM components to deployed/supported releases so impact can be assessed quickly.

- [ ] **GAP04-C52-010** — Define coordinated disclosure and security contact procedures.

- [ ] **GAP04-C52-011** — Define supported release windows and minimum upgrade cadence.

- [ ] **GAP04-C52-012** — Publish deprecation and end-of-life timelines with advance notice.

- [ ] **GAP04-C52-013** — Define behavior for dependencies/runtimes that reach upstream EOL before GAP-04.

- [ ] **GAP04-C52-014** — Require compensating controls/waivers for overdue vulnerabilities with owner and expiry.

- [ ] **GAP04-C52-015** — Maintain ability to revoke compromised release artifacts/signers.

- [ ] **GAP04-C52-016** — Track vulnerability remediation to verification tests and release evidence.

- [ ] **GAP04-C52-017** — Define customer/operator notification thresholds for exploited or high-impact issues.
...
- [ ] **GAP04-C52-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C52-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C53-002** — Use stable IDs and version-controlled records so decisions/status changes are traceable.

- [ ] **GAP04-C53-003** — Define objective completion/approval criteria and prohibit unsupported self-attestation.

- [ ] **GAP04-C53-004** — Link governance records to implementation, tests, releases, incidents, and exceptions.

- [ ] **GAP04-C53-005** — Review the control after major incident, architecture change, or release-process change.

- [ ] **GAP04-C53-006** — Schedule recurring reviews for access/identity, policy semantics, configuration, dependencies, cryptography, threat model, architecture, capacity, and incident lessons.

- [ ] **GAP04-C53-007** — Define review frequency based on risk; security-critical controls should have explicit minimum cadence.

- [ ] **GAP04-C53-008** — Assign required attendees/approvers for each review domain.

- [ ] **GAP04-C53-009** — Use checklists with evidence references rather than informal meetings.

- [ ] **GAP04-C53-010** — Review active exceptions/waivers and force expiration or renewal decisions.

- [ ] **GAP04-C53-011** — Review signer/trust-store contents and remove obsolete credentials.

- [ ] **GAP04-C53-012** — Review dependency compatibility/CVE status and supported runtime/OS lifecycle.

- [ ] **GAP04-C53-013** — Review policy/config drift across sites against approved baselines.

- [ ] **GAP04-C53-014** — Review SLO/error-budget and alert quality.

- [ ] **GAP04-C53-015** — Review capacity assumptions against recent production/load evidence.
...
- [ ] **GAP04-C53-019** — Trigger out-of-cycle review after major incident, security event, architecture change, or cryptographic migration.
...
- [ ] **GAP04-C53-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C54-002** — Use stable IDs and version-controlled records so decisions/status changes are traceable.

- [ ] **GAP04-C54-003** — Define objective completion/approval criteria and prohibit unsupported self-attestation.

- [ ] **GAP04-C54-004** — Link governance records to implementation, tests, releases, incidents, and exceptions.

- [ ] **GAP04-C54-005** — Review the control after major incident, architecture change, or release-process change.

- [ ] **GAP04-C54-006** — Create a unique ID for every accepted exception, waiver, or known control debt.

- [ ] **GAP04-C54-007** — Record affected control/component, risk statement, technical rationale, scope, environment, and why immediate remediation is not feasible.
...
- [ ] **GAP04-C54-010** — Set creation date, review date, and hard expiration date.

- [ ] **GAP04-C54-011** — Prohibit indefinite waivers for P0 controls without executive/security authority and explicit risk acceptance.

- [ ] **GAP04-C54-012** — Link each waiver to tracking issue/remediation plan and target release.

- [ ] **GAP04-C54-013** — Surface active waivers in release/GO evidence automatically.
...
- [ ] **GAP04-C54-016** — Track aggregate debt by priority/age/owner.

- [ ] **GAP04-C54-017** — Define criteria that force a waiver to block production or rollout.

- [ ] **GAP04-C54-018** — Retain historical waiver decisions for auditability.

- [ ] **GAP04-C54-019** — Integrate register status with requirements traceability.
...
- [ ] **GAP04-C54-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C55-002** — Use stable IDs and version-controlled records so decisions/status changes are traceable.

- [ ] **GAP04-C55-003** — Define objective completion/approval criteria and prohibit unsupported self-attestation.

- [ ] **GAP04-C55-004** — Link governance records to implementation, tests, releases, incidents, and exceptions.

- [ ] **GAP04-C55-005** — Review the control after major incident, architecture change, or release-process change.

- [ ] **GAP04-C55-006** — Define machine-readable GO criteria covering all P0 controls and designated mandatory P1 evidence.

- [ ] **GAP04-C55-007** — Consume objective artifacts: test results, SBOM/provenance verification, compatibility matrix, traceability matrix, vulnerability status, waivers, SLO baselines, and config/policy validation.

- [ ] **GAP04-C55-008** — Refuse GO automatically when any required control is missing, failed, stale, unsigned, or supported only by narrative assertion.

- [ ] **GAP04-C55-009** — Define freshness windows for evidence such as penetration tests, dependency scans, and soak tests.

- [ ] **GAP04-C55-010** — Bind gate output to exact source commit, build digest, package digest, config class, and target environment.

- [ ] **GAP04-C55-011** — Require named approvals from engineering, security, operations/SRE, and product/risk owners as appropriate.

- [ ] **GAP04-C55-012** — Separate technical pass from business acceptance of explicitly permitted residual risk.

- [ ] **GAP04-C55-013** — Produce a signed/immutable release decision artifact with timestamp and evidence references.

- [ ] **GAP04-C55-014** — Ensure an older GO decision cannot be replayed for a different artifact or newer environment.

- [ ] **GAP04-C55-015** — Add gate tests containing intentionally missing/failed evidence to prove fail-closed behavior.
...
- [ ] **GAP04-C55-019** — Version the gate policy itself and review changes.
...
- [ ] **GAP04-C55-022** — Implementation references are mapped to the control IDs above.
...
- [ ] **GAP04-C55-025** — Health/telemetry/runbook evidence exists for production diagnosis and recovery where applicable.
...
- [ ] **GAP04-C56-002** — Keep normative requirements synchronized with code, schemas, tests, and manifests.

- [ ] **GAP04-C56-003** — Use stable cross-references and automated link/version consistency checks.

- [ ] **GAP04-C56-004** — Document assumptions, non-goals, dependencies, and known limitations explicitly.

- [ ] **GAP04-C56-005** — Require documentation updates as part of the definition of done for affected controls.

- [ ] **GAP04-C56-006** — Recover or author the authoritative master source that defines GAP-04 purpose, scope, invariants, adjacent dependencies, implementation workflow, and acceptance gates.

- [ ] **GAP04-C56-007** — Assign a document version and map it to the GAP-04 software/release series.

- [ ] **GAP04-C56-008** — Include the 56 post-audit missing components and their control IDs or explicit references to this checklist.

- [ ] **GAP04-C56-009** — Define normative versus informative sections so implementers know which statements are requirements.

- [ ] **GAP04-C56-010** — Include architecture overview, data/control flows, trust boundaries, state machine, lease lifecycle, partition lifecycle, reconciliation lifecycle, and failure model.

- [ ] **GAP04-C56-011** — Document required inputs/outputs and versioned contracts for GAP-01, GAP-05, GAP-12, GAP-13, PLN-07, and `pk_core`.

- [ ] **GAP04-C56-012** — Include security assumptions, threat model references, cryptographic profiles, identity model, trusted-time requirements, and key-management dependencies.

- [ ] **GAP04-C56-013** — Include persistence, crash recovery, storage pressure, backup/restore, and migration requirements.

- [ ] **GAP04-C56-014** — Include test/verification strategy with links to contract, fault-injection, security, performance, and GO-gate evidence.

- [ ] **GAP04-C56-015** — Include operational runbooks, health/metrics/logging expectations, alerting, ownership, and escalation references.

- [ ] **GAP04-C56-016** — Correct any claims that a file/component is bundled when it is not actually present.

- [ ] **GAP04-C56-017** — Add CI/documentation checks for broken links, stale version references, missing required sections, and mismatch with manifest.

- [ ] **GAP04-C56-018** — Include a machine-readable manifest of normative artifacts if practical.
...
- [ ] **GAP04-C56-022** — Implementation references are mapped to the control IDs above.
