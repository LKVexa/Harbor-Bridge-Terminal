"""Inventory-framework conformance test for INV-63 (inv63_wasm_deployment_manager) -- stdlib unittest, no network.

Run from the folder that contains this package (and ``pk_core``), e.g.::

    python inv63_wasm_deployment_manager/tests/test_component.py
...
@unittest.skipIf(pk_core is None, "pk_core not importable; framework conformance not executed (core tests remain active)")
