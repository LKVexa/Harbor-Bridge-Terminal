"""Conformance test for INV-41 (inv41_capability_security) -- stdlib unittest, no network.

Run from the folder that contains this package (and ``pk_core``), e.g.::

    python inv41_capability_security/tests/test_component.py
...
class EstateGateTest(unittest.TestCase):
    """Section 6: estate integration never reports success when pk_core is absent.

    In the production profile (INV41_PROFILE=production) a missing pk_core is a
    FAILURE, not a skip.  In development it is recorded as BLOCKED by
    tools/estate_gate.py and the conformance tests above skip.
    """

    def test_missing_pk_core_is_not_success(self):
        if pk_core is not None:
            self.skipTest("pk_core present; ConformanceTest is authoritative")
        if os.environ.get("INV41_PROFILE") == "production":
            self.fail("production profile: pk_core missing -> estate gate FAIL (BLOCKERS.json B-EST-01)")
        out = subprocess.run([sys.executable, "-B", str(PKG_DIR / "tools" / "estate_gate.py"), "--profile", "production"],
