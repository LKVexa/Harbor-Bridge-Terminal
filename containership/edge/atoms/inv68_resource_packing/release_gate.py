"""Local production exit gate with machine-readable acceptance evidence (MC-40, MC-32; C090, C100).

    python -m inv68_resource_packing.release_gate [--evidence evidence/] [--pk-gate FILE]
           [--approval FILE] [--today YYYY-MM-DD] [--out evidence/EXIT_GATE.json]

Shape adapted from the owner's INV-64 4.3.0 ``release_gate.py`` (INV-44 lineage); the INV-68
gate is stricter:
...
* every INV-68-C### control must be ``verified`` or ``not_applicable`` (or
  waived as above);
* pk_core gate output is required (``--pk-gate``) and must be for INV-68 with
...
GATE_SCHEMA = "PK_PACK_EXIT_GATE/1"
...
def evaluate(*, policy: dict, components: dict, matrix: dict, register: dict, evidence_dir: Path,
    ...
    for ctl in policy["evidence"]:
        ...
        if not p.is_file():
            ...
            if not isinstance(doc, dict) or doc.get("schema") != ctl["schema"]:
                ...
                entry.update(state="fail", reason=f"{doc.get('skipped')} skipped tests (skips never satisfy a gate)")
            elif ctl.get("certification_field") and not doc.get(ctl["certification_field"]):
                entry.update(state="fail", reason=f"not certification grade ({ctl['certification_field']}=false)")
            else:
    ...
    if bad:
        blockers.append(f"{len(bad)} of {len(matrix['requirements'])} INV-68-C### controls not verified (first: {bad[0]})")
    # 4. external evidence
    if not pk_gate:
        blockers.append("pk_core PK_GATE_RESULTS not supplied (MC-02)")
    elif pk_gate.get("element", "INV-68") != "INV-68" or str(pk_gate.get("verdict")).upper() not in ("PASS", "GO"):
        blockers.append("pk_core gate result is not a PASS for INV-68")
...
def main(argv=None) -> int:
    ...
    from inv68_resource_packing.service import SUPPORTED_PROTOCOLS
    from inv68_resource_packing.tools.common import source_digest
    src = load(ROOT / "provenance" / "master-source.json")
    out = {"schema": GATE_SCHEMA, "element": "INV-68", "version": __version__, "gate_policy_version": policy["version"],
           "protocols": list(SUPPORTED_PROTOCOLS), "master_source_status": src["status"],
           "build_digest": source_digest(), "source_corpus_sha256": [x["sha256"] for x in src["sources"]],
