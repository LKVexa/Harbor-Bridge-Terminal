...
def build() -> Contract:
    ...
    return Contract(
        ...
        interfaces={
            "pack": "PK_PACK/1 - workloads to host assignments; schemas/PK_PACK_REQUEST_1.schema.json + schemas/PK_PACK_RESPONSE_1.schema.json",
            "capacity": "PK_PACK_CAPACITY/1 - per-host, per-dimension capacity; schemas/PK_PACK_CAPACITY_1.schema.json",
            "fragmentation": "PK_PACK_FRAG/1 - stranded capacity report; schemas/PK_PACK_FRAG_1.schema.json",
            "service": "PK_PACK_SERVICE_RESPONSE/1 - authenticated, audited boundary (service.py); INTERFACES.md",
            "errors": "PK_PACK_ERROR/1 + ERRORS.json registry",
            "config": "PK_PACK_CONFIG/1 - transactional configuration (config.py)",
            "status": "PK_PACK_STATUS/1 - health/readiness/config/dependencies",
            "explain": "PK_PACK_EXPLAIN/1 - operator explain view",
            "audit": "PK_PACK_AUDIT/1 - hash-chained security audit records"
        },
        threats=[
            ...
            "Forged, replayed or over-privileged caller tokens (THREAT_MODEL.md T1-T4)",
            "Hostile input and algorithmic-complexity exhaustion (T5-T7)",
            "Stale or spoofed capacity data (T8)",
            "Unaudited or split-brain configuration changes (T9-T13)"
        ],
        failure_modes=[
            ...
            "Headroom breached",
            "Capacity data stale",
            "Capacity source unreachable",
            "Audit sink unavailable",
            "Configuration store corrupt or torn",
            "Stale controller epoch",
            "Overload / queue exhausted"
        ],
        slos=[
        ...
        signals={
            ...
            "mem_overcommit_hosts": "gauge",
            "efficiency_ratio": "gauge"
        },
    )
