...
"""Conformance test for INV-30 (inv30_capability_hardware_sandbox) -- stdlib unittest, no network.

Run from the folder that contains this package (and ``pk_core``), e.g.::

    python inv30_capability_hardware_sandbox/tests/test_component.py
...
KNOWN_PARTIAL = []  # hardware-absent partials carry "not installed"/"no CHERI" notes, see component.py
...
class ConformanceTest(unittest.TestCase):
    ...
    def test_all_100_requirements_answered(self):
        ...
        unexpected = sorted(f.check_id for f in findings if not f.status.passing
                            ...
                            and "CHERI hardware" not in f.note and "sign-off" not in f.note)
