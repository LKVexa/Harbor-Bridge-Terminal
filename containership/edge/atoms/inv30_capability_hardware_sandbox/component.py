...
require_pk_core()  # stable DEPENDENCY_INCOMPATIBLE diagnostic for unsupported pk_core
...
class CapabilityHardwareSandboxComponent(Component):
    ...
    # -- 4.3.0: honest reporting of what only real hardware or humans can close ----
    _HARDWARE_ITEMS = {
        "INV-30-C031": "CHERI spec pinned (docs/CHERI_SPEC_PIN.md) but no approved CHERI implementation is bound",
        "INV-30-C084": "compatibility matrix exists; the CHERI rows have no executed evidence",
        "INV-30-C068": "power/thermal needs edge hardware sensors",
    }
    _SIGNOFF_ITEMS = {
        "INV-30-C009": "owner",
        "INV-30-C010": "architecture_approver",
    }

    def assess(self, dimension):
        from pathlib import Path
        import json as _json
        from .backend import CheriHardwareBackend
        findings = super().assess(dimension)
        hw = CheriHardwareBackend().available()
        signoffs = {}
        sp = Path(__file__).resolve().parent / "evidence" / "SIGNOFFS.json"
        if sp.exists():
            signoffs = _json.loads(sp.read_text(encoding="utf-8")).get("signoffs", {})
        items = {i.check_id: i for i in self.checklist.for_dimension(dimension)}
        for n, f in enumerate(findings):
            if f.check_id in self._HARDWARE_ITEMS and not hw["available"]:
                findings[n] = self.partial(items[f.check_id], f.statement,
                                           *f.artifacts, note=f"no CHERI hardware backend: "
                                           f"{self._HARDWARE_ITEMS[f.check_id]} ({hw['reason']})")
            role = self._SIGNOFF_ITEMS.get(f.check_id)
            if role and not (signoffs.get(role) or {}).get("name"):
                findings[n] = self.partial(items[f.check_id], f.statement, *f.artifacts,
                                           note=f"sign-off pending: {role} (evidence/SIGNOFFS.json)")
        return findings
