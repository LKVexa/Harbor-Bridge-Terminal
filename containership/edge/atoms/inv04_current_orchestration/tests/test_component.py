"""Conformance test for INV-04 (inv04_current_orchestration) -- stdlib unittest, no network.
...
try:
    ...
    pk_core = None
    if os.environ.get("INV04_REQUIRE_PK_CORE") == "1":
        # Component 78: absence is a failed gate, never a silent skip, when required.
        raise SystemExit("pk_core required (INV04_REQUIRE_PK_CORE=1) but not importable") from None
...
class ConformanceTest(unittest.TestCase):
    def test_version(self):
        self.assertEqual(_load().__version__, (PKG_DIR / "VERSION").read_text().strip())
