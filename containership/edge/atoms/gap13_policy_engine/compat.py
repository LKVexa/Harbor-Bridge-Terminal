"""G13-MC-017 compatibility/version matrix (machine-readable, test-enforced)."""
...
MATRIX = {
    ...
    "bundle_schemas": {"accept": ["PK_POLICY_BUNDLE/1"], "reject": ["*"]},
    "envelope_schemas": {"accept": ["PK_POLICY_SIGNED_BUNDLE/1"]},
    "verdict_schema": "PK_POLICY_VERDICT/1",
    "explanation_schema": "PK_POLICY_EXPLANATION/1",
    "error_schema": "PK_POLICY_ERROR/1",
    "rpc": {"path_prefix": "/v1", "versions": ["v1"]},
    "python": {"min": "3.10", "tested": ["3.10", "3.11", "3.12", "3.13"]},
    "platforms": {"tested": ["linux"], "declared_supported": ["linux", "windows", "macos"],
                  "note": "Windows/macOS declared portable (stdlib, os.replace, no fork); CI evidence pending -- see WAIVERS.json W-004"},
    "optional_dependencies": {"cryptography": ">=41 (Ed25519 verification); absent -> bundles fail closed"},
    "adjacent_interfaces": {
