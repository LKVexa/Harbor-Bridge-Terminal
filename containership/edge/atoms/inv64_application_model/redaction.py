"""Secret-material detection and centralized redaction (MC-14; C039, C075).

Two jobs, one pattern set:

* :func:`find_secrets` walks a decoded value and reports *where* credential
  material sits (never *what* it is). The manifest validator turns each hit
  into a ``secret.inline`` issue, so a manifest carrying a token is refused
  before it can obtain a canonical identity.
...
def allowlisted(record: Mapping[str, Any], allow: Iterable[str]) -> dict:
    """Export only allowlisted fields, each redacted (MC-14 allowlist export)."""
    allowed = set(allow)
