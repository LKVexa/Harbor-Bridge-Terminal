"""Tamper-evident security audit ledger (MC-18; C049, C090).

Adapted from the owner's INV-44 v4.3.0 ``audit_log.py`` (hash chain + optional
HMAC + external head anchor). Additions for INV-64:

* event schema ``PK_APP_AUDIT/1`` with ``event_id``, ``operation``,
...
* ``python -m inv64_application_model.audit verify LEDGER [--anchor A --key-env VAR]``
...
class AuditLog:
    ...
    def append(self, operation: str, *, actor: str, tenant: str | None = None, outcome: str,
        ...
        with self._lock:
            ...
            if self._unreported_drops and len(self._buffer) < self._buffer_limit:
                self._buffer.append(self._chain(self._body("audit.loss", "inv64", None, "recorded", None,
                                                           "sink outage", None,
            ...
            if len(self._buffer) >= self._buffer_limit:
                ...
                if self._metrics:
                    self._metrics.inc("inv64_audit_dropped_total")
                return None
            ...
            if self._metrics:
                self._metrics.set("inv64_audit_buffered", len(self._buffer))
            return rec
    ...
    def seal(self, anchor_path: str | os.PathLike, *, key: bytes) -> dict:
        with self._lock:
            self._flush()
            cp = {"schema": "PK_APP_AUDIT_ANCHOR/1", "seq": self._seq, "hash": self._prev,
                  "ts": round(float(self._clock()), 6)}
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(prog="inv64-audit")
    ...
    try:
        ...
        out = {"schema": "PK_APP_AUDIT_VERIFY/1", "ledger": a.ledger, "result": "PASS", "records": n,
               "head": list(log.head())}
        code = 0
    except (AuditChainBroken, OSError, ValueError, KeyError) as exc:
        out = {"schema": "PK_APP_AUDIT_VERIFY/1", "ledger": a.ledger, "result": "FAIL", "error": str(exc)}
        code = 2
