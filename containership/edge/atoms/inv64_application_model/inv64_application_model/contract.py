...
def build() -> Contract:
    ...
    return Contract(
        ...
        interfaces={
            "manifest": "PK_APP_MANIFEST/1 - app/v1 JSON schema plus semantic reference validation",
            "validate": "PK_APP_VALIDATE/1 - stable code/path/message validation issues",
            "canonical": "PK_APP_CANONICAL/1 - SHA-256 of deterministic canonical JSON",
            "submit": "PK_APP_SUBMIT_REQUEST/1 -> PK_APP_SUBMIT_RESPONSE/1 over protocol PK_APP_SUBMIT/2 (authn, authz, idempotency)",
            "error": "PK_APP_ERROR/1 - versioned error envelope with stable codes and retryability",
            "status": "PK_APP_STATUS/1 - liveness, readiness, degraded/blocked reasons, versions",
            "overlay": "PK_APP_OVERLAY/1 - site/environment overlays with provenance",
            "audit": "PK_APP_AUDIT/1 - hash-chained, sealed security audit ledger",
            "decision": "PK_APP_DECISION/1 - explainable decision records"
        },
        threats=[
            ...
            "Unauthenticated or cross-tenant submission",
            "Inline secret material in manifests or diagnostics",
            "Tampered artifacts, policies or audit records"
        ],
        failure_modes=[
            "Schema unsupported",
            "Dangling link",
