"""Conformance test for INV-64 (inv64_application_model) -- stdlib unittest, no network.

Run from the folder that contains this package (and ``pk_core``), e.g.::

    python inv64_application_model/tests/test_component.py
...
class DependencyTest(unittest.TestCase):
    def test_pk_core_is_available_for_integration_conformance(self):
        self.assertIsNotNone(
            pk_core,
            "pk_core is required for integration conformance; set PK_CORE_PATH or install/mount the sibling core repository",
        )
