...
## 4.3.0 declarative configuration

`config.py` now provides `PK_INV31_CONFIG/1` (schema in `schemas/`, examples in `config/`) with provenance, atomic whole-generation activation, and automatic/operator rollback. Signed configuration still needs a KMS (BLOCKED).
