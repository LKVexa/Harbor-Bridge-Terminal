"""Conformance test for INV-62 (inv62_edge_topology) -- stdlib unittest, no network.

Run from the folder that contains this package (and ``pk_core``), e.g.::

    python inv62_edge_topology/tests/test_component.py
...
class ConformanceTest(unittest.TestCase):
    ...
    def test_checks_survive_optimised_mode(self):
        ...
        code = (f"import sys; sys.path[:0]={list(sys.path[:3])!r}; import importlib; "
                f"m=importlib.import_module({_load().__name__!r}); c=m.COMPONENT(); "
...
if __name__ == "__main__":
    if pk_core is None:
        print("ERROR: pk_core is required for the INV-62 conformance gate; set PK_CORE_PATH", file=sys.stderr)
        raise SystemExit(2)
    unittest.main()
