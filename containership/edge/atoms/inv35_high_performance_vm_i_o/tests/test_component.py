"""Conformance test for INV-35 (inv35_high_performance_vm_i_o) -- stdlib unittest, no network.

Run from the folder that contains this package (and ``pk_core``), e.g.::

    python inv35_high_performance_vm_i_o/tests/test_component.py
...
class ConformanceTest(unittest.TestCase):
    ...
    def test_all_100_requirements_have_findings(self):
        ...
        # This test verifies assessment coverage, not production certification.
        # Readiness is decided by pk_core gate/evidence policy and the repository
        # evidence set; unit tests must not turn contract-derived defaults into a
        # blanket "all requirements satisfied" claim.
        self.assertTrue(all(getattr(f, "check_id", None) for f in findings))
