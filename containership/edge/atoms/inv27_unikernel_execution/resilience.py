...
* ``Deadline`` - absolute monotonic deadline; ``check()`` raises ``UK_DEADLINE_EXCEEDED``.
* ``CancelToken`` - cooperative cancellation; ``check()`` raises ``UK_CANCELLED``.
* ``Admission`` - token bucket (rate/burst) per tenant plus a global in-flight cap; excess is shed
  with ``UK_OVERLOADED`` before any work is done, and one tenant's burst cannot starve another
...
class Deadline:
    ...
    def after(cls, ms: int, clock: Callable[[], float] = time.monotonic) -> "Deadline":
        if isinstance(ms, bool) or not isinstance(ms, int) or not 1 <= ms <= 60_000:
            raise UkError("UK_INVALID_REQUIREMENT", "deadline_ms must be an integer in 1..60000")
    ...
    def check(self) -> None:
        if self.remaining() <= 0:
            raise UkError("UK_DEADLINE_EXCEEDED")
    ...
    def admit(self, tenant: str):
        with self._lock:
            if self._inflight >= self.max_inflight:
                self.shed += 1
                raise UkError("UK_OVERLOADED", "in-flight cap reached", reason="inflight")
            if not self._take(tenant):
                self.shed += 1
                raise UkError("UK_OVERLOADED", "tenant rate exceeded", reason="rate")
            self._inflight += 1
        try:
            yield
        finally:
    ...
    for i in range(attempts):
        try:
            ...
            if deadline is not None and deadline.remaining() <= d:
                raise UkError("UK_DEADLINE_EXCEEDED", "no time left to retry", cause=e.code)
            sleep(d)
        ...
        with self._lock:
            if self.state == "open":
                if self.clock() - self.opened_at >= self.reset_after_s:
                    ...
                    raise UkError("UK_CIRCUIT_OPEN", f"{self.name} circuit open", dependency=self.name)
        try:
            out = fn()
        except Exception:
            with self._lock:
