"""Conformance test for INV-27 (inv27_unikernel_execution) -- stdlib unittest, no network.

Run from the folder that contains this package (and ``pk_core``), e.g.::

    python inv27_unikernel_execution/tests/test_component.py
...
ROOT = PKG_DIR.parent
for p in filter(None, [os.environ.get("PK_CORE_PATH"), str(PKG_DIR / "_vendor"), str(ROOT)]):
...
class ConformanceTest(unittest.TestCase):
    ...
    def test_checks_survive_optimised_mode(self):
        ...
        out = subprocess.run([sys.executable, "-B", "-O", "-c", code], capture_output=True, text=True, cwd=str(ROOT))
