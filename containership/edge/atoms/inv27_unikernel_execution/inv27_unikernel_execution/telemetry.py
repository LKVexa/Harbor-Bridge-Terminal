"""Metrics, structured logs and trace context for INV-27 (C072-C075, C079).
...
class Metrics:
    ...
    def __post_init__(self) -> None:
        for name in ("uk_images_admitted_total", "uk_seal_failures_total", "uk_errors_total", "uk_shed_total",
                     "uk_instances_started_total", "uk_instances_stopped_total"):
            self.counters[(name, ())] = 0
        for name in ("uk_inflight", "uk_unikernel_instances", "uk_trust_root_age_seconds",
                     "uk_quarantined_images", "uk_saturation_ratio"):
...
class Logger:
    ...
    def log(self, level: str, operation: str, msg: str, *, tenant: str | None = None, workload: str | None = None,
        ...
        if level in ("debug", "info") and self.sample_rate < 1.0 and secrets.randbelow(10_000) >= self.sample_rate * 10_000:
            return None
        rec = {"ts": self.clock(), "level": level, "component": "INV-27", "node": self.node,
