...
def build() -> Contract:
    ...
    return Contract(
        ...
        interfaces={
            ...
            "decide": "PK_GRAVITY_DECISION_REQUEST/1 -> PK_GRAVITY_DECISION/2 - signed, provenance-bearing, audited decision (service.py)",
            "policy": "PK_POLICY_REQUEST/1 -> PK_POLICY_VERDICT/1 (GAP-13, signed, request-bound)",
            "topology": "PK_TOPOLOGY_SNAPSHOT/1 (GAP-03, signed, versioned)",
            "convergence": "PK_CONVERGENCE_PROOF/1 (GAP-05, signed, dataset-bound)",
            "placement": "PK_PLACEMENT_SNAPSHOT/1 (SCH-01, signed, tenant-bound)",
            "handoff": "PK_DATA_MOVE_HANDOFF/1 -> PK_DATA_MOVE_ACK/1 (PLN-06, idempotent)",
            "explain": "PK_GRAVITY_EXPLAIN/1",
            "simulate": "PK_GRAVITY_SIMULATION/1 (never executable)",
            "health": "PK_HEALTH/1",
            "audit": "PK_AUDIT_RECORD/1 (hash-chained, MAC'd)",
            "config": "PK_GAP14_CONFIG/1 (signed, transactional)"
        },
        threats=[
        ...
        signals={
            ...
            "decision_latency": "histogram gap14_decision_latency_seconds by mode",
            "dependency_latency": "histogram by dependency and outcome",
            "refusals": "counter by reason code and category",
            "audit_records": "counter by result",
            "shadow_divergence": "counter by model",
            "calibration_drift": "counter by dimension"
        },
    )
