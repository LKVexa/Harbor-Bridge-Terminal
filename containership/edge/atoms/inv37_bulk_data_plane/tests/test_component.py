"""Optional pk_core conformance tests for INV-37."""
...
class AlwaysRunTest(unittest.TestCase):
    ...
    def test_optional_adapter_state_is_explicit(self):
        module = _load()
        if pk_core is None:
            self.assertIsNone(module.COMPONENT)
            with self.assertRaises(RuntimeError):
                module.build_contract()
        else:
            self.assertIsNotNone(module.COMPONENT)


@unittest.skipIf(pk_core is None, "pk_core not importable; governance conformance not executable")
