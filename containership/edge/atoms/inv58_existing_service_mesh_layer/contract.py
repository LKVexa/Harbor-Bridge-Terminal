...
def build() -> Contract:
    ...
    return Contract(
        ...
        interfaces={
            "reconcile": "PK_MESH_RECONCILE/1 - effective retry ownership per route - schema: schemas/PK_MESH_RECONCILE-1.schema.json",
            "identity": "PK_MESH_IDENTITY/1 - certificate SAN to runtime identity - schema: schemas/PK_MESH_IDENTITY-1.schema.json",
            "bypass": "PK_MESH_BYPASS/1 - a flow observed outside the mesh - schema: schemas/PK_MESH_BYPASS-1.schema.json"
        },
        threats=[
            "Retry storm from multiplied retries",
            "Identity lost at the layer boundary",
            "Workload quietly bypassing mTLS",
            "Cross-tenant policy or identity access (see governance/threat_model.json T-01..T-18)",
            "Control-plane abuse: forged/replayed operator tokens, stale controllers, malicious configuration"
        ],
        failure_modes=[
            "Budget exceeded",
            "Identity unmappable",
            ...
            "Trust dependency unavailable (fail closed)",
        ...
        signals={
            ...
            "authz_decisions": "counter by operation, decision code",
            "saturation_ratio": "gauge",
            "config_activations": "counter by result"
        },
    )
