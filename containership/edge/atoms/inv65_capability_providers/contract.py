...
def build() -> Contract:
    ...
    return Contract(
        ...
        mandatory=[
            ...
            "Keep configuration per named link",
            "Refuse a call whose named link is not established or has been revoked",
            "Report health honestly",
            "Restore every non-revoked link after restart"
        ],
        optional=[
        ...
        threats=[
            "One named link's configuration or credential reference used for another component/link",
            "Links lost on provider restart or revoked links resurrected",
            "Unhealthy provider reporting healthy"
        ],
        failure_modes=[
            "Link not found",
            "Backend unhealthy",
            "Config invalid or contains inline secret material",
            "Restart failed"
        ],
        slos=[
