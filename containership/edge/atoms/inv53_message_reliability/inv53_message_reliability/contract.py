...
def build() -> Contract:
    ...
    return Contract(
        ...
        owns=[
            "Acknowledgement protocol with lease fencing tokens",
            "Visibility timeout, extension and redelivery",
        ...
        mandatory=[
            "Keep every message until acknowledged by its active lease",
            "Reject acknowledgements for expired or superseded lease tokens",
        ...
        interfaces={
            "deliver": "PK_MSG_DELIVER/1 - a leased delivery with attempt, deadline and opaque lease token",
            "ack": "PK_MSG_ACK/1 - acknowledgement carrying the active lease fencing token",
            "nack": "PK_MSG_NACK/1 - negative acknowledgement with requeue or terminal disposition",
            "extend_visibility": "PK_MSG_EXTEND_VISIBILITY/1 - active-lease deadline extension",
        ...
        threats=[
            ...
            "Stale acknowledgement settling a newer redelivery",
            "Caller mutation corrupting broker-owned message state"
        ],
        failure_modes=[
            "Visibility timeout expired",
            "Attempts exhausted",
            "Duplicate delivery",
            "Ack for unknown or stale lease",
            "Dead-letter capacity exhausted",
            "Deduplication capacity exhausted"
        ],
        slos=[
