...
def build() -> Contract:
    ...
    return Contract(
        ...
        name=ELEMENT_NAME,
        responsibility=(
            "Own signal ingestion and attribution: accept only signals signed by an attested reporter, attribute each to its tenant, environment, site and workload, and expose staleness so absence is never mistaken for a healthy zero."
        ),
        owns=[
            "Signal ingestion and reporter authentication",
            "Tenant/environment/site/workload attribution",
        ...
        mandatory=[
            "Accept signals only through a verified attestation + signature trust adapter",
            "Attribute every signal to tenant, environment, site and workload",
        ...
        interfaces={
            ...
            "query": "PK_SIGNAL_QUERY/2 - tenant-scoped read with staleness",
            "catalogue": "PK_SIGNAL_CATALOGUE/1 - declared signals, their meaning and unit"
        },
        threats=[
            "A forged or over-scoped reporter injecting demand to drive scaling or billing",
        ...
        slos=[
            Slo("attribution", "zero signals stored without tenant, environment, site and workload", "no budget"),
