"""Tamper-evident security audit ledger (INV-26 C049, C036, C090).

Carried into INV-26 from the owner's INV-68 v4.3.0 ``audit.py`` (schema renamed
PK_SNAPSHOT_AUDIT/1); INV-68 notes follow.
...
* event schema ``PK_SNAPSHOT_AUDIT/1`` with ``event_id``, ``operation``,
...
* ``python -m inv26_microvm_snapshotting.audit verify LEDGER [--anchor A --key-env VAR]``
...
AUDIT_SCHEMA = "PK_SNAPSHOT_AUDIT/1"
GENESIS = "0" * 64
...
class AuditLog:
    ...
    def append(self, operation: str, *, actor: str, tenant: str | None = None, outcome: str,
        ...
        with self._lock:
            ...
            if self._unreported_drops and len(self._buffer) < self._buffer_limit:
                self._buffer.append(self._chain(self._body("audit.loss", "inv26", None, "recorded", None,
                                                           "sink outage", None,
            ...
            if len(self._buffer) >= self._buffer_limit:
                ...
                if self._metrics:
                    self._metrics.inc("inv26_audit_dropped_total")
                return None
            ...
            if self._metrics:
                self._metrics.set("inv26_audit_buffered", len(self._buffer))
            return rec
    ...
    def seal(self, anchor_path: str | os.PathLike, *, key: bytes) -> dict:
        with self._lock:
            self._flush()
            cp = {"schema": "PK_SNAPSHOT_AUDIT_ANCHOR/1", "seq": self._seq, "hash": self._prev,
                  "ts": round(float(self._clock()), 6)}
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(prog="inv26-audit")
    ...
    try:
        ...
        out = {"schema": "PK_SNAPSHOT_AUDIT_VERIFY/1", "ledger": a.ledger, "result": "PASS", "records": n,
               "head": list(log.head())}
        code = 0
    except (AuditChainBroken, OSError, ValueError, KeyError) as exc:
        out = {"schema": "PK_SNAPSHOT_AUDIT_VERIFY/1", "ledger": a.ledger, "result": "FAIL", "error": str(exc)}
        code = 2
