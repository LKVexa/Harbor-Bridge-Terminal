<!-- EXECUTED COPY generated from COMPONENTS_STATUS.json by the 6.0.0 chop-shop pass; the supplied original is source/INV-26_v5.0.0_Missing_Component_Engineering_Checklist.md -->
...
> **6.0.0 status: GOVERNANCE_PENDING** — artifacts: `ops/ownership.json`, `CODEOWNERS`, `tools/governance_check.py`, `GOVERNANCE.md`
> evidence: `evidence/GOVERNANCE.json`
> open: no person assigned to any role; no paging route; escalation drill not performed
...
- [x] Repository artifact(s) implementing or specifying C009, with stable links from the requirements traceability matrix.
...
- [x] Machine-readable gate record showing C009 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C009.

## INV-26-C010 — MISSING

> **6.0.0 status: GOVERNANCE_PENDING** — artifacts: `ADR-0001-microvm-snapshotting.md`
> open: ADR status PROPOSED: architecture/security/operations approval pending; comparison is qualitative; no measured VMM comparison (no KVM)
**Priority:** P2 - governance / optimization completion  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C010, with stable links from the requirements traceability matrix.
...
- [x] Machine-readable gate record showing C010 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
...
> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `policy.py`, `ops/policy.json`, `APPLICABILITY.md`
> tests: `tests/test_service.py::Precedence.test_tier_applicability`
> evidence: `evidence/TESTS.json`
> open: tier targets PROPOSED; TPM/attestation per tier specified but not enforced (C044)
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C012, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C012, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C012 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C012.

## INV-26-C014 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `errors.py`, `ERRORS.json`, `SEMANTICS.md`, `service.py`
> tests: `tests/test_service.py::Outcomes`, `tests/test_service.py::OperatorControls`
> evidence: `evidence/TESTS.json`, `evidence/FAULTS.json`
> open: degraded_success/partial_success are reserved, never emitted
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C014, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C014, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C014 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C014.

## INV-26-C015 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `lifecycle.py`, `ops/lifecycle.json`, `SEMANTICS.md`
> tests: `tests/test_security_units.py::LifecycleTests`, `tools/faults.py crash_matrix`
> evidence: `evidence/FAULTS.json`
> open: distributed multi-controller ownership limited to lease/fencing on a shared-filesystem metastore
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C015, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C015, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C015 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C015.

## INV-26-C016 — MISSING

> **6.0.0 status: PARTIAL** — artifacts: `compatibility.json`, `COMPATIBILITY.md`, `schema.py`
> tests: `tests/test_service.py::Boundaries.test_unknown_fields_and_versions_rejected`, `tests/test_security_units.py::SchemaTests.test_conformance_fixtures`
> evidence: `evidence/TESTS.json`
> open: no N+1 fixtures; no CI job diffing schemas against the previous release (only one release exists)
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C016, with stable links from the requirements traceability matrix.
...
- [x] Machine-readable gate record showing C016 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C016.

## INV-26-C017 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `config.py (quotas, admission)`, `resilience.py`, `service.gc`, `CONFIGURATION.md`
> tests: `tests/test_service.py::Admission`, `tests/test_service.py::CaptureSemantics.test_quota_rejects_before_hypervisor_work`, `tests/test_service.py::MetadataGC`, `tests/test_concurrency.py::ThreadRaces.test_admission_ceiling_holds`
> evidence: `evidence/BENCH.json`, `evidence/SOAK.json`
> open: weighted fair queueing not implemented (per-tenant slot cap + token bucket instead); KMS request quota is implicit via admission
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C017, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C017, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C017 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C017.

## INV-26-C018 — MISSING

> **6.0.0 status: PARTIAL** — artifacts: `OFFLINE.md`, `policy.OUTAGE`, `service.reconcile`
> tests: `tools/faults.py dependency_matrix`
> evidence: `evidence/FAULTS.json`
> open: no local KMS replica for edge; no multi-node partition/reconnect test; no clock-discontinuity test beyond skew checks
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C018, with stable links from the requirements traceability matrix.
...
- [x] Machine-readable gate record showing C018 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C018.

## INV-26-C019 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `policy.PRECEDENCE/CONFLICTS`, `APPLICABILITY.md`, `explain.py`
> tests: `tests/test_service.py::Precedence`
> evidence: `evidence/TESTS.json`
> open: policy changes are versioned by POLICY_VERSION in code, not a separately signed bundle
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C019, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C019, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C019 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C019.

## INV-26-C020 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `tools/rtm.py`, `tools/status_data.py`, `REQUIREMENTS_TRACEABILITY.md`, `evidence/RTM.json`
> tests: `tools/rtm.py --check (in CI)`
> evidence: `evidence/RTM.json`
> open: RTM hash is in the signed SHA256SUMS; signer is ephemeral
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C020, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C020, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C020 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
...
> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `ops/boundaries.json`, `INTERFACES.md`
> tests: `tools/rtm.py boundary check`
> evidence: `evidence/RTM.json`
> open: diagram is hand-maintained Mermaid (CI checks boundary ids referenced, not the drawing)
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C021, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C021, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C021 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C021.

## INV-26-C022 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `schema.py`, `schemas/*.schema.json`, `schema.canonical_bytes`
> tests: `tests/test_security_units.py::SchemaTests`, `tests/test_fuzz.py`
> evidence: `evidence/SCHEMAS.json`, `evidence/FUZZ.json`
> open: no generated typed bindings for other languages
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C022, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C022, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C022 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C022.

## INV-26-C023 — MISSING

> **6.0.0 status: PARTIAL** — artifacts: `auth.py`, `SECURITY.md`
> tests: `tests/test_security_units.py::AuthTests`
> evidence: `evidence/TESTS.json`
> open: no mTLS/SPIFFE transport identity; no automated rotation/distribution of trust roots; local-socket peer-credential checks are the deployer's
**Priority:** P0 - production blocker  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C023, with stable links from the requirements traceability matrix.
...
- [x] Machine-readable gate record showing C023 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C023.

## INV-26-C024 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `auth.authorize`, `service._preamble`
> tests: `tests/test_security_units.py::AuthTests.test_authorization_matrix`, `tests/test_service.py::Boundaries.test_caller_scope_cannot_be_bypassed_by_request_fields`, `tests/test_service.py::OperatorControls`
> evidence: `evidence/TESTS.json`
**Priority:** P0 - production blocker  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C024, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C024, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C024 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C024.

## INV-26-C025 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `resilience.py`, `service.py`, `INTERFACES.md`
> tests: `tests/test_service.py::Outcomes.test_deadline_exceeded`, `tests/test_service.py::Outcomes.test_cancel`, `tests/test_service.py::AntiReplay`, `tests/test_concurrency.py`
> evidence: `evidence/TESTS.json`, `evidence/FAULTS.json`
> open: cancellation cannot interrupt a blocking VMM/KMS call already in flight (checked between steps)
**Priority:** P0 - production blocker  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C025, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C025, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C025 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C025.

## INV-26-C026 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `errors.py`, `ERRORS.json`
> tests: `tests/test_service.py::Outcomes.test_every_catalog_code_has_valid_envelope`, `tests/test_service.py::Outcomes.test_unexpected_exception_does_not_leak`, `tests/test_security_units.py::ErrorCatalogTests`
> evidence: `evidence/TESTS.json`
**Priority:** P0 - production blocker  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C026, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C026, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C026 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C026.

## INV-26-C027 — MISSING

> **6.0.0 status: PARTIAL** — artifacts: `compatibility.json`, `COMPATIBILITY.md`
> tests: `tests/test_security_units.py::SchemaTests.test_conformance_fixtures`
> open: no mixed-version rolling-upgrade/rollback test (single release); no runtime feature negotiation beyond major refusal
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C027, with stable links from the requirements traceability matrix.
...
- [x] Machine-readable gate record showing C027 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C027.

## INV-26-C028 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `schema.py limits`, `auth.MAX_TOKEN_BYTES`, `INTERFACES.md#limits`
> tests: `tests/test_security_units.py::SchemaTests.test_bounded_parse`, `tests/test_service.py::CaptureSemantics.test_limits`, `tests/test_fuzz.py`
> evidence: `evidence/FUZZ.json`
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C028, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C028, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C028 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C028.

## INV-26-C029 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `examples/`, `tests/fixtures/conformance/cases.json`, `tools/gen_artifacts.py`
> tests: `tests/test_security_units.py::SchemaTests.test_conformance_fixtures`
> evidence: `evidence/SCHEMAS.json`
> open: no downstream adapter consumes the fixtures in CI
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C029, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C029, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C029 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C029.

## INV-26-C030 — MISSING

> **6.0.0 status: BLOCKED_EXTERNAL** — artifacts: `tests/test_adapters.py`, `tests/fakes.py`
> tests: `tests/test_adapters.py`
> evidence: `evidence/INTEGRATION.json`
> open: only protocol fakes on real Unix sockets; no real VMM/KMS/object store; INV-24/INV-25 contracts not available
**Priority:** P0 - production blocker  
...
- [x] Machine-readable gate record showing C030 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
...
> **6.0.0 status: PARTIAL** — artifacts: `compatibility.json`, `requirements.lock`, `evidence/sbom.cdx.json`
> tests: `tools/release.py`
> evidence: `evidence/RELEASE.json`
> open: VMM/guest-kernel/KMS versions are not pinned to digests (not present); pk_core unpinned (unknown)
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C031, with stable links from the requirements traceability matrix.
...
- [x] Machine-readable gate record showing C031 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C031.

## INV-26-C032 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `CONFIGURATION.md#artifacts-vs-state`, `storage.py`, `metastore.py`
> tests: `tools/release.py install check (read-only package)`
> evidence: `evidence/INSTALL.json`
> open: read-only root filesystem run not performed
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C032, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C032, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C032 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C032.

## INV-26-C033 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `config.py`, `examples/config.production.json`, `examples/config.restrictive.json`, `CONFIGURATION.md`
> tests: `tests/test_security_units.py::ConfigTests`
> evidence: `evidence/TESTS.json`
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C033, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C033, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C033 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C033.

## INV-26-C034 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `config.validate`, `ConfigStore.activate`
> tests: `tests/test_security_units.py::ConfigTests`, `tests/test_fuzz.py`
> evidence: `evidence/FUZZ.json`
**Priority:** P0 - production blocker  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C034, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C034, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C034 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C034.

## INV-26-C035 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `config.compose`
> tests: `tests/test_security_units.py::ConfigTests.test_overlay_order_and_emergency_scope`
> evidence: `evidence/TESTS.json`
> open: artifact identity across environments is shown by one wheel digest; no multi-environment deployment exists
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C035, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C035, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C035 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C035.

## INV-26-C036 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `ConfigStore (provenance, history, governing)`, `audit.py`
> tests: `tests/test_security_units.py::ConfigTests.test_atomic_activation_provenance_rollback`
> evidence: `evidence/DRILLS.json`
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C036, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C036, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C036 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C036.

## INV-26-C037 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `ConfigStore.activate`, `metastore.py (WAL, CAS)`
> tests: `tests/test_security_units.py::ConfigTests.test_crash_during_activation_is_all_or_nothing`
> evidence: `evidence/TESTS.json`
> open: multi-node barrier semantics specified only (single-node metastore)
**Priority:** P0 - production blocker  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C037, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C037, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C037 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C037.

## INV-26-C038 — MISSING

> **6.0.0 status: PARTIAL** — artifacts: `ConfigStore.rollback`, `ops/ROLLOUT_POLICY.json`
> tests: `tools/drills.py rollback`
> evidence: `evidence/DRILLS.json`
> open: software rollback between two releases not drilled (one release); canary/partial-fleet drill needs a fleet
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C038, with stable links from the requirements traceability matrix.
...
- [x] Machine-readable gate record showing C038 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C038.

## INV-26-C039 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `redaction.py`, `config.validate`, `errors.envelope`, `DATA_LIFECYCLE.md`, `tools/secret_scan.py`
> tests: `tests/test_service.py::Observability.test_no_secret_material_in_logs_audit_or_responses`, `tests/test_security_units.py::ConfigTests.test_negative_values`
> evidence: `evidence/SECRET_SCAN.json`
> open: no secret-provider adapter (only secretref:// references); CPython cannot zeroize immutable bytes
**Priority:** P0 - production blocker  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C039, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C039, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C039 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C039.

## INV-26-C040 — MISSING

> **6.0.0 status: PARTIAL** — artifacts: `tools/bootstrap.sh`, `tools/preflight.py`, `tools/smoke.py`, `RUNBOOK.md#day-0`
> tests: `CI bootstrap-smoke`
> evidence: `evidence/PREFLIGHT.json`, `evidence/SMOKE.json`
> open: clean-node run with KVM + Firecracker + encrypted capture/restore not performed
**Priority:** P0 - production blocker  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C040, with stable links from the requirements traceability matrix.
...
- [x] Machine-readable gate record showing C040 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
...
> **6.0.0 status: GOVERNANCE_PENDING** — artifacts: `THREAT_MODEL.md`
> tests: `tests/test_threats.py`
> evidence: `evidence/TESTS.json`
> open: security-owner review pending
**Priority:** P0 - production blocker  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C041, with stable links from the requirements traceability matrix.
...
- [x] Machine-readable gate record showing C041 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C041.

## INV-26-C042 — MISSING

> **6.0.0 status: PARTIAL** — artifacts: `ops/least_privilege.json`, `SECURITY.md`
> tests: `tests/test_service.py::Boundaries`
> open: seccomp/capabilities/IAM enforcement is the deployer's; compromised-worker negative tests need the deployment
**Priority:** P0 - production blocker  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C042, with stable links from the requirements traceability matrix.
...
- [x] Machine-readable gate record showing C042 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C042.

## INV-26-C043 — MISSING

> **6.0.0 status: PARTIAL** — artifacts: `hypervisor.HypervisorPort (brokered)`, `ops/boundaries.json B12`, `SECURITY.md`
> open: no sandbox/policy-denial test harness (needs deployment sandbox)
**Priority:** P0 - production blocker  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C043, with stable links from the requirements traceability matrix.
...
- [x] Machine-readable gate record showing C043 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C043.

## INV-26-C044 — MISSING

> **6.0.0 status: PARTIAL** — artifacts: `auth.TrustStore (role, trust domain, revocation)`, `grant audience=node`
> tests: `tests/test_security_units.py::AuthTests.test_revocation_and_trust_domain_and_role`
> evidence: `evidence/TESTS.json`
> open: no hardware attestation; no out-of-band trust-anchor verification tooling; storage/KMS provider authentication is the adapters'
**Priority:** P0 - production blocker  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C044, with stable links from the requirements traceability matrix.
...
- [x] Machine-readable gate record showing C044 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C044.

## INV-26-C045 — MISSING

> **6.0.0 status: PARTIAL** — artifacts: `provenance.py`, `tools/release.py`
> tests: `tools/release.py verify (tamper test)`
> evidence: `evidence/RELEASE.json`
> open: ephemeral signer (no publisher identity); consumed-artifact allowlist of builder identities not enforced
**Priority:** P0 - production blocker  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C045, with stable links from the requirements traceability matrix.
...
- [x] Machine-readable gate record showing C045 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C045.

## INV-26-C047 — MISSING

> **6.0.0 status: PARTIAL** — artifacts: `crypto.py`, `service.rewrap_all`
> tests: `tests/test_security_units.py::CryptoTests`, `tests/test_service.py::HappyPath.test_blob_at_rest_is_ciphertext`
> evidence: `evidence/DRILLS.json`
> open: LocalKeyService is not a managed KMS; in-transit TLS belongs to remote adapters (none)
**Priority:** P0 - production blocker  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C047, with stable links from the requirements traceability matrix.
...
- [x] Machine-readable gate record showing C047 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C047.

## INV-26-C048 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `policy.OUTAGE`, `OFFLINE.md`, `telemetry.Health`
> tests: `tools/faults.py dependency_matrix`
> evidence: `evidence/FAULTS.json`
> open: attestation/time services modelled but not present
**Priority:** P0 - production blocker  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C048, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C048, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C048 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C048.

## INV-26-C049 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `audit.py (hash chain, HMAC, anchor)`, `service.py (audit events)`
> tests: `tests/test_service.py::Observability.test_audit_chain_verifies_and_detects_tamper`, `tests/test_service.py::OperatorControls.test_privileged_ops_fail_closed_without_audit`
> evidence: `evidence/AUDIT_VERIFY.json`
> open: anchor must be copied off-node (WORM/audit service not included)
**Priority:** P0 - production blocker  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C049, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C049, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C049 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C049.

## INV-26-C050 — MISSING

> **6.0.0 status: PARTIAL** — artifacts: `tests/test_threats.py`, `tools/fuzz.py`
> tests: `tests/*`
> evidence: `evidence/FUZZ.json`
> open: no side-channel, escape or local privilege-escalation testing (needs real VMM/host)
**Priority:** P0 - production blocker  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C050, with stable links from the requirements traceability matrix.
...
- [x] Machine-readable gate record showing C050 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
...
> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `FAILURE_MODEL.md`
> tests: `tools/faults.py`
> evidence: `evidence/FAULTS.json`
> open: node/site loss rows state 'no failover' (C055)
**Priority:** P0 - production blocker  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C051, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C051, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C051 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C051.

## INV-26-C052 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `telemetry.Health`, `service.stalled`, `lifecycle.STATE_TIMEOUT_S`, `ops/alerts.json`
> tests: `tests/test_service.py::Observability.test_health_metrics_logs_explain`
> evidence: `evidence/DRILLS.json`
**Priority:** P0 - production blocker  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C052, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C052, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C052 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C052.

## INV-26-C053 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `resilience.retry`, `RetryBudget`
> tests: `tools/faults.py breaker`, `tests/test_service.py::Outcomes.test_hypervisor_failure_on_load`
> evidence: `evidence/FAULTS.json`
**Priority:** P0 - production blocker  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C053, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C053, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C053 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C053.

## INV-26-C054 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `resilience.Admission`, `CircuitBreaker`
> tests: `tests/test_service.py::Admission`, `tests/test_concurrency.py::ThreadRaces.test_admission_ceiling_holds`
> evidence: `evidence/FAULTS.json`, `evidence/BENCH.json`
**Priority:** P0 - production blocker  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C054, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C054, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C054 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C054.

## INV-26-C055 — MISSING

> **6.0.0 status: BLOCKED_EXTERNAL** — artifacts: `FAILURE_MODEL.md F15/F16`, `APPLICABILITY.md (replication prohibited)`
> open: no failover: node-local storage adapter only; cross-node restore needs shared storage + residency approval
**Priority:** P0 - production blocker  
...
- [x] Machine-readable gate record showing C055 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C055.

## INV-26-C056 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `policy.OUTAGE (telemetry/audit degrade)`, `telemetry.Health degraded`
> tests: `tools/faults.py audit_sink_down`
> evidence: `evidence/FAULTS.json`
**Priority:** P0 - production blocker  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C056, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C056, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C056 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C056.

## INV-26-C057 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `metastore.py`, `service.reconcile`, `lifecycle.recovery_action`
> tests: `tools/faults.py crash_matrix`, `tests/test_service.py::Recovery`
> evidence: `evidence/FAULTS.json`
**Priority:** P0 - production blocker  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C057, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C057, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C057 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C057.

## INV-26-C058 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `metastore leases/fencing`, `grant CAS`
> tests: `tests/test_concurrency.py::ProcessRaces`, `tests/test_concurrency.py::ThreadRaces`
> evidence: `evidence/STRESS.json`
> open: fencing is enforced on metastore writes; VMM-side fencing is INV-24's
**Priority:** P0 - production blocker  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C058, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C058, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C058 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C058.

## INV-26-C059 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `service quarantine/release/disable/scrub`
> tests: `tests/test_service.py::OperatorControls`, `tests/test_service.py::Scrub`
> evidence: `evidence/DRILLS.json`
**Priority:** P0 - production blocker  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C059, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C059, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C059 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C059.

## INV-26-C060 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `tools/faults.py`
> tests: `tools/faults.py`
> evidence: `evidence/FAULTS.json`
> open: real-VMM fault injection pending (X001)
**Priority:** P0 - production blocker  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C060, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C060, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C060 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
...
> **6.0.0 status: PARTIAL** — artifacts: `tools/bench.py`, `BENCHMARKS.md`
> tests: `tools/bench.py`
> evidence: `evidence/BENCH.json`
> open: reference VMM only: VMM cost, network and power excluded
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C061, with stable links from the requirements traceability matrix.
...
- [x] Machine-readable gate record showing C061 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C061.

## INV-26-C062 — MISSING

> **6.0.0 status: PARTIAL** — artifacts: `BENCHMARKS.md (p50/p95/p99/max)`, `SLO.md`
> evidence: `evidence/BENCH.json`
> open: thresholds PROPOSED, not approved; no real-VMM numbers
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C062, with stable links from the requirements traceability matrix.
...
- [x] Machine-readable gate record showing C062 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C062.

## INV-26-C063 — MISSING

> **6.0.0 status: PARTIAL** — artifacts: `tools/bench.py load scenarios`, `tools/soak.py`
> evidence: `evidence/BENCH.json`, `evidence/SOAK.json`
> open: no scale-out/scale-in (single node); overload measured via admission only
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C063, with stable links from the requirements traceability matrix.
...
- [x] Machine-readable gate record showing C063 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C063.

## INV-26-C064 — MISSING

> **6.0.0 status: PARTIAL** — artifacts: `tools/bench.py breakdown`, `metrics tenant labels`
> evidence: `evidence/BENCH.json`
> open: per-tenant CPU/memory accounting not exported
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C064, with stable links from the requirements traceability matrix.
...
- [x] Machine-readable gate record showing C064 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C064.

## INV-26-C065 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `BENCHMARKS.md#copies`, `tools/bench.py breakdown`
> evidence: `evidence/BENCH.json`
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C065, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C065, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C065 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C065.

## INV-26-C066 — MISSING

> **6.0.0 status: PARTIAL** — artifacts: `crypto.open_envelope (digest off hot path, memoryview)`
> tests: `tests/test_security_units.py::CryptoTests`
> evidence: `evidence/BENCH.json`, `evidence/BENCH_PRE_OPT.json`
> open: streaming decrypt into VMM memory file / UFFD not implemented (DEBT-1)
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C066, with stable links from the requirements traceability matrix.
...
- [x] Machine-readable gate record showing C066 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C066.

## INV-26-C067 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `INTERFACES.md#limits`, `service.gc`, `Admission tenant table cap`, `telemetry series cap`
> tests: `tools/soak.py`, `tests/test_service.py::MetadataGC`
> evidence: `evidence/SOAK.json`
> open: plaintext image held fully in memory (DEBT-1)
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C067, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C067, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C067 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C067.

## INV-26-C068 — MISSING

> **6.0.0 status: BLOCKED_EXTERNAL** — artifacts: 
> open: no constrained edge hardware or power/thermal instrumentation available
**Priority:** P1 - required before broad production rollout  
...
- [x] Machine-readable gate record showing C068 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C068.

## INV-26-C069 — MISSING

> **6.0.0 status: PARTIAL** — artifacts: `BENCHMARKS.md#capacity`
> evidence: `evidence/BENCH.json`
> open: capacity model derived from reference-path throughput only
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C069, with stable links from the requirements traceability matrix.
...
- [x] Machine-readable gate record showing C069 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C069.

## INV-26-C070 — MISSING

> **6.0.0 status: PARTIAL** — artifacts: `tools/bench.py --compare`, `bench/baseline.json`
> tests: `CI perf-gate`
> evidence: `evidence/PERF_GATE.json`
> open: gates INV-26 overhead only; startup/density/tail thresholds on real VMM unapproved
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C070, with stable links from the requirements traceability matrix.
...
- [x] Machine-readable gate record showing C070 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
...
> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `telemetry.Health`, `service.health`
> tests: `tests/test_service.py::Observability`, `tests/test_service.py::OperatorControls.test_emergency_disable`
> evidence: `evidence/TESTS.json`
> open: no HTTP endpoint (in-process API)
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C071, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C071, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C071 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C071.

## INV-26-C072 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `telemetry.Metrics`, `ops/metrics.json`
> tests: `tests/test_service.py::Observability.test_health_metrics_logs_explain`
> evidence: `evidence/TESTS.json`
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C072, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C072, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C072 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C072.

## INV-26-C073 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `telemetry.StructuredLogger`
> tests: `tests/test_service.py::Observability.test_health_metrics_logs_explain`
> evidence: `evidence/TESTS.json`
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C073, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C073, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C073 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C073.

## INV-26-C074 — MISSING

> **6.0.0 status: PARTIAL** — artifacts: `telemetry traceparent`, `service.handle`
> tests: `tests/test_service.py::Observability.test_trace_context_continues`
> evidence: `evidence/TESTS.json`
> open: cannot propagate into VMM/KMS APIs; no trace exporter
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C074, with stable links from the requirements traceability matrix.
...
- [x] Machine-readable gate record showing C074 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C074.

## INV-26-C075 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `telemetry label policy`, `explain decision records`
> tests: `tests/test_service.py::Observability.test_lineage_is_allowlisted_and_redacted`
> evidence: `evidence/TESTS.json`
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C075, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C075, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C075 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C075.

## INV-26-C076 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `explain.DecisionRecorder`, `audit decision.* events`
> tests: `tests/test_service.py::Observability.test_health_metrics_logs_explain`
> evidence: `evidence/TESTS.json`
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C076, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C076, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C076 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C076.

## INV-26-C077 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `service.explain`, `explain.render_text`
> tests: `tests/test_service.py::Observability.test_health_metrics_logs_explain`
> evidence: `evidence/TESTS.json`
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C077, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C077, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C077 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C077.

## INV-26-C078 — MISSING

> **6.0.0 status: PARTIAL** — artifacts: `explain.lineage`
> tests: `tests/test_service.py::Observability.test_lineage_is_allowlisted_and_redacted`
> open: no live infrastructure graph to correlate with
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C078, with stable links from the requirements traceability matrix.
...
- [x] Machine-readable gate record showing C078 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C078.

## INV-26-C079 — MISSING

> **6.0.0 status: GOVERNANCE_PENDING** — artifacts: `OBSERVABILITY.md#telemetry-policy`
> open: retention values proposed; privacy/legal approval pending
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C079, with stable links from the requirements traceability matrix.
...
- [x] Machine-readable gate record showing C079 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C079.

## INV-26-C080 — MISSING

> **6.0.0 status: PARTIAL** — artifacts: `ops/alerts.json`, `dashboards/inv26-overview.json`
> tests: `tools/drills.py alerts`
> evidence: `evidence/DRILLS.json`
> open: dashboards not deployed; alert routing unassigned
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C080, with stable links from the requirements traceability matrix.
...
- [x] Machine-readable gate record showing C080 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
...
> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `tests/test_service.py`, `tests/fixtures/conformance`
> tests: `tests/test_service.py`, `tests/test_security_units.py::SchemaTests`
> evidence: `evidence/TESTS.json`
**Priority:** P0 - production blocker  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C082, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C082, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C082 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C082.

## INV-26-C083 — MISSING

> **6.0.0 status: BLOCKED_EXTERNAL** — artifacts: `tests/test_adapters.py`
> tests: `tests/test_adapters.py`
> evidence: `evidence/INTEGRATION.json`
> open: adjacent layers (INV-24, INV-25, KMS, storage) not available; protocol fakes only
**Priority:** P0 - production blocker  
...
- [x] Machine-readable gate record showing C083 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C083.

## INV-26-C084 — MISSING

> **6.0.0 status: BLOCKED_EXTERNAL** — artifacts: `compatibility.json tested_matrix`
> open: one OS/arch/Python; no aarch64, no real VMM versions
**Priority:** P0 - production blocker  
...
- [x] Machine-readable gate record showing C084 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C084.

## INV-26-C085 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `tools/fuzz.py`
> tests: `tests/test_fuzz.py`
> evidence: `evidence/FUZZ.json`
> open: mutation fuzzer (stdlib), not coverage-guided
**Priority:** P0 - production blocker  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C085, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C085, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C085 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
...
> **6.0.0 status: PARTIAL** — artifacts: `tests/test_concurrency.py`
> tests: `tests/test_concurrency.py`
> evidence: `evidence/STRESS.json`
> open: thread + multi-process races covered; distributed multi-node races need a cluster
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C086, with stable links from the requirements traceability matrix.
...
- [x] Machine-readable gate record showing C086 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C086.

## INV-26-C087 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `THREAT_MODEL.md`, `tests/test_threats.py`
> tests: `tests/test_threats.py`
> evidence: `evidence/TESTS.json`
> open: T18 (VMM escape/side channels) explicitly out of scope
**Priority:** P0 - production blocker  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C087, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C087, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C087 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C087.

## INV-26-C088 — MISSING

> **6.0.0 status: PARTIAL** — artifacts: `tools/bench.py`, `tools/soak.py`
> evidence: `evidence/BENCH.json`, `evidence/SOAK.json`
> open: soak < 1 h, reference VMM, single node (no fleet-scale)
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C088, with stable links from the requirements traceability matrix.
...
- [x] Machine-readable gate record showing C088 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C088.

## INV-26-C089 — MISSING

> **6.0.0 status: PARTIAL** — artifacts: `tools/faults.py`, `tools/drills.py backup_restore`
> evidence: `evidence/FAULTS.json`, `evidence/DRILLS.json`
> open: no multi-node partition/reconnect or control-plane degradation test
**Priority:** P0 - production blocker  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C089, with stable links from the requirements traceability matrix.
...
- [x] Machine-readable gate record showing C089 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C089.

## INV-26-C090 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `tools/gate.py`, `tools/run_evidence.py`
> tests: `tools/gate.py self-test (tampered evidence)`
> evidence: `evidence/EXIT_GATE.json`
> open: pk_core gate cannot execute (X013)
**Priority:** P0 - production blocker  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C090, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C090, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C090 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
...
> **6.0.0 status: GOVERNANCE_PENDING** — artifacts: `SLO.md`
> open: SLOs proposed; restore-time SLO unmeasured on real VMM and not supportable above ~4 MiB as specified
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C091, with stable links from the requirements traceability matrix.
...
- [x] Machine-readable gate record showing C091 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C091.

## INV-26-C092 — MISSING

> **6.0.0 status: PARTIAL** — artifacts: `ops/ROLLOUT_POLICY.json`, `RUNBOOK.md#day-1`
> tests: `tools/drills.py emergency`
> evidence: `evidence/DRILLS.json`
> open: canary/staged rollout not exercised (no fleet)
**Priority:** P0 - production blocker  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C092, with stable links from the requirements traceability matrix.
...
- [x] Machine-readable gate record showing C092 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C092.

## INV-26-C093 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `compatibility.json`, `COMPATIBILITY.md`
> tests: `tools/rtm.py compat check`
> evidence: `evidence/RTM.json`
> open: peer ranges untested on real peers
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C093, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C093, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C093 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C093.

## INV-26-C094 — MISSING

> **6.0.0 status: GOVERNANCE_PENDING** — artifacts: `SECURITY_RESPONSE.md`
> open: SLAs proposed; no reporting channel or security owner
**Priority:** P0 - production blocker  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C094, with stable links from the requirements traceability matrix.
...
- [x] Machine-readable gate record showing C094 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C094.

## INV-26-C095 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `BACKUP_RESTORE.md`, `MetaStore.export/import_backup`, `service.scrub`
> tests: `tools/drills.py backup_restore`, `tests/test_service.py::Scrub`
> evidence: `evidence/DRILLS.json`
**Priority:** P0 - production blocker  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C095, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C095, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C095 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C095.

## INV-26-C096 — MISSING

> **6.0.0 status: PARTIAL** — artifacts: `RUNBOOK.md`
> tests: `tools/smoke.py`
> evidence: `evidence/SMOKE.json`
> open: not exercised by an operator; production day-0 needs KVM
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C096, with stable links from the requirements traceability matrix.
...
- [x] Machine-readable gate record showing C096 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C096.

## INV-26-C097 — MISSING

> **6.0.0 status: GOVERNANCE_PENDING** — artifacts: `INCIDENT_RESPONSE.md`, `ops/ownership.json escalation`
> open: paging destinations unassigned; no drill
**Priority:** P0 - production blocker  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C097, with stable links from the requirements traceability matrix.
...
- [x] Machine-readable gate record showing C097 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C097.

## INV-26-C098 — MISSING

> **6.0.0 status: GOVERNANCE_PENDING** — artifacts: `ops/REVIEWS.json`
> tests: `tools/governance_check.py`
> evidence: `evidence/GOVERNANCE.json`
> open: no review performed
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C098, with stable links from the requirements traceability matrix.
...
- [x] Machine-readable gate record showing C098 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C098.

## INV-26-C099 — MISSING

> **6.0.0 status: GOVERNANCE_PENDING** — artifacts: `ops/REGISTER.json`
> tests: `tools/governance_check.py`
> evidence: `evidence/GOVERNANCE.json`
> open: waivers lack approver/expiry
**Priority:** P1 - required before broad production rollout  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C099, with stable links from the requirements traceability matrix.
...
- [x] Machine-readable gate record showing C099 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
- [ ] Owner/reviewer sign-off and any operational/runbook update required by C099.

## INV-26-C100 — MISSING

> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `tools/gate.py`, `GOVERNANCE.md`
> tests: `tools/gate.py`
> evidence: `evidence/EXIT_GATE.json`
> open: gate verdict is NO_GO by construction until blockers close
**Priority:** P0 - production blocker  
...
### Required evidence / acceptance package
- [x] Repository artifact(s) implementing or specifying C100, with stable links from the requirements traceability matrix.
- [x] Automated test evidence for C100, including negative/failure-path coverage where applicable, tied to the exact build digest.
- [x] Machine-readable gate record showing C100 as satisfied (or, for C086 until complete, explicitly partial) with no unexplained skipped tests. *(recorded in evidence/EXIT_GATE.json with its true status)*
...
> **6.0.0 status: BLOCKED_EXTERNAL** — artifacts: `hypervisor.FirecrackerAdapter`, `hypervisor.CloudHypervisorAdapter`, `tests/fakes.py`, `tests/test_adapters.py`
> open: never run against a real VMM (no /dev/kvm); CPU-feature/page-size compatibility checks rely on the VMM refusing
...
> **6.0.0 status: PARTIAL** — artifacts: `hypervisor.VsockAgentInjector (proof-of-receipt)`, `service: READY only after ack`
> open: in-guest agent not in repo (DEBT-2); VMGenID not driven
...
> **6.0.0 status: PARTIAL** — artifacts: `storage.BlobStore port`, `storage.FilesystemBlobStore (atomic, fsync, 0600, quota)`
> open: object-store adapter not implemented
...
> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `pyproject.toml`, `requirements.lock`, `tools/release.py (wheel, sdist, SBOM, SHA256SUMS, DSSE)`
> open: build not bit-reproducible across hosts (timestamps normalised via SOURCE_DATE_EPOCH only for sdist)
...
> **6.0.0 status: BLOCKED_EXTERNAL** — artifacts: `provenance.py`, `tools/release.py verify`
> open: no managed signer; ephemeral key
...
> **6.0.0 status: PARTIAL** — artifacts: `crypto.KeyService port`, `crypto.LocalKeyService (versions, rotate, rewrap, disable, destroy)`
> open: no cloud KMS/HSM adapter (WAIVER-2)
...
> **6.0.0 status: BLOCKED_EXTERNAL** — artifacts: 
> open: no attestation provider; trust-domain binding only
...
> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `auth.verify_grant`, `service grant CAS`, `gc keeps nonces until expiry`
> open: grant issuer (control plane) not in repo; tests issue grants with a test key
...
> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `crypto.seal_envelope/open_envelope`
> open: temp plaintext relies on tmpfs (deployer)
...
> **6.0.0 status: PARTIAL** — artifacts: `DATA_LIFECYCLE.md`, `delete crypto-erase`, `hypervisor.wipe`, `storage.delete overwrite`
> open: forensic canary scan of swap/page cache not possible here
...
> **6.0.0 status: IMPLEMENTED_LOCAL** — artifacts: `metastore.py (WAL, CAS, leases, export/import)`, `service.reconcile`
> open: shared-filesystem only; no replicated store
...
> **6.0.0 status: PARTIAL** — artifacts: `telemetry.py`, `ops/alerts.json`, `dashboards/`
> open: no HTTP endpoint/exporter process; dashboards not deployed
...
> **6.0.0 status: BLOCKED_EXTERNAL** — artifacts: `tests/test_component.py (skips loudly)`, `tools/gate.py treats skips as blockers`
> open: pk_core distribution/version/digest unknown; not in the upload or on the lot
...
> **6.0.0 status: BLOCKED_EXTERNAL** — artifacts: `tools/bench.py (certifiable=false)`, `tools/soak.py`
> open: real hypervisor/storage/KMS benchmark impossible here
