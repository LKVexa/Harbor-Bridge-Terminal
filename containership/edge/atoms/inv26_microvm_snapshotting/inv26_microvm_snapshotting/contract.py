...
def build() -> Contract:
    ...
    return Contract(
        ...
        owns=[
            ...
            "Workload and environment binding of snapshot material",
            "Device-model compatibility checking on restore",
            "Entropy re-seeding after restore",
            "Restore-time budget and its measurement",
            "Snapshot confidentiality envelope and DEK lifecycle",
            "One-shot restore grant consumption (anti-replay)",
            "Snapshot metadata, lifecycle state and crash recovery"
        ],
        not_owns=[
            "MicroVM lifecycle",
            "Device definitions",
        ...
        dependencies=[
            ...
            Dependency("Key management service", "upstream", "Wraps per-snapshot DEKs; restores fail closed without it"),
            Dependency("Snapshot blob storage", "upstream", "Holds ciphertext blobs behind the BlobStore port")
        ],
        ...
        mandatory=[
            "Bind every snapshot to tenant, workload, and environment at capture",
            "Refuse a restore into a different tenant, workload, or environment",
            "Refuse a restore when the device model fingerprint differs",
            "Re-seed guest entropy on every restore",
            "Emit a non-secret proof that fresh entropy material was generated",
        ...
        interfaces={
            "capture": "PK_SNAPSHOT/2 - capture an instance with tenant, workload, environment, and model fingerprint",
            "restore": "PK_SNAPSHOT_RESTORE/2 - restore result with boundary bindings, elapsed time, and entropy proof",
            "restore_request": "PK_SNAPSHOT_RESTORE_REQUEST/2 - restore request carrying a one-shot EdDSA restore grant",
            "manifest": "PK_SNAPSHOT_MANIFEST/1 - encrypted snapshot manifest (AES-256-GCM-CHUNKED/1 envelope)",
            "error": "PK_SNAPSHOT_ERROR/1 - stable public error envelope"
        },
        threats=[
            "Cross-tenant restore leaking guest memory",
            "Cross-workload or cross-environment restore leaking guest state",
