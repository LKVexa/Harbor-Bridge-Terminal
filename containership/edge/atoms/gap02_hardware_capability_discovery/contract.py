...
def build() -> Contract:
    ...
    return Contract(
        ...
        dependencies=[
            Dependency("GAP-06 Device identity and attestation", "upstream", "Attests the node identity and hardware-rooted evidence used to authorize report signing"),
            Dependency("GAP-07 Trust and signature service", "upstream", "Signs and verifies the canonical capability report envelope"),
        ...
        interfaces={
            "probe": "PK_CAPABILITY_PROBE/1 - run and record one capability probe (schemas/PK_CAPABILITY_PROBE_1.schema.json)",
            "report": "PK_NODE_CAPABILITIES/1 - capability facts plus consumer freshness (schemas/PK_NODE_CAPABILITIES_1.schema.json)",
            "schedule": "PK_PROBE_SCHEDULE/1 - which probes run when (schemas/PK_PROBE_SCHEDULE_1.schema.json)",
            "inventory": "PK_HARDWARE_INVENTORY/1 - privacy-minimized host facts feeding probes (schemas/PK_HARDWARE_INVENTORY_1.schema.json)"
        },
        threats=[
