"""Conformance tests for INV-11 (inv11_interface_contract_language).
...
When this file is executed directly, a missing ``pk_core`` is a hard preflight
failure (exit 2) rather than a false-green test result.
"""
...
class ConformanceTest(unittest.TestCase):
    ...
    def test_checks_survive_optimised_mode(self):
        ...
        ) % ([path for path in sys.path[:3]], module.__name__)
        out = subprocess.run(
...
if __name__ == "__main__":
    if pk_core is None:
        sys.stderr.write(
            "ERROR: pk_core is required for INV-11 conformance tests. "
            "Set PK_CORE_PATH to the estate package root.\n"
        )
        raise SystemExit(2)
    unittest.main()
