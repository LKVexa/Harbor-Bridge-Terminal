...
def build() -> Contract:
    """Return the production contract for INV-55 (requires pk_core)."""
    ...
    return Contract(
        ...
        source_of_truth=("The configured secret provider owns secret versions and revocation state; this component "
                         "treats its broker-mediated lease as valid only until expiry, revocation, or version retirement."),
        assumptions=[
            ...
            "Lease expiry gates broker-mediated use; plaintext already delivered to application code cannot be retroactively erased by this component",
            "Production time decisions use a monotonic clock and fail closed on detected rollback"
        ],
        boundaries={
        ...
        threats=[
            "Secret value written to a log or diagnostic",
            "Secret value materialized through inherited string, formatting, or serialization behavior",
            "Application reading another's secret or replaying a lease in the wrong context",
            "Leaked value usable beyond its provider-enforced lifetime",
            "Clock rollback extending an otherwise expired lease",
