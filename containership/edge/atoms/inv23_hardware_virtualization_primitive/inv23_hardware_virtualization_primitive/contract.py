...
def build() -> Contract:
    ...
    return Contract(
        ...
        owns=[
            ...
            "Exclusivity of the project-defined virtualization slot (the KVM/WHPX/HVF APIs themselves are not exclusive)",
            "Refusal when the primitive is claimed by another hypervisor",
        ],
        not_owns=["The VMM itself", "Guest lifecycle", "Device emulation", "Isolation policy", "Scheduling"],
        dependencies=[
        ...
        optional=["IOMMU presence reporting", "Extension feature-bit detail", "Performance counter availability"],
        non_goals=[
            "Implementing a VMM",
        ...
        interfaces={
            "probe": "PK_VIRT_PRIMITIVE/2 - detect and report the virtualization primitive (schemas/PK_VIRT_PRIMITIVE-2.schema.json; legacy /1 frozen)",
            "claim": "PK_VIRT_CLAIM/2 - fenced, lease-based exclusive claim on the project virtualization slot (schemas/PK_VIRT_CLAIM-2.schema.json; legacy /1 frozen)",
        },
        threats=[
