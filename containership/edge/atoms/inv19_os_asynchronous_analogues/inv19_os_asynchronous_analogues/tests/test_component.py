"""Conformance test for INV-19 (inv19_os_asynchronous_analogues) -- stdlib unittest, no network.

Run from the folder that contains this package (and ``pk_core``), e.g.::

    python inv19_os_asynchronous_analogues/tests/test_component.py
...
class DependencyPresenceTest(unittest.TestCase):
    def test_pk_core_is_present_for_production_gate(self):
        # Deliberately a FAILURE (not a skip) so a plain unittest run can never be
        # green without pk_core.  tools/run_gate.py classifies this exact prefix
        # as DEPENDENCY_UNAVAILABLE -> BLOCKED, distinct from a component test
        # failure (MC-01).
        self.assertIsNotNone(
            pk_core,
            "DEPENDENCY_UNAVAILABLE: pk_core is required to execute the 100-item pk_core "
            "conformance gate; see deps/pk_core.lock.json and tools/pk_core_preflight.py",
        )


@unittest.skipIf(pk_core is None, "BLOCKED: pk_core not importable (DEPENDENCY_UNAVAILABLE)")
