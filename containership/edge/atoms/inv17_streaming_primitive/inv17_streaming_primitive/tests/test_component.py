"""Conformance test for INV-17 (inv17_streaming_primitive) -- stdlib unittest, no network.

Run from the folder that contains this package (and ``pk_core``), e.g.::

    python inv17_streaming_primitive/tests/test_component.py
...
ROOT = PKG_DIR.parent
for p in filter(None, [os.environ.get("PK_CORE_PATH"), str(PKG_DIR / "vendor"), str(ROOT.parent), str(ROOT)]):
