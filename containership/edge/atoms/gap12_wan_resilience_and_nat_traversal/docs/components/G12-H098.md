# G12-H098 — Fleet-scale retry-storm test (P1)

_Generated from `evidence/registry.py`; lifecycle **validation**._

## Normative requirement
- **Success:** a 10,000-peer storm never synchronises.
...
- **Unsupported:** a real fleet — reported as `UNSUPPORTED` / NOT-EVIDENCED, never emulated.
- **Policy rejection:** `POLICY_*` reason codes (egress, mechanism disabled, quarantine, untrusted peer, config rejected).

## Interfaces
- `tests.test_h_engineering.RetryStormTest`
