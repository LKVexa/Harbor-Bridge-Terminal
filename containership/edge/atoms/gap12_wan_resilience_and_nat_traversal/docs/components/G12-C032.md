# G12-C032 — Available-bandwidth estimation (P1)

_Generated from `evidence/registry.py`; lifecycle **validation**._

## Normative requirement
- **Success:** an estimate that ages to unknown and is disabled on metered links.
...
- **Unsupported:** the probe train sender (caller) — reported as `UNSUPPORTED` / NOT-EVIDENCED, never emulated.
- **Policy rejection:** `POLICY_*` reason codes (egress, mechanism disabled, quarantine, untrusted peer, config rejected).

## Interfaces
- `wan.quality.BandwidthEstimator`
...
- `max_probe_bytes` (validated at construction; see module docstring for units/ranges)
