# G12-B026 — MSS/MTU policy integration (P1)

_Generated from `evidence/registry.py`; lifecycle **validation**._

## Normative requirement
- **Success:** usable MTU/MSS after encapsulation overhead, refusing < 576.
...
- **Unsupported:** clamping packets (policy output only) — reported as `UNSUPPORTED` / NOT-EVIDENCED, never emulated.
- **Policy rejection:** `POLICY_*` reason codes (egress, mechanism disabled, quarantine, untrusted peer, config rejected).

## Interfaces
- `wan.netenv.usable_mtu`
- `wan.netenv.mss_for`
