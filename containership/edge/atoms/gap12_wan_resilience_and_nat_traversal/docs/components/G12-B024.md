# G12-B024 — Captive-portal / walled-garden detection (P1)

_Generated from `evidence/registry.py`; lifecycle **validation**._

## Normative requirement
- **Success:** open | captive | blocked | unknown from a connectivity probe.
...
- **Unsupported:** the probe HTTP client (caller) — reported as `UNSUPPORTED` / NOT-EVIDENCED, never emulated.
- **Policy rejection:** `POLICY_*` reason codes (egress, mechanism disabled, quarantine, untrusted peer, config rejected).

## Interfaces
- `wan.netenv.captive_verdict`

## Dependencies and trust boundaries
- connectivity-check endpoint

## Configurable parameters
- `expected_status` (validated at construction; see module docstring for units/ranges)
- `expected_body` (validated at construction; see module docstring for units/ranges)
