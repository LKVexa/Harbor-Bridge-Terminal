# G12-B027 — Local firewall reachability diagnostics (P1)

_Generated from `evidence/registry.py`; lifecycle **validation**._

## Normative requirement
- **Success:** an explanation with reason codes and an empty mutation list.
...
- **Unsupported:** active probing (inputs are observations) — reported as `UNSUPPORTED` / NOT-EVIDENCED, never emulated.
- **Policy rejection:** `POLICY_*` reason codes (egress, mechanism disabled, quarantine, untrusted peer, config rejected).

## Interfaces
- `wan.netenv.diagnose_firewall`
