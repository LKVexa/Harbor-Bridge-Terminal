# G12-H094 — Fuzz tests (P0)

_Generated from `evidence/registry.py`; lifecycle **validation**._

## Normative requirement
- **Success:** structure-aware fuzzing with corpus and limits.
...
- **Unsupported:** coverage-guided fuzzing (no atheris) — reported as `UNSUPPORTED` / NOT-EVIDENCED, never emulated.
- **Policy rejection:** `POLICY_*` reason codes (egress, mechanism disabled, quarantine, untrusted peer, config rejected).

## Interfaces
- `tests.test_h_engineering.FuzzTest`
