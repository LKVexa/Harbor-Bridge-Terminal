# G12-D044 — End-to-end authenticated encryption integration (P0)

_Generated from `evidence/registry.py`; lifecycle **validation**._

## Normative requirement
- **Success:** AEAD frames that only the bound peer/direction/epoch can open.
...
- **Unsupported:** channel establishment (key agreement comes from GAP-06/INV-36) — reported as `UNSUPPORTED` / NOT-EVIDENCED, never emulated.
- **Policy rejection:** `POLICY_*` reason codes (egress, mechanism disabled, quarantine, untrusted peer, config rejected).

## Interfaces
- `wan.security.SecureChannel`

## Dependencies and trust boundaries
- cryptography==46.0.7 (optional extra)
...
- AUTH_INTEGRITY / AUTH_REPLAY on open()
