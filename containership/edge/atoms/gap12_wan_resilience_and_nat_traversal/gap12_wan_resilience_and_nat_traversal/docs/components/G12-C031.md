# G12-C031 — Latency/loss/jitter measurement (P0)

_Generated from `evidence/registry.py`; lifecycle **validation**._

## Normative requirement
- **Success:** RTT/loss/jitter summaries only above min_samples, round-trip only.
...
- **Unsupported:** one-way delay — reported as `UNSUPPORTED` / NOT-EVIDENCED, never emulated.
- **Policy rejection:** `POLICY_*` reason codes (egress, mechanism disabled, quarantine, untrusted peer, config rejected).

## Interfaces
- `wan.quality.QualityWindow`

## Dependencies and trust boundaries
- none declared

## Configurable parameters
- `size` (validated at construction; see module docstring for units/ranges)
- `min_samples` (validated at construction; see module docstring for units/ranges)
