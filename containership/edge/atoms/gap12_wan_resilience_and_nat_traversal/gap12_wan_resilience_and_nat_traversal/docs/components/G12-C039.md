# G12-C039 — Relay quota/budget enforcement (P0)

_Generated from `evidence/registry.py`; lifecycle **validation**._

## Normative requirement
- **Success:** reservations never exceed the hard limit under concurrency.
...
- **Unsupported:** authoritative provider counters (input) — reported as `UNSUPPORTED` / NOT-EVIDENCED, never emulated.
- **Policy rejection:** `POLICY_*` reason codes (egress, mechanism disabled, quarantine, untrusted peer, config rejected).

## Interfaces
- `wan.quality.RelayQuota`

## Dependencies and trust boundaries
- none declared

## Configurable parameters
- `soft` (validated at construction; see module docstring for units/ranges)
- `hard` (validated at construction; see module docstring for units/ranges)
