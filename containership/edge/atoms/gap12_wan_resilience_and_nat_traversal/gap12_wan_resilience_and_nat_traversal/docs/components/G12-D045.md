# G12-D045 — Key acquisition/rotation/revocation behavior (P0)

_Generated from `evidence/registry.py`; lifecycle **validation**._

## Normative requirement
- **Success:** epoch rotation installs new directional keys and rejects old-epoch frames.
...
- **Unsupported:** key distribution/revocation service — reported as `UNSUPPORTED` / NOT-EVIDENCED, never emulated.
- **Policy rejection:** `POLICY_*` reason codes (egress, mechanism disabled, quarantine, untrusted peer, config rejected).

## Interfaces
- `wan.security.SecureChannel.rotate`
