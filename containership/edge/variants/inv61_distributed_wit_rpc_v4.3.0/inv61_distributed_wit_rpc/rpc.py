...
class EndpointStats:
    ...
    def bump(self, name: str) -> None:
        """Atomically increment one counter (M25: counters are thread-safe)."""
        with self._lock:
...
class Endpoint:
    ...
    def handle(self, incoming: object, now: float) -> dict[str, Any]:
        ...
        if frame["interface"] != self.interface:
            self.stats.bump("unknown_function")
            return {"error": "unknown-interface"}
        if frame["version"] != self.version:
            self.stats.bump("version_mismatch")
        ...
        if frame["fp"] != fingerprint(fn[0], fn[1]):
            self.stats.bump("signature_mismatch")
            return {"error": "signature-mismatch"}

        try:
            result = fn[2](*frame["args"])
        except Exception:
            self.stats.bump("callee_trap")
