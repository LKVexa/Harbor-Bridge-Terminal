...
> **v4.3.0 status (2026-09-23):** every item below now has an implementation/artifact and an RTM disposition. Residual work is
> item-level and tracked in `governance/CHECKLIST_STATUS.md` and waivers W-001…W-015 in `governance/WAIVERS.json`. The inventory
> below is kept unchanged as the historical v4.2.0 baseline.
