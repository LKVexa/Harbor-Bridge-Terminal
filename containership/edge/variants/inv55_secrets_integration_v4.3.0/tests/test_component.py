"""Conformance test for INV-55 (inv55_secrets_integration) -- stdlib unittest, no network.

Run from the folder that contains this package (and ``pk_core``), e.g.::

    python inv55_secrets_integration/tests/test_component.py

Set PK_CORE_PATH if ``pk_core`` lives elsewhere (e.g. ``<project>/pk``).
"""
import importlib, os, pathlib, subprocess, sys, unittest

PKG_DIR = pathlib.Path(__file__).resolve().parents[1] / "inv55_secrets_integration"
ROOT = PKG_DIR.parent
for p in filter(None, [os.environ.get("PK_CORE_PATH"), str(ROOT)]):
