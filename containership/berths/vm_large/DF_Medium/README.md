# vm_large / DF_Medium -- node slot N_MEDIUM

Hollow slot of berth `vm_large` (UC-2.1.3). It holds no VM: the engine is `hold/DF_Medium.zip`, extracted and built once under `_engines/`. This slot holds the 359 script(s) (3387916 bytes) the sort policy placed on `N_MEDIUM` (rules {"S5": 177, "S6": 109, "S8": 73}), under `cargo/` with their original relative paths, plus `cargo/CARGO_MANIFEST.json`, `node/NODE.pal` (this berth's declaration of the node, sealed), `SLOT.json`, and this container's own .tif fabric: `fabric/GENESIS.fabric.tif` + `fabric/FABRIC.json` (tick 0, sealed) and, once BUILD or RUN made it, the live image `_fabric/DF_Medium.fabric.tif` (state, outside the seal): tile 0 the state, tiles 1-2 the berth's declaration as row-word cells, tile 3 the seals. A tick (`../../../uc.py run vm_large`) reads it, executes it on this engine, and appends a frame.
