...
> Shell qualification includes dense city, sparse wilderness, underground/interior, transport corridor, and rapid oscillation profiles.
...
1. **Baseline the exact requirement.** Capture the current PARTIAL record for `RCPW-13-R19`, all existing tests that touch it, and canonical/ledger digests for a minimal deterministic scenario.
...
3. **Close the implementation gap.** Implement the behavior required by `RCPW-13-R19` in the real target paths. If a bounded mechanism already exists, raise its fidelity until the entire requirement—not only a simplified approximation—is exercised.
...
13. **Evidence receipt.** Emit `vm/world/evidence/remediation_315/RCPW-13-R19.json` conforming to `00_COMMON_EVIDENCE_CONTRACT.md`, including exact commands, exit codes, observed values, digests, profile, hashes, and blocker field.
14. **Promotion decision.** Change `RCPW-13-R19` from PARTIAL to OPERATIONAL only if its evidence predicate passes. If any mandatory high-profile environment is unavailable, retain the appropriate profile cell as PARTIAL/BLOCKED and state the missing evidence explicitly.

## Acceptance criteria

- the exact statement `Shell qualification includes dense city, sparse wilderness, underground/interior, transport corridor, and rapid oscillation profiles.` is directly asserted by executable evidence;
...
- `vm/world/evidence/remediation_315/RCPW-13-R19.json`
- `vm/world/evidence/remediation_315/RCPW-13-R19.log`
