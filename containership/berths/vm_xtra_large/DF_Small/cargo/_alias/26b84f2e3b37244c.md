...
> Fold/LOD/reference metadata carried into rendering is version stamped so mixed-frame state is detected.
...
1. **Baseline the exact requirement.** Capture the current PARTIAL record for `RCPW-11-R22`, all existing tests that touch it, and canonical/ledger digests for a minimal deterministic scenario.
...
3. **Close the implementation gap.** Implement the behavior required by `RCPW-11-R22` in the real target paths. If a bounded mechanism already exists, raise its fidelity until the entire requirement—not only a simplified approximation—is exercised.
...
11. **LOD_TRANSITION.** Define the exact conservation vector and transition payload for source/target representations.
12. **LOD_TRANSITION.** Cycle promotion/demotion repeatedly, change update cadence, inject cancellation/failure, and compare canonical digests for drift.
...
19. **Evidence receipt.** Emit `vm/world/evidence/remediation_315/RCPW-11-R22.json` conforming to `00_COMMON_EVIDENCE_CONTRACT.md`, including exact commands, exit codes, observed values, digests, profile, hashes, and blocker field.
20. **Promotion decision.** Change `RCPW-11-R22` from PARTIAL to OPERATIONAL only if its evidence predicate passes. If any mandatory high-profile environment is unavailable, retain the appropriate profile cell as PARTIAL/BLOCKED and state the missing evidence explicitly.

## Acceptance criteria

- the exact statement `Fold/LOD/reference metadata carried into rendering is version stamped so mixed-frame state is detected.` is directly asserted by executable evidence;
...
- `vm/world/evidence/remediation_315/RCPW-11-R22.json`
- `vm/world/evidence/remediation_315/RCPW-11-R22.log`
