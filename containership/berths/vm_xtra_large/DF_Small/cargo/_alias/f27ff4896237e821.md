...
> Qualification includes duplicate/reordered completion messages, retries, cancellations, partition moves, and crash at each transition phase.
...
1. **Baseline the exact requirement.** Capture the current PARTIAL record for `RCPW-14-R24`, all existing tests that touch it, and canonical/ledger digests for a minimal deterministic scenario.
...
3. **Close the implementation gap.** Implement the behavior required by `RCPW-14-R24` in the real target paths. If a bounded mechanism already exists, raise its fidelity until the entire requirement—not only a simplified approximation—is exercised.
...
5. **SAVE_RECOVERY.** Checkpoint at deterministic canonical barriers and record dirty partitions/ledger cut/version metadata.
6. **SAVE_RECOVERY.** Inject interruption/corruption/stale fragments, recover repeatedly, and compare canonical checkpoint digests plus recovery-time objective.
...
13. **Evidence receipt.** Emit `vm/world/evidence/remediation_315/RCPW-14-R24.json` conforming to `00_COMMON_EVIDENCE_CONTRACT.md`, including exact commands, exit codes, observed values, digests, profile, hashes, and blocker field.
14. **Promotion decision.** Change `RCPW-14-R24` from PARTIAL to OPERATIONAL only if its evidence predicate passes. If any mandatory high-profile environment is unavailable, retain the appropriate profile cell as PARTIAL/BLOCKED and state the missing evidence explicitly.

## Acceptance criteria

- the exact statement `Qualification includes duplicate/reordered completion messages, retries, cancellations, partition moves, and crash at each transition phase.` is directly asserted by executable evidence;
...
- `vm/world/evidence/remediation_315/RCPW-14-R24.json`
- `vm/world/evidence/remediation_315/RCPW-14-R24.log`
