...
> Uncertainty/confidence attached to experimental mathematical mappings propagates into diagnostics and gate status.
...
1. **Baseline the exact requirement.** Capture the current PARTIAL record for `RCPW-16-R13`, all existing tests that touch it, and canonical/ledger digests for a minimal deterministic scenario.
...
3. **Close the implementation gap.** Implement the behavior required by `RCPW-16-R13` in the real target paths. If a bounded mechanism already exists, raise its fidelity until the entire requirement—not only a simplified approximation—is exercised.
...
5. **MATHEMATICAL_VALIDATION.** Declare units/domains/ownership/precision for every involved variable or operator and reject invalid mixed-domain operations.
6. **MATHEMATICAL_VALIDATION.** Run golden vectors, boundary/NaN/Inf/overflow cases, sensitivity tests, and compare adapter-disabled canonical truth.
...
10. **Evidence receipt.** Emit `vm/world/evidence/remediation_315/RCPW-16-R13.json` conforming to `00_COMMON_EVIDENCE_CONTRACT.md`, including exact commands, exit codes, observed values, digests, profile, hashes, and blocker field.
11. **Promotion decision.** Change `RCPW-16-R13` from PARTIAL to OPERATIONAL only if its evidence predicate passes. If any mandatory high-profile environment is unavailable, retain the appropriate profile cell as PARTIAL/BLOCKED and state the missing evidence explicitly.

## Acceptance criteria

- the exact statement `Uncertainty/confidence attached to experimental mathematical mappings propagates into diagnostics and gate status.` is directly asserted by executable evidence;
...
- `vm/world/evidence/remediation_315/RCPW-16-R13.json`
- `vm/world/evidence/remediation_315/RCPW-16-R13.log`
