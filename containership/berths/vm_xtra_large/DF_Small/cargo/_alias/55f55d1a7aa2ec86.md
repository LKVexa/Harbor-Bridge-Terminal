...
> The final qualification executes a content-scale world containing wilderness, multiple settlements, interiors, transport networks, ecology, economy, law/reputation, weather, narrative events, persistent agents, and long-lived history under continuous folding.
...
- Test families: `FOLD_PROPERTY, IDENTITY_PERSISTENCE, CAUSAL_REPLAY, ECOLOGY_CONSERVATION, ECONOMY_CONSERVATION, NARRATIVE_KNOWLEDGE, SCALE_SOAK`

## QUORUM requirement-specific prompt

Using QUORUM, modify the actual RC-PW 7 VM candidate to make **RCPW-20-R19** fully evidence-backed OPERATIONAL. The exact semantic obligation is: **The final qualification executes a content-scale world containing wilderness, multiple settlements, interiors, transport networks, ecology, economy, law/reputation, weather, narrative events, persistent agents, and long-lived history under continuous folding.** Treat this as a T2_HIGH_FIDELITY remediation. Preserve canonical identity, topology, time, ownership, causality, save/replay compatibility, and all existing passing behavior. Use MSSL for semantic authority; use Columned LCTL/canonical LCTL when this behavior belongs in native guest execution; use the hosted VM world-service layer where that is the repository's declared architecture. Do not satisfy this requirement with prose, mocks, prewritten evidence, or a status edit.
...
1. **Baseline the exact requirement.** Capture the current PARTIAL record for `RCPW-20-R19`, all existing tests that touch it, and canonical/ledger digests for a minimal deterministic scenario.
...
3. **Close the implementation gap.** Implement the behavior required by `RCPW-20-R19` in the real target paths. If a bounded mechanism already exists, raise its fidelity until the entire requirement—not only a simplified approximation—is exercised.
...
11. **ECOLOGY_CONSERVATION.** Create a small high-fidelity reference ecology and a remote/cohort abstraction using identical seeds.
12. **ECOLOGY_CONSERVATION.** Compare population/resource conservation, migration topology, deterministic stochastic outcomes, and rehydration error.
13. **ECONOMY_CONSERVATION.** Use conservation-auditable money/inventory/population/logistics fixtures across at least two settlements.
14. **ECONOMY_CONSERVATION.** Inject route loss, shortages, migration, worker/replay boundaries, and verify no duplicate inventory/population or impossible travel-time shortcuts.
15. **NARRATIVE_KNOWLEDGE.** Separate canonical facts, narrative priority/reservations, and observer-local knowledge/belief state.
16. **NARRATIVE_KNOWLEDGE.** Test competing events, stale/false information, bounded priority, off-screen deferral, and replay of the same eligibility/arbitration decision.
17. **SCALE_SOAK.** Define the precise duration/load/capacity profile and success thresholds before execution.
18. **SCALE_SOAK.** Run a sustained stress/soak with checkpoints and fault injection; measure drift, leaks, storage growth, queue stability, and recovery.
...
23. **Evidence receipt.** Emit `vm/world/evidence/remediation_315/RCPW-20-R19.json` conforming to `00_COMMON_EVIDENCE_CONTRACT.md`, including exact commands, exit codes, observed values, digests, profile, hashes, and blocker field.
24. **Promotion decision.** Change `RCPW-20-R19` from PARTIAL to OPERATIONAL only if its evidence predicate passes. If any mandatory high-profile environment is unavailable, retain the appropriate profile cell as PARTIAL/BLOCKED and state the missing evidence explicitly.

## Acceptance criteria

- the exact statement `The final qualification executes a content-scale world containing wilderness, multiple settlements, interiors, transport networks, ecology, economy, law/reputation, weather, narrative events, persistent agents, and long-lived history under continuous folding.` is directly asserted by executable evidence;
...
- `vm/world/evidence/remediation_315/RCPW-20-R19.json`
- `vm/world/evidence/remediation_315/RCPW-20-R19.log`
