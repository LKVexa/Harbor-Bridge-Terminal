...
> Penteract extended-state schema may encode world-age/history relevance only through explicitly typed components or adapters, never by overloading canonical spatial coordinates.
...
- Test families: `VERSION_MIGRATION, REFERENCE_PRECISION, CAUSAL_REPLAY, NARRATIVE_KNOWLEDGE, RESOURCE_STRESS, MATHEMATICAL_VALIDATION, AUTONOMOUS_HISTORY`

## QUORUM requirement-specific prompt

Using QUORUM, modify the actual RC-PW 7 VM candidate to make **RCPW-16-R25** fully evidence-backed OPERATIONAL. The exact semantic obligation is: **Penteract extended-state schema may encode world-age/history relevance only through explicitly typed components or adapters, never by overloading canonical spatial coordinates.** Treat this as a T1_DIRECT_PROOF remediation. Preserve canonical identity, topology, time, ownership, causality, save/replay compatibility, and all existing passing behavior. Use MSSL for semantic authority; use Columned LCTL/canonical LCTL when this behavior belongs in native guest execution; use the hosted VM world-service layer where that is the repository's declared architecture. Do not satisfy this requirement with prose, mocks, prewritten evidence, or a status edit.
...
1. **Baseline the exact requirement.** Capture the current PARTIAL record for `RCPW-16-R25`, all existing tests that touch it, and canonical/ledger digests for a minimal deterministic scenario.
...
3. **Close the implementation gap.** Implement the behavior required by `RCPW-16-R25` in the real target paths. If a bounded mechanism already exists, raise its fidelity until the entire requirement—not only a simplified approximation—is exercised.
...
13. **RESOURCE_STRESS.** Declare explicit budgets, pressure metrics, degradation order, hysteresis/backpressure, and starvation bounds.
14. **RESOURCE_STRESS.** Increase load past budget, capture p50/p95/p99/high-water/backlog recovery, and prove authoritative state survives overload.
15. **MATHEMATICAL_VALIDATION.** Declare units/domains/ownership/precision for every involved variable or operator and reject invalid mixed-domain operations.
16. **MATHEMATICAL_VALIDATION.** Run golden vectors, boundary/NaN/Inf/overflow cases, sensitivity tests, and compare adapter-disabled canonical truth.
17. **AUTONOMOUS_HISTORY.** Create an exact historical cut plus long-horizon/archive/rehydration path with provenance classes.
18. **AUTONOMOUS_HISTORY.** Compare fine-step vs abstract evolution, old-save continuation, compaction/rehydration, and pre-existing history digests after expansion.
...
21. **Regression.** Run the relevant stage suite plus base VM tests, existing RC-PW tests, blocker-remediation tests, world compiler determinism, LCTL verification, and any upstream/downstream stages affected by the change.
22. **Evidence receipt.** Emit `vm/world/evidence/remediation_315/RCPW-16-R25.json` conforming to `00_COMMON_EVIDENCE_CONTRACT.md`, including exact commands, exit codes, observed values, digests, profile, hashes, and blocker field.
23. **Promotion decision.** Change `RCPW-16-R25` from PARTIAL to OPERATIONAL only if its evidence predicate passes. If any mandatory high-profile environment is unavailable, retain the appropriate profile cell as PARTIAL/BLOCKED and state the missing evidence explicitly.

## Acceptance criteria

- the exact statement `Penteract extended-state schema may encode world-age/history relevance only through explicitly typed components or adapters, never by overloading canonical spatial coordinates.` is directly asserted by executable evidence;
...
- `vm/world/evidence/remediation_315/RCPW-16-R25.json`
- `vm/world/evidence/remediation_315/RCPW-16-R25.log`
