...
> Simulation LOD authority defines subsystem-specific fidelity contracts for AI, ecology, economy, weather, navigation, combat, transport, and narrative events.
...
- Test families: `LOD_TRANSITION, CAUSAL_REPLAY, ECOLOGY_CONSERVATION, ECONOMY_CONSERVATION, NARRATIVE_KNOWLEDGE`

## QUORUM requirement-specific prompt

Using QUORUM, modify the actual RC-PW 7 VM candidate to make **RCPW-04-R15** fully evidence-backed OPERATIONAL. The exact semantic obligation is: **Simulation LOD authority defines subsystem-specific fidelity contracts for AI, ecology, economy, weather, navigation, combat, transport, and narrative events.** Treat this as a T2_HIGH_FIDELITY remediation. Preserve canonical identity, topology, time, ownership, causality, save/replay compatibility, and all existing passing behavior. Use MSSL for semantic authority; use Columned LCTL/canonical LCTL when this behavior belongs in native guest execution; use the hosted VM world-service layer where that is the repository's declared architecture. Do not satisfy this requirement with prose, mocks, prewritten evidence, or a status edit.
...
1. **Baseline the exact requirement.** Capture the current PARTIAL record for `RCPW-04-R15`, all existing tests that touch it, and canonical/ledger digests for a minimal deterministic scenario.
...
3. **Close the implementation gap.** Implement the behavior required by `RCPW-04-R15` in the real target paths. If a bounded mechanism already exists, raise its fidelity until the entire requirement—not only a simplified approximation—is exercised.
...
5. **LOD_TRANSITION.** Define the exact conservation vector and transition payload for source/target representations.
6. **LOD_TRANSITION.** Cycle promotion/demotion repeatedly, change update cadence, inject cancellation/failure, and compare canonical digests for drift.
...
9. **ECOLOGY_CONSERVATION.** Create a small high-fidelity reference ecology and a remote/cohort abstraction using identical seeds.
10. **ECOLOGY_CONSERVATION.** Compare population/resource conservation, migration topology, deterministic stochastic outcomes, and rehydration error.
...
13. **NARRATIVE_KNOWLEDGE.** Separate canonical facts, narrative priority/reservations, and observer-local knowledge/belief state.
14. **NARRATIVE_KNOWLEDGE.** Test competing events, stale/false information, bounded priority, off-screen deferral, and replay of the same eligibility/arbitration decision.
...
19. **Evidence receipt.** Emit `vm/world/evidence/remediation_315/RCPW-04-R15.json` conforming to `00_COMMON_EVIDENCE_CONTRACT.md`, including exact commands, exit codes, observed values, digests, profile, hashes, and blocker field.
20. **Promotion decision.** Change `RCPW-04-R15` from PARTIAL to OPERATIONAL only if its evidence predicate passes. If any mandatory high-profile environment is unavailable, retain the appropriate profile cell as PARTIAL/BLOCKED and state the missing evidence explicitly.

## Acceptance criteria

- the exact statement `Simulation LOD authority defines subsystem-specific fidelity contracts for AI, ecology, economy, weather, navigation, combat, transport, and narrative events.` is directly asserted by executable evidence;
...
- `vm/world/evidence/remediation_315/RCPW-04-R15.json`
- `vm/world/evidence/remediation_315/RCPW-04-R15.log`
