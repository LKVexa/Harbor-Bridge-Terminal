...
> Civilization/economy authority includes household, workplace, production-site, merchant, transport, and jurisdiction relationships in the canonical world graph.
...
1. **Baseline the exact requirement.** Capture the current PARTIAL record for `RCPW-08-R15`, all existing tests that touch it, and canonical/ledger digests for a minimal deterministic scenario.
...
3. **Close the implementation gap.** Implement the behavior required by `RCPW-08-R15` in the real target paths. If a bounded mechanism already exists, raise its fidelity until the entire requirement—not only a simplified approximation—is exercised.
...
7. **SCALE_SOAK.** Define the precise duration/load/capacity profile and success thresholds before execution.
8. **SCALE_SOAK.** Run a sustained stress/soak with checkpoints and fault injection; measure drift, leaks, storage growth, queue stability, and recovery.
...
13. **Evidence receipt.** Emit `vm/world/evidence/remediation_315/RCPW-08-R15.json` conforming to `00_COMMON_EVIDENCE_CONTRACT.md`, including exact commands, exit codes, observed values, digests, profile, hashes, and blocker field.
14. **Promotion decision.** Change `RCPW-08-R15` from PARTIAL to OPERATIONAL only if its evidence predicate passes. If any mandatory high-profile environment is unavailable, retain the appropriate profile cell as PARTIAL/BLOCKED and state the missing evidence explicitly.

## Acceptance criteria

- the exact statement `Civilization/economy authority includes household, workplace, production-site, merchant, transport, and jurisdiction relationships in the canonical world graph.` is directly asserted by executable evidence;
...
- `vm/world/evidence/remediation_315/RCPW-08-R15.json`
- `vm/world/evidence/remediation_315/RCPW-08-R15.log`
